...
public class SuggestionServiceImpl implements SuggestionService{
	...
	public PageList<SuggestionVO> queryPagedResult(String queryStr,
		...
		PageList<SuggestionVO> groups = queryChannel.queryPagedResult(
				SuggestionVO.class, queryStr, params, currentPage, pageSize,
				sortString);
		...
		PageList<SuggestionVO> vos = new PageList<SuggestionVO>(tmp,
				groups.getPaginator());
		groups.clear();
		return vos;
	}
	...
	public PageList<SuggestionVO> selectPagedByExample(BaseExample example,
		...
		PageList<Suggestion> list = queryChannel.selectPagedByExample(
				Suggestion.class, example, currentPage, pageSize, sortString);
		PageList<SuggestionVO> vos = new PageList<SuggestionVO>(
				list.getPaginator());
	...
	public boolean delete(String[] ids) {
		...
		try {
			for (String id : ids) {
				Suggestion au = BeanUtils.getNewInstance(Suggestion.get(id),
						Suggestion.class);
				au.remove();
			}
			return true;
	...
	@Override
	public List<SuggestionVO> queryResult(String queryStr,
			Map<String, Object> params) throws MRTException {
		
		List<Suggestion> resources = queryChannel.queryResult(
				Suggestion.class, queryStr, params);
