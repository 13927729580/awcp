...
public abstract class WebContralBase extends BaseController {
	...
	public final String getDoType() {
		// 获得执行的方法.
		if (StringUtils.isNotBlank(this.GetRequestVal("DoType"))) {
			return this.GetRequestVal("DoType");
		}else if(StringUtils.isNotBlank(this.GetRequestVal("Action"))){
			return this.GetRequestVal("Action");
		}else if(StringUtils.isNotBlank(this.GetRequestVal("action"))){
			return this.GetRequestVal("action");
		}else if(StringUtils.isNotBlank(this.GetRequestVal("Method"))){
			return this.GetRequestVal("Method");
		}else{
			throw new RuntimeException("执行的方法名不能为空");
		}
