...
import cn.org.awcp.core.utils.Blowfish;
...
public class AppACE extends WebContralBase{
    ...
    public String Login_Submit() throws UnsupportedEncodingException
    {
		...
		//TODO 此处密码校验更改
		if (!pass.equals(Blowfish.getInstance().decryptString(emp.getPass())))
		{
			return "err@用户名或者密码错误.";
		}
	...
	*/
	...
	public final String DB_GenerReturnWorks()
	{
		...
		rws.Retrieve(BP.WF.ReturnWorkAttr.ReturnToNode, this.getFK_Node(), BP.WF.ReturnWorkAttr.WorkID, this.WorkID, BP.WF.ReturnWorkAttr.RDT);
		StringBuilder append = new StringBuilder();
		append.append("[");
		if (rws.size() != 0)
		{
