	...
	*/
	public final void DeleteToNodesData(Nodes nds)
	{
		...
		for (Node nd : nds.ToJavaList())
		{
			...
			// 删除表单附件信息。
			BP.DA.DBAccess.RunSQL("DELETE FROM Sys_FrmAttachmentDB WHERE RefPKVal=" + dbStr + "WorkID AND FK_MapData=" + dbStr + "FK_MapData ", "WorkID", (new Long(this.getWorkID())).toString(), "FK_MapData", "ND" + nd.getNodeID());
			// 删除签名信息。
			BP.DA.DBAccess.RunSQL("DELETE FROM Sys_FrmEleDB WHERE RefPKVal=" + dbStr + "WorkID AND FK_MapData=" + dbStr + "FK_MapData ", "WorkID", (new Long(this.getWorkID())).toString(), "FK_MapData", "ND" + nd.getNodeID());
	...
	*/
	private GenerWorkerLists InitWorkerLists(WorkNode town, DataTable dt, long fid)
	{
		...
		else
		{
			...
			//增加天数. 考虑到了节假日.                
			dtOfShould = Glo.AddDayHoursSpan(new Date(), this.town.getHisNode().getTimeLimit(),this.town.getHisNode().getTSpanMinues());
		}
		...
		else
		{
			//计算警告日期。
			// 增加小时数. 考虑到了节假日.
			dtOfWarning = Glo.AddDayHoursSpan(new Date(), this.town.getHisNode().getTimeLimit(), this.town.getHisNode().getTSpanMinues());
		}
...
//                     
			...
			if (town.getHisNode().getIsGuestNode())
			{
				...
				else
				{
					...
					{
						throw new RuntimeException("@当前节点["+this.town.getHisNode().getName()+"]是中间节点，并且是外部用户处理节点，您需要正确的设置，这个外部用户接受人规则。");
					}
				}
...
//                         
		...
		else
		{
			...
			//写入到日志.
			this.AddToTrack(at, this.getExecer(), "多人接受(见信息栏)", town.getHisNode().getNodeID(), town.getHisNode().getName(), info, this.getndFrom(), null, emps);
		}
	...
	*/
	public final Node NodeSend_GenerNextStepNode()
	{
		...
		//写入到达信息.
		//this.addMsg(SendReturnMsgFlag.VarToNodeID, (new Integer(nd.getNodeID())).toString(), (new Integer(nd.getNodeID())).toString(), SendReturnMsgType.SystemMsg);
...
///#warning 被 zhoupeng 删除 2014-06-20, 不应该存在这里.
					...
					else
					{
						...
						if (athDesc.size() > 0)
						{
							FrmAttachmentDBs athDBs = new FrmAttachmentDBs("ND" + this.getHisNode().getNodeID(), (new Long(this.getWorkID())).toString());
							int idx = 0;
							if (athDBs.size() > 0)
							{
						...
						{
							FrmImgAthDBs athDBs = new FrmImgAthDBs("ND" + this.getHisNode().getNodeID(), (new Long(this.getWorkID())).toString());
							int idx = 0;
							if (athDBs.size() > 0)
							{
								athDBs.Delete(FrmAttachmentDBAttr.FK_MapData, "ND" + nd.getNodeID(), FrmAttachmentDBAttr.RefPKVal, this.getWorkID());
						...
						if (dtls.size() > 0)
						{
							...
							for (MapDtl dtl : dtls.ToJavaList())
							{
								...
								DBAccess.RunSQL("DELETE FROM " + toDtl.getPTable() + " WHERE RefPK=" + dbStr + "RefPK", "RefPK", (new Long(this.getWorkID())).toString());
									...
									if (toDtl.getIsEnableAthM())
									{
										...
										if (athDBs.size() > 0)
										{
											...
											for (FrmAttachmentDB athDB : athDBs.ToJavaList())
											{
												...
												athDB_N.setMyPK(toDtl.getNo() + "_" + dtCopy.getOID() + "_" + (new Integer(i)).toString());
												athDB_N.setFK_FrmAttachment(athDB_N.getFK_FrmAttachment().replace("ND" + this.getHisNode().getNodeID(), "ND" + nd.getNodeID()));
...
//                                         
			...
			if (nd.getAutoJumpRole1())
			{
				...
				{
					// 如果出现了处理人就是提交人的情况.				
					String sql = "SELECT COUNT(*) FROM WF_GenerWorkerList WHERE FK_Emp='" + dr.getValue(0) + "' AND WorkID=" + this.getWorkID();
			...
			if (nd.getAutoJumpRole2())
			{
				...
				for (DataRow dr : dt.Rows)
				{
					String sql = "SELECT COUNT(*) FROM WF_GenerWorkerList WHERE FK_Emp='" + dr.get(0) + "' AND WorkID=" + this.getWorkID() + " AND FK_Node=" + (beforeSkipNodeID > 0 ? beforeSkipNodeID : prvNodeID); //edited by liuxc,2015-7-13
					if (DBAccess.RunSQLReturnValInt(sql) == 1)
					{
						//这里不处理签名.
						isHave = true;
				...
				if (isHave == true)
				{
					...
					for (FrmImg img : imgs.ToJavaList())
					{
						...
						try
						{
							...
							//设置主键
							String myPK = DotNetToJavaStringHelper.isNullOrEmpty(img.getEnPK()) ? "seal" : img.getEnPK();
	...
	private void Func_CheckCompleteCondition()
	{
		...
		try
		{
			...
			if (this.getHisNode().getIsCCFlow() && this.getHisFlowCompleteConditions().getIsPass())
			{
				...
				//   string mymsgHtml = mymsg + "@查看<img src='" + VirPath + "WF/Img/Btn/PrintWorkRpt.gif' ><a href='" + VirPath + "WF/WFRpt.jsp?WorkID=" + this.HisWork.OID + "&FID=" + this.HisWork.FID + "&FK_Flow=" + this.HisNode.FK_Flow + "' target='_self' >工作轨迹</a>";
	...
	*/
	private void NodeSend_11(Node toND)
	{
		...
		{
			String htmlInfo = String.format("@发送给如下%1$s位处理人,%2$s.",(new Integer(this.HisRememberMe.getNumOfObjs())).toString(), this.getHisGenerWorkFlow().getGuestNo()+ " " + this.getHisGenerWorkFlow().getGuestName());

			String textInfo = String.format("@发送给如下%1$s位处理人,%2$s.",(new Integer(this.HisRememberMe.getNumOfObjs())).toString(), this.getHisGenerWorkFlow().getGuestName());

			this.addMsg(SendReturnMsgFlag.ToEmps, textInfo, htmlInfo);
		}
		else
		{
			String htmlInfo = String.format("@发送给如下%1$s位处理人,%2$s.",(new Integer(this.HisRememberMe.getNumOfObjs())).toString(), this.HisRememberMe.getEmpsExt());

			String textInfo = String.format("@发送给如下%1$s位处理人,%2$s.",(new Integer(this.HisRememberMe.getNumOfObjs())).toString(), this.HisRememberMe.getObjsExt());
		...
		if (this.getHisNode().getHisFormType() != NodeFormType.SDKForm || 1==1)
		{
			if (this.getHisNode().getIsStartNode())
			{
				if (WebUser.getIsWap())
				{
					this.addMsg(SendReturnMsgFlag.ToEmpExt, null, "@<a href='" + getVirPath() + "WF/Wap/MyFlowInfo.jsp?DoType=UnSend&WorkID=" + this.getHisWork().getOID() + "&FK_Flow=" + toND.getFK_Flow() + "'><img src='" + getVirPath() + "WF/Img/Action/UnSend.png' border=0/>撤销本次发送</a>， <a href='" + getVirPath() + "WF/Wap/MyFlow.htm?FK_Flow=" + toND.getFK_Flow() + "&FK_Node=" + Integer.parseInt(toND.getFK_Flow()) + "01'><img src='" + getVirPath() + "WF/Img/New.gif' border=0/>新建流程</a>。", SendReturnMsgType.Info);
				}
				else
				{
					this.addMsg(SendReturnMsgFlag.ToEmpExt, null, "@<a href='" + this.getVirPath() + this.getAppType() + "/MyFlowInfo.jsp?DoType=UnSend&WorkID=" + this.getHisWork().getOID() + "&FK_Flow=" + toND.getFK_Flow() + "'><img src='" + getVirPath() + "WF/Img/Action/UnSend.png' border=0/>撤销本次发送</a>， <a href='" + getVirPath() + "WF/MyFlow.htm?FK_Flow=" + toND.getFK_Flow() + "&FK_Node=" + Integer.parseInt(toND.getFK_Flow()) + "01'><img src='" + getVirPath() + "WF/Img/New.gif' border=0/>新建流程</a>。", SendReturnMsgType.Info);
				}
			}
			else
			{
	...
	private void NodeSend_24_UnSameSheet(Nodes toNDs)
	{
		...
		for (Node nd : toNDs.ToJavaList())
		{
			...
			if (athDBs.size() > 0)
			{
				...
				mydb.Delete(FrmAttachmentDBAttr.FK_MapData, "ND" + nd.getNodeID(), FrmAttachmentDBAttr.RefPKVal, wk.getOID());
...
//                     * 
			...
			if (isGroupMarkWorklist == false)
			{
				...
				if (this.getHisNode().getMapData().getFrmImgAths().size() > 0)
				{
					FrmImgAthDBs frmImgAthDBs = new FrmImgAthDBs("ND" + this.getHisNode().getNodeID(), (new Long(this.getWorkID())).toString());
		...
		if (this.getHisNode().getIsStartNode())
		{
			if (WebUser.getIsWap())
			{
				this.addMsg("UnDoNew", "@<a href='" + getVirPath() + "WF/Wap/MyFlowInfo.jsp?DoType=UnSend&WorkID=" + this.getWorkID() + "&FK_Flow=" + toNode.getFK_Flow() + "'><img src='" + getVirPath() + "WF/Img/Action/UnSend.png' border=0/>撤销本次发送</a>， <a href='" + getVirPath() + "WF/Wap/MyFlow.htm?FK_Flow=" + toNode.getFK_Flow() + "&FK_Node=" + Integer.parseInt(toNode.getFK_Flow()) + "01' ><img src='" + getVirPath() + "WF/Img/New.gif' border=0/>新建流程</a>。");
			}
			else
			{
				this.addMsg("UnDoNew", "@<a href='" + this.getVirPath() + this.getAppType() + "/MyFlowInfo.jsp?DoType=UnSend&WorkID=" + this.getWorkID() + "&FK_Flow=" + toNode.getFK_Flow() + "'><img src='" + getVirPath() + "WF/Img/Action/UnSend.png' border=0/>撤销本次发送</a>， <a href='" + this.getVirPath() + this.getAppType() + "/MyFlow.htm?FK_Flow=" + toNode.getFK_Flow() + "&FK_Node=" + Integer.parseInt(toNode.getFK_Flow()) + "01' ><img src='" + getVirPath() + "WF/Img/New.gif' border=0/>新建流程</a>。");
			}
		}
		else
		{
		...
		switch (this.getHisNode().getHisRunModel())
		{
			...
			case FL: // 2: 分流节点向下发送的
				...
				if (toNDs.size() == 1)
				{
					...
					//加入系统变量.
					this.addMsg(SendReturnMsgFlag.VarToNodeID, (new Integer(toND2.getNodeID())).toString(), (new Integer(toND2.getNodeID())).toString(), SendReturnMsgType.SystemMsg);
			...
			case HL: // 3: 合流节点向下发送 
				...
				//加入系统变量.
				this.addMsg(SendReturnMsgFlag.VarToNodeID, (new Integer(toND3.getNodeID())).toString(), (new Integer(toND3.getNodeID())).toString(), SendReturnMsgType.SystemMsg);
			...
			case FHL: // 4: 分流节点向下发送的 
				...
				//加入系统变量.
				this.addMsg(SendReturnMsgFlag.VarToNodeID, (new Integer(toND4.getNodeID())).toString(), (new Integer(toND4.getNodeID())).toString(), SendReturnMsgType.SystemMsg);
			...
			case SubThread: // 5: 子线程节点向下发送的 
				...
				//加入系统变量.
				this.addMsg(SendReturnMsgFlag.VarToNodeID, (new Integer(toND5.getNodeID())).toString(), (new Integer(toND5.getNodeID())).toString(), SendReturnMsgType.SystemMsg);
	...
	public final boolean CheckFrmIsNotNull()
	{
		...
		if (this.getHisNode().getHisFormType() == NodeFormType.FreeForm)
		{
			...
			for (MapAttr attr : attrs.ToJavaList())
			{
				...
				String str = (String)((row.get(attr.getKeyOfEn()) instanceof String) ? row.get(attr.getKeyOfEn()) : null);
				//如果是检查不能为空 
...
*/
	public final String DealAlertZhuChiRen()
	{
		...
		if (dt.Rows.size() != 1)
			...
			//从会签列表里移动到待办.
			BP.DA.DBAccess.RunSQL("UPDATE WF_GenerWorkerList SET IsPass=0 WHERE WorkID='" + this.getWorkID() + "' AND IsPass=90");

			//发消息.
			String fk_emp = dt.Rows.get(0).getValue("FK_Emp").toString();
			BP.WF.Dev2Interface.Port_SendMsg(fk_emp, "工作会签完毕", this.getHisGenerWorkFlow().getTitle() + " 工作会签完毕,请到待办查看.", "HuiQian" + this.getWorkID() + "_" + WebUser.getNo(), "HuiQian", getHisGenerWorkFlow().getFK_Flow(), 
					this.getHisGenerWorkFlow().getFK_Node(), this.getWorkID(), 0);
			
			
			   //删除回签人的信息在generworkerlist里面，不然就会显示到他的下一级的退回列表里.
              sql = "DELETE FROM WF_GenerWorkerList WHERE WorkID=" + this.getWorkID() + " AND FK_Node=" + this.getHisNode().getNodeID() + " AND FK_Emp!='" + fk_emp + "'";
            DBAccess.RunSQL(sql);
	...
	*/
	public final boolean TeamupGroupLeader()
	{
		...
		String sql = "SELECT COUNT(No) AS num FROM Port_Dept WHERE No='"+WebUser.getFK_Dept()+"' AND Leader='"+WebUser.getNo()+"'";
	...
	*/
    ...
    private SendReturnObjs NodeSendToYGFlow(Node node, String toEmpIDs)
    {
        ...
        if (StringUtils.isBlank(toEmpIDs))
        {
            ...
            DataTable dt= DBAccess.RunSQLReturnTable(sql);
            if (dt.Rows.size() == 0)
                throw new RuntimeException("@延续子流程目前仅仅支持选择接受人方式.");
	...
	*/
	public final SendReturnObjs NodeSend(Node jumpToNode, String jumpToEmp)
	{
	  ...
	  //      throw new Exception("@当前工作您已经处理完成，或者您(" + this.Execer + " " + this.ExecerName + ")没有处理当前工作的权限。");
		...
		String sendWhen = this.getHisFlow().DoFlowEventEntity(EventListOfNode.SendWhen, this.getHisNode(), this.getHisWork(), null);
		...
		//加入系统变量.
		this.addMsg(SendReturnMsgFlag.VarCurrNodeID, (new Integer(this.getHisNode().getNodeID())).toString(), (new Integer(this.getHisNode().getNodeID())).toString(), SendReturnMsgType.SystemMsg);
		...
		this.addMsg(SendReturnMsgFlag.VarWorkID, (new Long(this.getWorkID())).toString(), (new Long(this.getWorkID())).toString(), SendReturnMsgType.SystemMsg);
		...
		if (this.getHisNode().getIsStartNode() == false && this.getHisGenerWorkFlow().getWFState() == WFState.Askfor)
		{
			...
			for (GenerWorkerList item : gwls.ToJavaList())
			{
				...
				if (item.getIsPassInt() == AskforHelpSta.AfterDealSendByWorker.getValue())
				{
					...
					//加入系统变量。
					this.addMsg(SendReturnMsgFlag.VarToNodeID, (new Integer(this.getHisNode().getNodeID())).toString(), SendReturnMsgType.SystemMsg);
		...
		if (this.getHisNode().getIsHL() || this.getHisNode().getHisRunModel() == RunModel.FHL)
		{
			...
			if (dtWL.Rows.size() != 0)
			{
				...
				if (this.getHisNode().getThreadKillRole() == ThreadKillRole.ByAuto)
				{
					...
					for (DataRow dr : dtWL.Rows)
					{
						BP.WF.Dev2Interface.Flow_DeleteSubThread(this.getHisFlow().getNo(), Long.parseLong(dr.get(0).toString()), "合流点发送时自动删除");
					}
				}
			}
		}
		...
		try
		{
			...
			if (this.getHisNode().getHisTurnToDeal() == TurnToDeal.SpecMsg)
			{
				...
				{
					String url = Glo.getHostURL()+"/WF/MyFlow.htm?FK_Flow=" + this.getHisFlow().getNo() + "&WorkID=" + this.getWorkID() + "&FK_Node=" + town.getHisNode().getNodeID() + "&FID=" + this.rptGe.getFID();
			...
			if (town != null)
			{
				...
				{
					this.addMsg(SendReturnMsgFlag.VarToNodeID, (new Integer(town.getHisNode().getNodeID())).toString(), (new Integer(town.getHisNode().getNodeID())).toString(), SendReturnMsgType.SystemMsg);
			...
			if (this.getHisNode().getIsStartNode())
			{
				...
				else if (SystemConfig.getIsBSsystem() == true)
				{
					...
					if (StringHelper.isNullOrEmpty(pflowNo) == false)
					{
						...
						String paras = "@CFlowNo=" + this.getHisFlow().getNo() + "@CWorkID=" + this.getWorkID();
						Glo.AddToTrack(ActionType.StartChildenFlow, pflowNo, Long.parseLong(pWorkID), Long.parseLong((new Long(fid)).toString()), pND.getNodeID(), pND.getName(), WebUser.getNo(), WebUser.getName(), pND.getNodeID(), pND.getName(), WebUser.getNo(), WebUser.getName(), "<a href='/WF/WFRpt.jsp?FK_Flow=" + this.getHisFlow().getNo() + "&WorkID=" + this.getWorkID() + "' target=_blank >打开子流程</a>", paras);
					}
				}
			}
			//执行时效考核.
...
//                                 
		...
		catch (RuntimeException ex)
		{
			...
			throw new RuntimeException("Message:" + ex.getMessage() + " StackTrace:" + ex.getMessage());
		}
	}
	/** 
	 执行业务表数据同步.
...
//                         
		...
		catch (RuntimeException ex1)
		{
			...
			throw new RuntimeException(ex.getMessage() + "@回滚发送失败数据出现错误：" + ex1.getMessage() + "@有可能系统已经自动修复错误，请您在重新执行一次。");
		}
	}
		///#region 用户到的变量
	...
	*/
	...
	private void InitStartWorkDataV2()
	{
		...
		if (this.title == null)
		{
			if (this.getHisFlow().getTitleRole().equals("@OutPara") || DotNetToJavaStringHelper.isNullOrEmpty(this.getHisFlow().getTitleRole()) == true)
			{
				//如果是外部参数,
				...
				{
					gwf.setTitle(this.getExecer() + "," + this.getExecerName() + "在:" + DataType.getCurrentDataTimeCN() + "发起.");
				}
		...
		if (DotNetToJavaStringHelper.isNullOrEmpty(rptGe.getBillNo()))
		{
			...
			{
				billNo = BP.WF.WorkFlowBuessRole.GenerBillNo(billNo, this.getWorkID(), this.rptGe, this.getHisFlow().getPTable());
	...
	*/
	public final void AddToTrack(ActionType at, String toEmp, String toEmpName, int toNDid, String toNDName, String msg, Node ndFrom, String frmDBJson, String tag)
	{
		...
		if (at == ActionType.SubFlowForward || at == ActionType.StartChildenFlow || at == ActionType.Start || at == ActionType.Forward || at == ActionType.SubFlowForward || at == ActionType.ForwardHL || at == ActionType.FlowOver)
		{
			if (this.getHisNode().getIsFL())
			{
				at = ActionType.ForwardFL;
	...
	*/
	public final void SendMsgToThem(GenerWorkerLists gwls)
	{
		...
		for (GenerWorkerList wl : gwls.ToJavaList())
		{
			...
			// 邮件标题.
			String title = "";
			if (DotNetToJavaStringHelper.isNullOrEmpty(mailTitle))
			{
			...
			mailDoc += "\t\n " + String.format(mailEnd, url);
			mailDoc = mailDoc.replace("{Url}", url);

			// 短信信息.
			if (DotNetToJavaStringHelper.isNullOrEmpty(msgTemp) == true)
			{
			...
			BP.WF.Dev2Interface.Port_SendMsg(wl.getFK_Emp(), title, mailDoc, "WKAlt" + wl.getFK_Node() + "_" + wl.getWorkID(), BP.WF.SMSMsgType.SendSuccess, wl.getFK_Flow(), wl.getFK_Node(), wl.getWorkID(), wl.getFID());
		}*/
	}
	...
	*/
	private void GenerHieLiuHuiZhongDtlData_2013(Node ndOfHeLiu)
	{
		...
		for (MapDtl dtl : mydtls.ToJavaList())
		{
			...
			geDtl.setRefPK((new Long(this.getHisWork().getFID())).toString()); // RefPK 就是当前子线程的FID.
