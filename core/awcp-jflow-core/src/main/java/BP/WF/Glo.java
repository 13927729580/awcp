...
public class Glo
{
	...
	/**
	 ...
	 BasicHttpBinding tempVar = new BasicHttpBinding();
	 tempVar.ReceiveTimeout = ts;
	 tempVar.SendTimeout = ts;
	 tempVar.MaxBufferSize = 2147483647;
	 tempVar.MaxReceivedMessageSize = 2147483647;
	 tempVar.Name = "PortalInterfaceSoap";
	 BasicHttpBinding basicBinding = tempVar;
	 ...
	 var ctor = CCInterface.PortalInterfaceSoapClient.class.GetConstructor(new java.lang.Class[] { Binding.class, EndpointAddress.class });
	 return (CCInterface.PortalInterfaceSoapClient)ctor.invoke(new Object[] { basicBinding, endPoint });
	 }
	 */
...
//                 
	...
	public static String DealExp(String exp, Entity en, String errInfo)
	{
		...
		for (String key : row.keySet())
		{
			exp = exp.toLowerCase();
			//sunxd
			//问题：exp转换后为全小写，key没有进行转换，大小写全都有，所以永远匹配不上，导致焦点字段检查过不去
			//解决：key 转换小写key.toLowerCase()后与 exp 进行匹配   
			if(exp.contains("@" + key.toLowerCase()))
			{
				if (exp.contains("@" + key.toLowerCase() + ";"))
				{
					exp = exp.replace("@" + key.toLowerCase() + ";", row.get(key).toString());
				}else
				{
					exp = exp.replace("@" + key.toLowerCase(), row.get(key).toString());
				}
	...
	{
		//TODO 如果request不为null，直接从request中获取 venson 20180126
		HttpServletRequest request=ContextHolderUtils.getRequest();
		if (null!=request)
		{
...
//		
	...
	{
		InitCH2017(fl, nd, workid, fid, title, null, null, new java.util.Date());
	}
	/**
	 执行考核
	 @param fl 流程
	 ...
	 @param dtNow 当前日期
	 */
	private static void InitCH2017(Flow fl, Node nd, long workid, long fid, String title, String prvRDT, String sdt, java.util.Date dtNow)
	{
		// 开始节点不考核.
		...
		if (nd.getIsEndNode() == true)
		{
			...
			prvRDT = dt.Rows.get(0).getValue(0).toString(); //上一个时间点的记录日期.
			sdt = dt.Rows.get(0).getValue(1).toString(); //应完成日期.
			todoEmps = dt.Rows.get(0).getValue(2).toString(); //参与人员.
		}
		...
		if (nd.getIsEndNode() == false)
		{
			...
			{
				case MSSQL:
					ps.SQL = "SELECT TOP 1 Tag FROM ND" + Integer.parseInt(fl.getNo()) + "Track WHERE WorkID=" + dbstr + "WorkID  AND NDTo=" + dbstr + "NDTo ORDER BY RDT DESC";
					break;
				case Oracle:
					ps.SQL = "SELECT  Tag FROM ND" + Integer.parseInt(fl.getNo()) + "Track  WHERE WorkID=" + dbstr + "WorkID  AND NDTo=" + dbstr + "NDTo AND ROWNUM=1 ORDER BY RDT DESC ";
					break;
				case MySQL:
					ps.SQL = "SELECT  Tag FROM ND" + Integer.parseInt(fl.getNo()) + "Track  WHERE WorkID=" + dbstr + "WorkID AND NDTo=" + dbstr + "NDTo ORDER BY RDT DESC limit 0,1 ";
					break;
				default:
					break;
			}
			ps.Add("WorkID", workid);
			ps.Add("NDTo", nd.getNodeID());

			dt = BP.DA.DBAccess.RunSQLReturnTable(ps);
			if (dt.Rows.size() ==1)
			{
				String str=(String)((dt.Rows.get(0).getValue(0) instanceof String) ? dt.Rows.get(0).getValue(0) : null);
				if (StringHelper.isNullOrEmpty(str)==false)
				{
					todoEmps=str;
				}
			}
			//从轨迹里找到他的相关责任人. // 应该处理的人员,需要从轨迹表里取,上一步骤的处理人.
