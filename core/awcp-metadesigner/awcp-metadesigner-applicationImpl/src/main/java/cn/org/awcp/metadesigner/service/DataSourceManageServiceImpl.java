...
public class DataSourceManageServiceImpl implements DataSourceManageService {
	...
	public String save(DataSourceManageVO vo) {
	...
	public DataSourceManageVO get(String id) {
	...
	public PageList<DataSourceManageVO> selectPagedByExample(BaseExample baseExample, int currentPage, int pageSize,
			String sortString) {
	...
	public DataSourceManageVO queryDataSourceByNameAndSystemId(String name, Long systemId) {
		...
		DataSourceManage dsm = queryChannel.querySingleResult(DataSourceManage.class,
				"queryDataSourceByNameAndSystemId", map);
		if (dsm == null) {
			return null;
		} else {
			DataSourceManageVO dsmv = BeanUtils.getNewInstance(dsm, DataSourceManageVO.class);
