...
public class PunUserGroupServiceImpl implements PunUserGroupService {
	...
	public void remove(PunUserGroupVO vo) {
		try {
			...
			for (int i = 0; i < voList.size(); i++) {
				...
				workflowSyncService.removeRelation(tempVO.getPositionId(), WORKFLOW_POSITION_TYPE, tempVO.getUserId(),
			...
			workflowSyncService.removeRelation(group.getGroupId(), wkGroupType, vo.getUserId(), WORKFLOW_PERSON_TYPE,
					WORKFLOW_EXECUTE_RELATION);
	...
	public void update(PunUserGroupVO vo, String queryStr) {
		try {
			...
			workflowSyncService.createRelation(vo.getPositionId(), WORKFLOW_POSITION_TYPE, vo.getUserGruopId(),
					WORKFLOW_PERSON_TYPE, WORKFLOW_EXECUTE_RELATION, vo.getIsManager());
	...
	public void save(PunUserGroupVO vo) {
		try {
			...
			// 增加人员－岗位关系;
			workflowSyncService.createRelation(vo.getPositionId(), WORKFLOW_POSITION_TYPE, mmc.getUserId(),
					WORKFLOW_PERSON_TYPE, WORKFLOW_EXECUTE_RELATION, mmc.getIsManager());
			...
			for (int i = 0; i < voList.size(); i++) {
				PunUserGroupVO userGroup = (PunUserGroupVO) voList.get(i);
				if (userGroup.getPositionId() != null
						&& userGroup.getPositionId().longValue() != mmc.getPositionId().longValue()) {
					hasOtherPosition = true;
					break;
				}
			}
	...
	// 参数：1.类名 2.mapper文件中对应的id 固定位queryList 3.根据条件来分页查询 4.当前页 5.取的记录他条数 6.
	// 根据字段排序("name.asc")列子
	public PageList<PunUserGroupVO> selectPagedByExample(String queryStr, Map<String, Object> params, int currentPage,
			int pageSize, String sortString) {
		PageList<PunUserGroup> result = queryChannel.queryPagedResult(PunUserGroup.class, queryStr, params, currentPage,
				pageSize, sortString);
		PageList<PunUserGroupVO> resultVO = new PageList<PunUserGroupVO>(result.getPaginator());
	...
	/**
	 * ljw 2014-12-6 修改 修改分页失败问题
	 */
	@Override
	public PageList<PunUserBaseInfoVO> queryUserListByGroupId(Long groupId, int currentPage, int pageSize,
			String sortString) {
		Map<String, Object> map = new HashMap<String, Object>();
		map.put("groupId", groupId);
		PageList<PunUserBaseInfo> result = queryChannel.queryPagedResult(PunUserBaseInfo.class,
				"queryUserListByGroupId", map, currentPage, pageSize, sortString);
	...
	public PageList<PunUserBaseInfoVO> queryUserByUserIdAndGroupId(Long groupId, Long userId) {
		...
		PageList<PunUserGroup> result = queryChannel.queryPagedResult(PunUserGroup.class, "queryUserByUserIdAndGroupId",
				map, 0, 99999, null);
		for (Object dd : result) {
	 ...
	 * 
	 * @see
	 * org.szcloud.framework.unit.service.PunUserGroupService#queryParentManager(
	 * java.lang.Long)
	 */
	@Override
