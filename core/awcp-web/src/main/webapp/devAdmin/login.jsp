			...
			<td align="center" colspan="2">
				<div class="login_box admin">
					...
					<div class="login_form">
						<form action="<%=basePath%>devAdmin/login.do" id="loginForm"
							method="post">
							<ul>
								<li><h1>开发人员账号登录</h1></li>
								<div class="errorTip">${result}</div>
								<li><input type="text" id="orgCode" name="orgCode"
									title="组织机构代码为10位，例如123456-9" class="form-control w300"
									value = "" placeholder="组织机构代码"
									onfocus="javascript:if(this.value=='组织机构代码')this.value='';"
									onBlur="javascript:if(this.value=='')this.value='组织机构代码';"></li>
								<li><input type="text" id="identityNumber" value=""
									title="用户名为6-12位字母或数字" name="identityNumber"
								...
								<li class="clearfix"><input type="text" id="verifyCode"
									name="verifyCode" title="5位校验码" class="form-control w160"
									value="验证码"
									placeholder="验证码"
...
$(function(){
	$("#orgCode").val("78945612-3");
	$("#identityNumber").val("422228197909141766");
	$("#password").val("791013");
	...
	$("#orgCode").formValidator({ onEmpty: "请输入组织机构代码",  onFocus: "组织机构代码为10位，例如12345678-9", onCorrect: ""})
		.regexValidator({ regExp: "orgCode", dataType: "enum", onError: "组织机构代码格式不正确" });
	 
	$("#identityNumber").formValidator({onShow:"请输入身份证号",onFocus:"为15或18位",onCorrect:"符合要求"})
		.regexValidator({regExp:"^(\\d{18,18}|\\d{15,15}|\\d{17,17}x)$",onError:"请输入正确的身份证号码"});

    $("#password").formValidator({ onEmpty: "请输入密码", onFocus: "6~20个字符，包括字母、数字、特殊符号，区分大小写"})
		.inputValidator({ min: 6, max: 20, empty: { leftEmpty: false, rightEmpty: false, emptyError: "密码两边不能有空符号" }, onError: "密码长度错误,请确认" });
  /**
    $("#verifycode").formValidator({ onShowText: "请输入验证码", onFocus: "请输入图片中的字符，不分区大小写", onEmpty: "请输入验证码"})
