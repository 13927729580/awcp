...
function dataToList(data){
	//获取json数据拼接成html片段
	if(data.length==0){
		$(".select-panel .panel-body").html('<div class="tip">当前没有符合条件的用户.</div>');
	}else{
...
//加载下级用户
function searchSqlUser(){
    $(".select-panel .panel-body").html('<div class="tip"></div>');
	var sql = $("#sql").val();
	var userName = $("#userName").val();
	var url = "./common/user/querySqlUser.do?sql="+sql+"&userName="+userName;
	$.ajaxSetup({ 
		async : false
	}); 
...
function initUserSelectPanel(){
	...
	$("#search").click(searchSqlUser);
	
}

function selectOnlyUser($node){
