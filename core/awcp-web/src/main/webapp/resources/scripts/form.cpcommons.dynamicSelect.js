...
function validateKeywords(name){
	var keywords="MYLayoutAndCom,MYpageJScript,MYpageActScript,MYvalidate".split(",");
	var flag = true;
	for(var i=0;i<keywords.length;i++){
		if(keywords[i].indexOf(name)!=-1){
			flag=false;
			break;
		}
	}
...
function loadCommonComponentData(storeObject){
	...
	if(componentObject.pageId != null){
		...
		if(componentObject.mt)	$("#mt").val(componentObject.mt);
		//附件类型
		...
		//动态选择框
		if(componentObject.showDataItemCode) $("#showDataItemCode").val(componentObject.showDataItemCode);
		if(componentObject.zTreeId) $("#zTreeId").val(componentObject.zTreeId);
		if(componentObject.pId) $("#pId").val(componentObject.pId);
		if(componentObject.zTreeName) $("#zTreeName").val(componentObject.zTreeName);
		if(componentObject.showName) $("#showName").val(componentObject.showName);
		if(componentObject.showWhere) $("#showWhere").val(componentObject.showWhere);
		if(componentObject.returnVal) $("#returnVal").val(componentObject.returnVal);
		//包含组件
		...
		for(var i = 0; i < modelJsonArray.length;i++){
			if(modelJsonArray[i].modelItemCodes){
				 var itemCodes = modelJsonArray[i].modelItemCodes.split(",");
					var option = "<li><a href='javascript:;' data='" + modelJsonArray[i].name +"'>" +
						modelJsonArray[i].name +"</a></li>";
					$("#selectInput .sI_select").append(option);
					$("#selectInputShow .sI_select").append(option);
			}
		}
	}
}
...
function initializeDocument(componentType, typeName){
	...
	if(componentId != null){
		...
		if($("#name"))	$("#name").val("data"+Math.ceil(Math.random()*400));
	}
	//alert(dialog != null);
	if(dialog != null){
		//alert(typeName + "组件配置");
	...
	$("body").on("click","#selectInputShow a",function(){
		var value = $(this).attr("data");
		$("#selectInputShow .sI_input").val(value);
	});
