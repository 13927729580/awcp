...
function SetRowCanEdit(row) {  
    for (var j = 0; j < row.cells.length; j++) {  
        ...
        if (!editType) {  
            ...
            editType = row.offsetParent.rows[0].cells[j].getAttribute("edittype");  
        }  
        if (editType) {  
...
function EditCell(element, editType) {  
    ...
    if (!editType) {  
        //如果当前单元格没有指定，则查看当前列是否指定  
        editType = element.offsetParent.rows[0].cells[element.cellIndex].getAttribute("edittype");  
    }  
  
    switch (editType) {  
        case "TextBox":  
            CreateTextBox(element, element.innerText);  
            break;  
        case "DropDownList":  
...
function GetRowData(row) {  
    var rowData = {};  
    for (var j = 0; j < row.cells.length; j++) {  
        name = row.parentNode.rows[0].cells[j].getAttribute("data-field");  
        if (name) {  
