				...
				<div class="form-group ">
					...
					<div class='col-md-5'>
						<select class=" form-control" name="actType" id="actType"
							disabled="disabled">
							<option value="2014" selected="selected">Pdf打印</option>
						</select>
					</div>
					<label class="col-md-1 control-label">Pdf模板</label>
					<div class='col-md-5'>
						<select class=" form-control" tabindex="2" name="extbute[templet]" id="target">
							<c:forEach items="${printTemplets}" var="printTemplet">
								<c:choose>
									<c:when test="${act.extbute['templet'] ==  printTemplet.id}">
										<option value="${printTemplet.id}" selected="selected">${printTemplet.name}</option>
									</c:when>
									<c:otherwise>
										<option value="${printTemplet.id}">${printTemplet.name}</option>
									</c:otherwise>
								</c:choose>
							</c:forEach>
						</select>
