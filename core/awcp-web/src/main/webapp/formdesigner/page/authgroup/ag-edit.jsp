	...
	<body id="main">
			...
			<div class="row" id="dataform">
				<form class="form-horizontal"  id="funcForm"  action="<%=basePath%>func/save.do" method="post" >
						...
						<div class="form-group">       	
				            <label class="col-md-3 control-label required">函数库名称：</label>
				            <div class="col-md-5">
						...
						<div class="form-group">       	
				            <label class="col-md-3 control-label required">脚本：</label>
				            <div class="col-md-5">
		...
		<script type="text/javascript">
         $(document).ready(function(){
			...
			$("#name").formValidator({empty:true,onShow:"请输入函数库名称",onFocus:"至少1个长度"})
				.inputValidator({min:1,max:32,empty:{leftEmpty:false,rightEmpty:false,emptyError:"不能有空符号"},onError:"不能为空,请确认"})
				.regexValidator({regExp:"[_a-zA-Z][_a-zA-Z0-9]*",dataType:"string",onError:"名称只能包含字母、数字、下划线(以字母或下划线开头)"});
			$("#description").formValidator({onShow:"请输入描述信息",onFocus:"至少1个长度"})
				.inputValidator({min:1,empty:{leftEmpty:false,rightEmpty:false,emptyError:"不能有空符号"},onError:"不能为空,请确认"});
		    $("#script").formValidator({onShow:"请输入脚本",onFocus:"至少1个长度"})
		    	.inputValidator({min:1,empty:{leftEmpty:false,rightEmpty:false,emptyError:"不能有空符号"},onError:"不能为空,请确认"}); 
		
		    $("#saveBtn").click(function(){
				if(dialog){
					var data = $("#funcForm").serializeJSON();
					$.ajax({
						type:"POST",
						async:false,
						url:"<%=basePath%>func/saveByAjax.do",
						data:data,
