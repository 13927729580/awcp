...
<title>拥有角色：${role.name }的用户</title>
		...
		<div class="container-fluid">
			...
			<div class="row" id="breadcrumb">
				<ul class="breadcrumb">
					...
					<li><a href="<%=basePath%>dev/punDevUserBaseInfoList.do">角色管理</a></li>
					<li class="active">角色关联用户</li>
				</ul>
			</div>
			...
			<div class="row" id="buttons">
				...
				<a href="<%=basePath%>dev/punDevRoleInfoList.do" class="btn" id="undoBtn">
			...
			<div class="row" id="searchform">
				<div id="collapseButton" class="collapse">
					<form action="<%=basePath%>dev/role/listIn.do" id="createForm">
						<input type="hidden" name="currentPage" value="0" />							
						<input type="hidden" name="boxs" id="boxs" value="${role.id}">			
						<div class="col-md-3">
					...
					<table class="table datatable table-bordered">
						...
						<tbody>
							<c:forEach items="${vos}" var="vo">
								<tr>
									<td class="hidden formData"><input id="boxs" type="hidden" value="${vo.id}"></td>
									<td>${vo.account}</td>
									<td>${vo.name}</td>
			...
			$(function(){
				  ...
				  $("#addBtn").click(function(){
		      			top.dialog({ 
							...
							title: '添加用户到角色:${role.name}',
							url:'<%=basePath%>dev/role/listNotIn.do?roleId=${role.id}',
							width : 800,
							height : 600,
							onremove: function () {
								if (this.returnValue) {
				  ...
				  $("#deleteBtn").click(function(){
					  if(count<1){
						...
						//ajax提交
						$.ajax({
							url : "<%=basePath%>dev/role/groupWithNotUsers.do",
							data :$("#userList").serialize(),// {"userList":$("input[name='boxs']").val(), "roleId":"${role.id}"},
							success : function(o){
								if(o.status == "0"){
									alertMessage(o.message);
									window.location.reload();
								} 
								else{
									alertMessage(o.message);
