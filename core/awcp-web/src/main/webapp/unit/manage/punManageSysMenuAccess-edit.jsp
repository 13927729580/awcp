...
<div class="whole" style="margin:20px;">
	<%-- <ul class="nav nav-tabs">
	     <li>
	       <a href="<%=basePath%>unit/punSystemGet.do?boxs=${sysVO.sysId}">系统信息编辑</a>
	     </li>
	     <li class="active">
	...
	<div class="C_btns">
		<a class="btn btn-info" href="<%=basePath%>/manage/punManageRoleAccessList.do?currentPage=0">保存并返回</a>
	</div>
</div>
<p>${menuJson}</p>
...
function zTreeBeforCheck(treeId, treeNode){
	...
	if(!checked){//添加权限
		checkResult = result("<%=basePath%>manage/punDevRoleAccessAJAXSave.do",{ roleId: roleId, resourceId: treeNode.id, operType:"1"});
		checkResult || alertMessage("抱歉，操作失败");
		return checkResult;
		//checkResult?(return true):(alertMessage("抱歉，操作失败");return false;) 
	}else{//删除权限
		checkResult = result("<%=basePath%>manage/punManageRoleAccessAJAXDelete.do", { roleId: roleId, resourceId: treeNode.id })
		checkResult || alertMessage("抱歉，操作失败");
		return checkResult;
		//checkResult?(return true):(alertMessage("抱歉，操作失败");return false;)	
	}
