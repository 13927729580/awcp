...
public class RefDtlController {
	 ...
	 * 日志对象
	 */
	private static Log logger = LogFactory.getLog(RefDtlController.class);
	private final int PageSize = SystemConfig.getPageSize();

	@RequestMapping(value = "/DtlSave", method = RequestMethod.POST)
	public ModelAndView dtlSave(TempObject object, HttpServletRequest request, HttpServletResponse response) {
	...
	public ModelAndView dtlDel(TempObject object, HttpServletRequest request, HttpServletResponse response) {
		ModelAndView mv = new ModelAndView();
		...
		try {
			...
			for (Entity dtl : Entities.convertEntities(dtls)) {
				Object cb = request.getParameter("CB_" + dtl.getPKVal());// 选种了为on
																			// 不选中为null
				if (null == cb)
					continue;
