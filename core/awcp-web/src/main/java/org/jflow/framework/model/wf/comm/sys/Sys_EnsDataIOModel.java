package org.jflow.framework.model.wf.comm.sys;
...
public class Sys_EnsDataIOModel extends BaseModel {
	...
	public Sys_EnsDataIOModel(HttpServletRequest request, HttpServletResponse response) {
	...
	public void Bind2() {
		...
		String filePath = Glo.getFlowFileTemp() + WebUser.getNo() + "DTS.xls";
		...
		builder.append(" - <a href='EnsDataIO.jsp?EnsName=" + this.getEnsName()
				+ "&Step=1' >返回数据文件上传</a> - <a href='EnsDataIO.jsp?EnsName=" + this.getEnsName()
	...
	public void pageLoad() {
		if (this.getDoType().equals("OutHtml")) {
			...
			sendRedirect(Glo.getCCFlowAppPath() + "Temp" + File.separator + file1);
			return;
			...
			// sendRedirect(Glo.getCCFlowAppPath() + "Temp" + File.separator+
			// file);
			return;
		}
		builder = new StringBuilder();
		if (this.getStep().equals("3")) {
			...
			builder.append(AddH3("<a href='EnsDataIO.jsp?EnsName=" + this.getEnsName()
					+ "&DoType=OutAll' target=_self >导出全部数据到Excel。</a>"));
			builder.append("<font color=green>导出全部数据到Excel。</font>");

			builder.append(AddH3(
					"<a href='EnsDataIO.jsp?EnsName=" + this.getEnsName() + "&Step=1' target=_self  >执行数据导入</a>"));
	...
	public void OutExcel() {
		...
		for (Attr attr : attrs) {
			...
			if (attr.getMyFieldType() == FieldType.Enum || attr.getMyFieldType() == FieldType.PKEnum
					|| attr.getMyFieldType() == FieldType.PKFK || attr.getMyFieldType() == FieldType.FK) {
				continue;
			}
