	...
	private void execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			if (this.getActionType().equals("DoOpenCC")) {
				...
				response.sendRedirect(Glo.getCCFlowAppPath() + "WF/WorkOpt/OneWork/Track.jsp?FK_Flow=" + fk_flow1
						+ "&FK_Node=" + fk_node1 + "&WorkID=" + workid1 + "&FID=" + fid1);
				return;
			}
			...
			{
				try {
					WorkFlow wf14 = new WorkFlow(this.getFK_Flow(), this.getWorkID());
			...
			// ORIGINAL LINE: case "OF":
				...
				String u = "MyFlow.jsp?FK_Flow=" + wl.getFK_Flow() + "&WorkID=" + wl.getWorkID();
				if (this.getIsWap() != 0) {
					u = "./.../WAP/" + u;
				}
				this.wirteMsg(response, "<script> window.location.href='" + u
						+ "'</script> *^_^*  <br><br>正在进入系统请稍后，如果长时间没有反应，请<a href='" + u + "'>点这里进入。</a>");
				return;
			}
			...
			{
				try {
					String str1 = BP.WF.Dev2Interface.Flow_DoUnSend(this.getFK_Flow(), this.getWorkID());
					request.getSession().putValue("info", str1);
					response.sendRedirect("MyFlowInfo" + this.getPageID() + ".jsp?FK_Flow=" + this.getFK_Flow()
							+ "&WorkID=" + this.getWorkID());
					return;
				} catch (RuntimeException ex) {
					request.getSession().putValue("info", "@执行撤消失败。@失败信息" + ex.getMessage());
					this.printAlert(response, ex.getMessage());
					// this.WinCloseWithMsg(ex.Message);
					response.sendRedirect("MyFlowInfo" + this.getPageID() + ".jsp?FK_Flow=" + this.getFK_Flow()
							+ "&WorkID=" + this.getWorkID() + "&FK_Type=warning");
					return;
				}
			...
			// ORIGINAL LINE: case "WorkRpt":
				...
				response.sendRedirect("WFRpt.jsp?WorkID=" + bk1.getWorkID() + "&FID=" + bk1.getFID() + "&FK_Flow="
			...
			// ORIGINAL LINE: case "DeleteFlow":
				...
				throw new RuntimeException("ActionType error" + this.getActionType());
			}
		} catch (RuntimeException ex) {
