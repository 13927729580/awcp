...
public class HungUpController extends BaseController {
	...
	public ModelAndView toHungUp(HttpServletRequest request, HttpServletResponse response) {
	...
	public void doHungUp(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			if (gwf.getWFState() == WFState.HungUp) {
				BP.WF.Dev2Interface.Node_UnHungUpWork(this.getFK_Flow(), getWorkID(), TB_Note);
			} else {
				BP.WF.Dev2Interface.Node_HungUpWork(this.getFK_Flow(), getWorkID(), way.getValue(), TB_RelData,
						TB_Note);
			}
		} catch (Exception ex) {
