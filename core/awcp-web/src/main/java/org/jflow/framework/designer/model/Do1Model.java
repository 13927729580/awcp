...
public class Do1Model extends BaseModel {
	...
	public void init() {
		...
		try {
			if (doType.equals("DownTempFrm")) {
				 ...
				 */
				String file = request.getSession().getServletContext().getRealPath("") + File.separator + "Temp"
						+ File.separator + getFK_MapData() + ".xml";
				logger.debug("11111111111" + file);
				ds.WriteXml(file);
				try {
				...
				mattr.setUIContralType(UIContralType.forValue(Integer.parseInt(request.getParameter("UIContralType"))));
				mattr.setLGType(FieldTypeS.forValue(Integer.parseInt(request.getParameter("LGType"))));
				if (i == 0) {
				...
				if (attrAdd.IsExit(MapAttrAttr.FK_MapData, getMyPK(), MapAttrAttr.KeyOfEn, sem1.getNo())) {
			...
			} else if (doType.equals("AddFG")) // 执行一个插入列组的命令.
			{
				...
				fromAttr.Retrieve();
				if (toGFID == fromAttr.getGroupID() && toID.equals(fromAttr.getMyPK())) {
					// 如果没有移动.
					WinClose();
					return;
				}
			 ...
			 * Request.RawUrl);
			 */
			Pub1.append(AddMsgOfWarning("错误:", ex.getMessage() + " <br>" + request.getRequestURI()));
		}
	}
	...
	public final void AddF() {
		...
		Pub1.append(AddUL());
		Pub1.append(AddLi("<a href='EditF.jsp?DoType=Add&MyPK=" + getMyPK() + "&FType=" + BP.DA.DataType.AppString
				+ "&IDX=" + getIDX() + "&GroupField=" + getGroupField()
				+ "'><b>字符型</b></a> - <font color=Note>如:姓名、地址、邮编、电话</font>"));
		Pub1.append(AddLi("<a href='EditF.jsp?DoType=Add&MyPK=" + getMyPK() + "&FType=" + BP.DA.DataType.AppInt
				+ "&IDX=" + getIDX() + "&GroupField=" + getGroupField()
				+ "'><b>整数型</b></a> - <font color=Note>如:年龄、个数。</font>"));
		Pub1.append(AddLi("<a href='EditF.jsp?DoType=Add&MyPK=" + getMyPK() + "&FType=" + BP.DA.DataType.AppMoney
				+ "&IDX=" + getIDX() + "&GroupField=" + getGroupField()
				+ "'><b>金额型</b></a> - <font color=Note>如:单价、薪水。</font>"));
		Pub1.append(AddLi("<a href='EditF.jsp?DoType=Add&MyPK=" + getMyPK() + "&FType=" + BP.DA.DataType.AppFloat
				+ "&IDX=" + getIDX() + "&GroupField=" + getGroupField()
				+ "'><b>浮点型</b></a> - <font color=Note>如：身高、体重、长度。</font>"));
		Pub1.append(AddLi("<a href='EditF.jsp?DoType=Add&MyPK=" + getMyPK() + "&FType=" + BP.DA.DataType.AppDate
				+ "&IDX=" + getIDX() + "&GroupField=" + getGroupField()
				+ "'><b>日期型</b></a> - <font color=Note>如：出生日期、发生日期。</font>"));
		Pub1.append(AddLi("<a href='EditF.jsp?DoType=Add&MyPK=" + getMyPK() + "&FType=" + BP.DA.DataType.AppDateTime
				+ "&IDX=" + getIDX() + "&GroupField=" + getGroupField()
				+ "'><b>日期时间型</b></a> - <font color=Note>如：发生日期时间</font>"));
		Pub1.append(AddLi("<a href='EditF.jsp?DoType=Add&MyPK=" + getMyPK() + "&FType=" + BP.DA.DataType.AppBoolean
				+ "&IDX=" + getIDX() + "&GroupField=" + getGroupField()
		...
		Pub1.append(AddUL());
		Pub1.append(AddLi("<a href='Do.jsp?DoType=AddSysEnum&MyPK=" + getMyPK() + "&IDX=" + getIDX() + "&GroupField="
				+ getGroupField() + "'><b>枚举型</b></a> -  比如：性别:男/女。请假类型：事假/病假/婚假/产假/其它。"));
		Pub1.append(AddULEnd());
		Pub1.append(AddFieldSetEnd());

		Pub1.append(AddFieldSet("新增外键字段(字典表，通常只有编号名称两个列)"));
		Pub1.append(AddUL());
		Pub1.append(AddLi("<a href='Do.jsp?DoType=AddSFTable&MyPK=" + getMyPK() + "&FType=Class&IDX=" + getIDX()
				+ "&GroupField=" + getGroupField() + "'><b>外键型</b></a> -  比如：岗位、税种、行业、经济性质。"));
		Pub1.append(AddULEnd());
		Pub1.append(AddFieldSetEnd());
		...
		Pub1.append("<div id='SysField' style='display:none' >" + info + "</div>");
		Pub1.append(AddFieldSetEnd());
	}
	...
	public final void AddFEnum() {
		...
		Pub1.append(AddCaptionLeft("<a href='Do.jsp?DoType=AddF&MyPK=" + getMyPK() + "&IDX=" + getIDX()
				+ "'>增加新字段向导</a> - <a href='SysEnum.jsp?DoType=New&MyPK=" + getMyPK() + "&IDX=" + getIDX()
		...
		Pub2.append(BindPageIdx(Pub1, qo.GetCount(), pageSize, getPageIdx(),
				"Do.jsp?DoType=AddSysEnum&MyPK=" + getMyPK() + "&IDX=&GroupField"));
		qo.DoQuery("No", pageSize, getPageIdx());

		boolean is1 = false;
		int idx = 0;
		for (SysEnumMain sem : SysEnumMains.convertSysEnumMains(sems)) {
			DDL ddl = null;
			try {
				ddl = new DDL();
			...
			Pub1.append(AddTD("<a  href=\"javascript:AddEnum('" + getMyPK() + "','" + getIDX() + "','" + sem.getNo()
					+ "')\" >" + sem.getNo() + "</a>"));
			Pub1.append(AddTD(sem.getName()));
			Pub1.append(AddTD("[<a href='SysEnum.jsp?DoType=Edit&MyPK=" + getMyPK() + "&IDX=" + getIDX() + "&RefNo="
					+ sem.getNo() + "' >编辑</a>]"));
	...
	public final void AddFGroup() {
		...
		xmls.RetrieveAll();
		for (FieldGroupXml en : FieldGroupXmls.convertFieldGroupXmls(xmls)) {
			Pub1.append(AddLi("<a href='Do.jsp?DoType=AddFG&RefNo=" + en.getNo() + "&FK_MapData=" + getFK_MapData()
	...
	public final void AddSFTable() {
		...
		Pub1.append(AddTable());
		Pub1.append(AddCaption("<a href='Do.jsp?DoType=AddF&MyPK=" + getMyPK() + "&IDX=" + getIDX()
				+ "'>增加新字段向导</a> - 增加外键字段 - <a href='SFTable.jsp?DoType=New&MyPK=" + getMyPK() + "&IDX=" + getIDX()
				+ "' > 新建表</a>"));
		Pub1.append(AddTR());
		Pub1.append(AddTDTitle("IDX"));
		Pub1.append(AddTDTitle("编号(点击增加到表单)"));
		...
		Pub2.append(BindPageIdx(Pub1, qo.GetCount(), pageSize, getPageIdx(),
				"Do.jsp?DoType=AddSFTable&MyPK=" + getMyPK() + "&IDX=&GroupField"));
		qo.DoQuery("No", pageSize, getPageIdx());

		boolean is1 = false;
		int idx = 0;
		for (SFTable sem : SFTables.convertSFTables(ens)) {
			idx++;
			// is1 = Pub1.AddTR(is1);
			Pub1.append(AddTR(is1));
			Pub1.append(AddTDIdx(idx));
			Pub1.append(AddTD("<a  href=\"javascript:AddSFTable('" + getMyPK() + "','" + getIDX() + "','" + sem.getNo()
					+ "')\" >" + sem.getNo() + "</a>"));
			Pub1.append(AddTD(sem.getName()));

			if (sem.getIsClass()) {
				Pub1.append(AddTD("<a href=\"javascript:WinOpen('../Comm/Search.jsp?EnsName=" + sem.getNo()
						+ "','sg')\"  ><img src='../Img/Btn/Edit.gif' border=0/>" + sem.getTableDesc() + "</a>"));
			} else {
				Pub1.append(AddTD("<a href=\"javascript:WinOpen('SFTable.jsp?DoType=Edit&MyPK=" + getMyPK() + "&IDX="
						+ getIDX() + "&RefNo=" + sem.getNo() + "','sg')\"  ><img src='../Img/Btn/Edit.gif' border=0/>"
						+ sem.getTableDesc() + "</a>"));
			}
			...
			if (sem.getNo().contains(".")) {
				...
				Pub1.append(AddTD(
						"<a href=\"javascript:WinOpen('SFTableEditData.jsp?RefNo=" + sem.getNo() + "');\" >编辑</a>"));
			}
			Pub1.append(AddTREnd());
		}
		Pub1.append(AddTableEnd());
