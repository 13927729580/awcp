...
public class HelperOfDDLModel extends BaseModel {
	 ...
	 * 日志对象
	 */
	private static Log logger = LogFactory.getLog(HelperOfDDLModel.class);
	...
	public HelperOfDDLModel(HttpServletRequest request, HttpServletResponse response) {
	...
	public void loadPage() {
		...
		for (Attr attr : map.getAttrs()) {
			if (attr.getMyFieldType() == FieldType.FK || attr.getMyFieldType() == FieldType.Enum) {
				// this.DropDownList1_SelectedIndexChanged(attr.getDesc(),
				// attr.getKey());
				this.DropDownList1.Items.add(new ListItem(attr.getDesc(), attr.getKey()));
			}
		...
		try {
			 ...
			 */
			if (ex.getMessage().contains("枚举类型") || ex.getMessage().contains("集合已修改")) {
				try {
					Thread.sleep(3000);
		...
		if (this.DropDownList1.getSelectedItemStringVal().equals("None")) {
			...
			if (isGrade) {
				...
				if (num > 1) {
					...
					this.UCSys1.append(AddTDTitle("<img src='../../images/Home.gif' border=0 />数据选择导航"));
					...
					for (Entity myen : Entities.convertEntities(ens)) {
						...
						this.UCSys1.append(AddTD("<a href='#ID" + myen.GetValStringByKey(this.getRefKey())
								+ "' >&nbsp;&nbsp;" + myen.GetValStringByKey(this.getRefKey()) + "&nbsp;&nbsp;"
								+ myen.GetValStringByKey(this.getRefText()) + "</a>"));
						this.UCSys1.append(AddTREnd());
					}
			...
			for (Entity myen : Entities.convertEntities(ens)) {
				...
				if (isGrade) {
					...
					space = ConvertTools.padLeft(String.valueOf(grade), -1, "-");

					space = space.replace("-", "&nbsp;&nbsp;&nbsp;");
					// this.UCSys1.AddTD(space);
					switch (grade) {
					case 2:
						rb.setText("<a href='#top' name='ID" + myen.GetValStringByKey(this.getRefKey())
								+ "' ><Img src='../../images/Top.gif' border=0 /></a><b><font color=green>"
								+ myen.GetValStringByKey(this.getRefKey()) + myen.GetValStringByKey(this.getRefText())
								+ "</font></b>");
						break;
					case 3:
						rb.setText("<b>" + myen.GetValStringByKey(this.getRefKey())
								+ myen.GetValStringByKey(this.getRefText()) + "</b>");
						break;
					default:
						rb.setText(
								myen.GetValStringByKey(this.getRefKey()) + myen.GetValStringByKey(this.getRefText()));
				...
				rb.setName("RB_Group");
				String clientscript = "window.returnValue = '" + myen.GetValStringByKey(this.getRefKey())
						+ "';window.close();";
		...
		if (attr.getKey().equals("FK_Dept")) {
			...
			for (BP.Port.Dept Dept : Depts.convertDepts(depts)) {
				...
				if (Dept.getGrade() == 2 || Dept.getGrade() == 1) {
					...
					this.UCSys1.append("<TR>");
					this.UCSys1.append(AddTDIdx(i));
					this.UCSys1.append("<a href='#ID_2" + Dept.getNo() + "' >&nbsp;&nbsp;" + Dept.getNo()
							+ "&nbsp;&nbsp;" + Dept.getName() + "</a><BR>");
					this.UCSys1.append("</TR>");
				}
			}
			...
			for (BP.Port.Dept Dept : Depts.convertDepts(depts)) {
				...
				if (Dept.getGrade() == 2) {
					...
					// + Dept.getName() +
					// "</b></a><A HREF='#top'><Img src='../../images/Top.gif'
					// border=0 /></a><BR>");
					this.UCSys1.append(AddTD("&nbsp;&nbsp;<a name='ID_2" + Dept.getNo() + "' >" + Dept.getNo()
							+ "</A>&nbsp;&nbsp;<a href='#ID" + Dept.getNo() + "' ><b>" + Dept.getName()
					...
					this.UCSys1.append(AddTD("&nbsp;&nbsp;" + Dept.getNo() + "&nbsp;&nbsp;<a href='#ID" + Dept.getNo()
							+ "' name='#ID_2" + Dept.getNo() + "' >" + Dept.getName() + "</a><BR>"));
				}
			...
			this.UCSys1.append("</TD></TR>");
			// ============ 数据
			for (BP.Port.Dept groupen : Depts.convertDepts(depts)) {
				/*
				 * this.UCSys1.Add("<TR><TD class='Toolbar' >");
				 ...
				 */
				this.UCSys1.append("<TR><TD class='Toolbar' >");
				this.UCSys1.append("<a href='#ID_2" + groupen.getNo() + "' name='ID" + groupen.getNo()
						+ "' ><Img src='../../images/Top.gif' border=0 /></a>&nbsp;&nbsp;"
				...
				i = -1;
				for (Entity myen : Entities.convertEntities(ens)) {
					if (myen.GetValStringByKey(attr.getKey()) != groupen.GetValStringByKey(attr.getUIRefKeyValue())) {
						continue;
					}
			...
			if (size > 19) {
				...
				int i = 0;
				// int span = 2;
				for (Entity groupen : Entities.convertEntities(ens)) {
					i++;
					/*
					 * this.UCSys1.AddTR(); this.UCSys1.AddTDIdx(i);
					 * this.UCSys1.AddTD("<a href='#ID" +
					...
					this.UCSys1.append(AddTR());
					this.UCSys1.append(AddTDIdx(i));
					this.UCSys1.append(AddTD("<a href='#ID" + groupen.GetValStringByKey(attr.getUIRefKeyValue())
							+ "' >&nbsp;&nbsp;" + groupen.GetValStringByKey(attr.getUIRefKeyValue()) + "&nbsp;&nbsp;"
							+ groupen.GetValStringByKey(attr.getUIRefKeyText()) + "</a><BR>"));
					this.UCSys1.append(AddTREnd());
				}
				/*
				 * this.UCSys1.Add("</Table>"); this.UCSys1.Add("</TD></TR>");
			...
			for (Entity groupen : Entities.convertEntities(groupens)) {
				/*
				 * this.UCSys1.Add("<TR><TD class='Toolbar' >");
				...
				this.UCSys1.append("<TR><TD class='Toolbar' >");
				this.UCSys1.append("<a href='#top' name='ID" + groupen.GetValStringByKey(attr.getUIRefKeyValue())
						+ "' ><Img src='../../images/Top.gif' border=0 /></a>&nbsp;&nbsp;"
						+ groupen.GetValStringByKey(attr.getUIRefKeyText()));
				this.UCSys1.append("</TD></TR>");
				this.UCSys1.append("<TR><TD>");
