...
public class PrintDocModel extends BaseModel {
	...
	public PrintDocModel(HttpServletRequest request, HttpServletResponse response) {
	...
	public final void loadData() {
		...
		for (BillTemplate en : BillTemplates.convertBillTemplates(templetes)) {
			appendPub1(AddTR());
			appendPub1(AddTD(en.getNo()));
			appendPub1(AddTD(en.getName()));
			appendPub1(AddTD("<a href='PrintDoc.jsp?WorkID=" + this.getWorkID() + "&FK_Node=" + this.getFK_Node()
					+ "&FK_Bill=" + en.getNo() + "&FK_Flow=" + this.getFK_Flow() + "' >打印</a>"));
			appendPub1(AddTREnd());
		}
	...
	public final void PrintDocV2(BillTemplate func) {
		...
		String file = new Date().getYear() + "_" + WebUser.getFK_Dept() + "_" + func.getNo() + "_" + getWorkID()
				+ ".doc";
		...
		try {
			...
			if (func.getHisBillFileType() == BillFileType.PDF) {
				...
				billInfo += "<img src='" + getBasePath() + "WF/Img/FileType/PDF.gif' /><a href='" + billUrl
						+ "' target=_blank >" + func.getName() + "</a>";
			} else {
				billInfo += "<img src='" + getBasePath() + "WF/Img/FileType/doc.gif' /><a href='" + billUrl
						+ "' target=_blank >" + func.getName() + "</a>";
			}

			path = BP.WF.Glo.getFlowFileBill() + new Date().getYear() + File.separator + WebUser.getFK_Dept()
					+ File.separator + func.getNo() + File.separator;
	...
	public final void PrintDoc(BillTemplate en) {
		...
		wk.Retrieve();
		String msg = "";
		String file = DataType.getCurrentYear() + "_" + WebUser.getFK_Dept() + "_" + en.getNo() + "_" + this.getWorkID()
				+ ".doc";
		BP.Pub.RTFEngine rtf = new BP.Pub.RTFEngine();
		// Works works;
		String[] paths;
		...
		try {
			...
			path = BP.WF.Glo.getFlowFileBill() + DataType.getCurrentYear() + File.separator + WebUser.getFK_Dept()
					+ File.separator + en.getNo() + File.separator;
			File pathFile = new File(path);
			if (!pathFile.exists()) {
				pathFile.mkdirs();
			}
			...
			String url = getBasePath() + "DataUser/Bill/" + DataType.getCurrentYear() + "/" + WebUser.getFK_Dept() + "/"
					+ en.getNo() + "/" + file;
			try {
			...
			path = BP.WF.Glo.getFlowFileBill() + DataType.getCurrentYear() + File.separator + WebUser.getFK_Dept()
					+ File.separator + en.getNo() + File.separator;
			String msgErr = "@生成单据失败，请让管理员检查目录设置 [" + BP.WF.Glo.getFlowFileBill() + "]。@Err：" + ex.getMessage()
					+ " @File=" + file + " @Path:" + path;
			throw new RuntimeException(msgErr + "@其它信息:" + ex.getMessage());
		}
	}
