...
public class Dot2DotModel {
	...
	public AttrOfOneVSM getAttrOfOneVSM() throws Exception {
		...
		for (AttrOfOneVSM attr : en.getEnMap().getAttrsOfOneVSM()) {
			// logger.debug(attr.getEnsOfMM().getClass().getName());
	...
	public final String getShowWay() {
		String showWay = getParameter("IsTreeShowWay");
		if (showWay.equals(""))
	...
	public void SetDataV2() throws Exception {
		...
		if (this.UCSys.getTmpList().size() > 0)
			this.UCSys.getTmpList().clear();
			...
			if ("None".equals(this.getDDL_Group().getSelectedItemStringVal())) {
				...
				if (this.IsLine) {
					this.UIEn1ToMGroupKey_Line(ensOfM, attrOM.getAttrOfMValue(), attrOM.getAttrOfMText(), ensOfMM,
							attrOM.getAttrOfMInMM(), this.getDDL_Group().getSelectedItemStringVal());
				} else {
	...
	public void UIEn1ToMGroupKey_Line(Entities ens, String showVal, String showText, Entities selectedEns,
		...
		this.UCSys.append(
				BaseModel.AddTable("<TABLE class='Table' cellSpacing='1' cellPadding='1'  border='1' width='100%' >"));
