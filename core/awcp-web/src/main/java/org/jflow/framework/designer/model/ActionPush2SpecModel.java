        ...
        if(i==PushWay.SpecSQL.getValue()){
            ...
            if (BP.WF.Glo.getOSModel() == OSModel.WorkFlow)
            {
                ...
                if (dt.Rows.size() != 0)
                {
                    ...
                    for(Station st:sts.convertStations(sts))
                    {
                        st.setStaGrade(0);
                        st.Save();
                    }
...
//                    mycb.Attributes["onclick"] = "SetSelected(this,'" + ctlIDs + "')";
            ...
            else
            {
                ...
                if (dt.Rows.size() != 0)
                {
                    ...
                    for(BP.GPM.Station st:sts.convertStations(sts))
                    {
                ...
                for(BP.GPM.StationType tp:tps.convertStationTypes(tps))
                {
                    this.Pub1.append(AddTR());
                    CheckBox mycb = new CheckBox();
...
//                CheckBox cb = null;
//
//                for(Node nd:nds.convertNodes(nds))
//                {
//                    if (nd.getNodeID() == this.getNodeID())
//                        continue;
//
