...
public class UIEnsModel extends BaseModel {
			...
			if (attr.getIsPK()) {
				...
				if (StringHelper.isNullOrEmpty(str)) {
					if (en.getIsMIDEntity()) {
						...
						return null;
						// throw new Exception("@没有把主键值[" + attr.getKey() +
						// "]传输过来.");
					}
				}
			}
	...
	public void init() {
		...
		get_response().setDateHeader("Expires", 0); // prevents caching at the
													// proxy
		// #endregion 清除缓存
		...
		try {
			...
			if (getEnsName().contains("BP.WF.Template.Ext.Selector")
					|| getEnsName().contains("BP.WF.Template.FlowSorts"))
			...
			// if(EnsName().equals("BP.WF.Template.Selectors") ||
			// EnsName().equals("BP.WF.Template.CCs")){
			// this.UCEn1.Bind(this.CurrEn, this.CurrEn.toString(),
			// this.isReadOnly, true);
			// }else{
			// if (EnsName().contains("BP.WF.Template") == true)
			// this.UCEn1.BindV2(this.CurrEn, this.CurrEn.toString(),
			// this.isReadOnly, true);
			// else
			// this.UCEn1.Bind(this.CurrEn, this.CurrEn.toString(),
			// this.isReadOnly, true);
		...
		// this.Btn_DelFile.Click += new
		// ImageClickEventHandler(Btn_DelFile_Click);
		...
		// this.UCEn1.GetImageButtonByID("Btn_DelFile" + fl.FileNo).Click += new
		// ImageClickEventHandler(Btn_DelFile_X_Click);
		// }
	}
	...
	// string sql = "DELETE FROM " + sf.EnMap.PhysicsTable + " WHERE " +
	// SysFileManagerAttr.EnName + "='" + this.GetEns.GetNewEntity.ToString() +
	// "' AND RefVal='" + this.PKVal + "' AND " + SysFileManagerAttr.AttrFileNo
	// + "='" + id + "'";
	// BP.DA.DBAccess.RunSQL(sql);
	// this.Response.Redirect("UIEn.aspx?EnsName=" + this.EnsName + "&PK=" +
	// this.PKVal, true);
	// }
	...
	// en.RetrieveFromDBSources();
	//
	// string file = en.GetValStringByKey("MyFilePath") + "//" + en.PKVal + "."
	// + en.GetValStringByKey("MyFileExt");
	// try
	// {
	...
	// en.Update();
	// this.Response.Redirect("UIEn.aspx?EnsName=" + this.EnsName + "&EnName=" +
	// this.EnName + "&PK=" + this.PKVal, true);
	// }
	...
	// }
	// this.Response.Redirect("UIEn.aspx?EnsName=" + this.EnsName + "&PK=" +
	// this.PKVal + "&EnName=" + this.EnName + "&tab=" +
	// Uri.EscapeDataString(GetHiddenTabTitle()), true);
	// break;
	...
	private SysFileManager FileSave(String fileNameDesc, TextBox tbf, Entity myen) {
		...
		String filePath = cfg.getFJSavePath(); // BP.Sys.SystemConfig.PathOfFDB
												// + File.separator+ this.EnName
												// + "\\";
