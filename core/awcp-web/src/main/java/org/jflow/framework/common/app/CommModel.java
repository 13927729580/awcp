package org.jflow.framework.common.app;
...
public class CommModel {
	 ...
	 * 日志对象
	 */
	private static Log logger = LogFactory.getLog(CommModel.class);
	 ...
	 */
	public static void InitByMap(Map map, int page, String ensName, UiFatory ui, ToolBar toolBar) throws Exception {
		UserRegedit ur = new UserRegedit(WebUser.getNo(), ensName + "_SearchAttrs");

		InitByMap(map.IsShowSearchKey, map.DTSearchWay, map.getAttrsOfSearch(), map.getSearchAttrs(), null, page, ur,
				ensName, ui, toolBar);

		/*
		 * String cfgKey = ur.getVals();// 当前实体的枚举值与外键条件查询项组合 String[] keys =
		 * cfgKey.split("@");// 组合之间@分隔 java.util.Map<String, Object>
		 * tempContollers = ui.getTmpMap();// 获取所有toolbar组件map for
		 * (java.util.Map.Entry<String, Object> entry : tempContollers
		 * .entrySet()) {// 设置默认值 String tempId = entry.getKey(); if (null ==
		 * tempId) continue; if ("TB_Key".equals(tempId)) { TextBox tb =
		 * (TextBox) tempContollers.get(tempId); tb.setText(""); } else if
		 * ("TB_S_From".equals(tempId)) { TextBox tb = (TextBox)
		 * tempContollers.get(tempId); switch (map.DTSearchWay) {// 日期查询 case
		 * ByDate: tb.setText(ur.getDTFrom_Data()); tb.addAttr("onfocus",
		 * "WdatePicker();"); break; case ByDateTime:
		 * tb.setText(ur.getDTFrom_Datatime()); tb.addAttr("onfocus",
		 * "WdatePicker({dateFmt:'yyyy-MM-dd HH:mm'});"); break; case None:
		 * default: break; } } else if ("TB_S_To".equals(tempId)) { TextBox tb =
		 * (TextBox) tempContollers.get(tempId); switch (map.DTSearchWay) {//
		 * 日期查询 case ByDate: tb.setText(ur.getDTTo_Data());
		 * tb.addAttr("onfocus", "WdatePicker();"); break; case ByDateTime:
		 * tb.setText(ur.getDTTo_Datatime()); tb.addAttr("onfocus",
		 * "WdatePicker({dateFmt:'yyyy-MM-dd HH:mm'});"); break; case None:
		 * default: break; } } else { if (tempId.indexOf("DDL_") >= 0 &&
		 * cfgKey.indexOf(tempId) >= 0) { for (String key : keys) { if
		 * (key.length() > 3 && key.indexOf(tempId) >= 0) { String[] vals =
		 * key.split("="); DDL ddl = (DDL) tempContollers.get(tempId); boolean
		 * isHave = ddl.SetSelectItem(vals[1]); if (!isHave) { 没有有找到要选择的人员 try {
		 * Attr attr = map.GetAttrByKey(vals[0] .replace("DDL_", ""));
		 * ddl.SetSelectItem(vals[1], attr); } catch (Exception e) {
		 * logger.info("ERROR", e); } } } }
		 * 
		 * } } }
		 */
		...
		for (AttrSearch attr : temp.subList(0, temp.size())) {
			if (!StringHelper.isNullOrEmpty(attr.reationGrandChildKey)) {// 三层级联
				DDL bigDDL = toolBar.GetDDLByKey("DDL_" + attr.Key) == null ? null
						: toolBar.GetDDLByKey("DDL_" + attr.Key);
				DDL smallDDL = toolBar.GetDDLByKey("DDL_" + attr.reationChildKey) == null ? null
						: toolBar.GetDDLByKey("DDL_" + attr.reationChildKey);
				DDL litSmallDDL = toolBar.GetDDLByKey("DDL_" + attr.reationGrandChildKey) == null ? null
						: toolBar.GetDDLByKey("DDL_" + attr.reationGrandChildKey);
				three += "\t\n onThreeCasca(\"" + bigDDL.getId() + "\",\"" + smallDDL.getId() + "\",\""
						+ litSmallDDL.getId() + "\",\"" + ensName + "\",\"" + attr.reationChildKey + "\",\""
						+ attr.reationGrandChildKey + "\");";
				...
				if (StringHelper.isNullOrEmpty(attr.reationParentKey)) {
					...
					DDL smallDDL = toolBar.GetDDLByKey("DDL_" + attr.reationChildKey) == null ? null
							: toolBar.GetDDLByKey("DDL_" + attr.reationChildKey);
					two += "\t\n onCasca(\"" + bigDDL.getId() + "\",\"" + smallDDL.getId() + "\",\"" + ensName + "\",\""
							+ attr.reationChildKey + "\");";
				}
			}
		}
	 ...
	 */
	public static void InitByMap(boolean isShowKey, DTSearchWay searchWay, AttrsOfSearch attrsOfSearch,
			AttrSearchs attrsOfFK, Attrs attrD1, int page, UserRegedit ur, String ensName, UiFatory ui, ToolBar toolBar)
			throws Exception {
		...
		case ByDateTime:
			...
			tbDT2_1.addAttr("onfocus", "WdatePicker({dateFmt:'yyyy-MM-dd HH:mm'});");
			ui.append(tbDT2_1);
			...
			tbDT2_2.addAttr("onfocus", "WdatePicker({dateFmt:'yyyy-MM-dd HH:mm'});");
			ui.append(tbDT2_2);
			break;
		case None:
		default:
			...
			if (attr.getMyFieldType() == FieldType.Enum) {
				...
				if (!StringHelper.isNullOrEmpty(attr1.reationGrandChildKey)) {// 三层级联
					...
					ddl.addAttr("onchange",
							"onThreeCasca('DDL_" + attr.getKey() + "','DDL_" + attr1.reationChildKey + "','DDL_"
									+ attr1.reationGrandChildKey + "','" + ensName + "','" + attr1.reationChildKey
									+ "','" + attr1.reationGrandChildKey + "')");
				} else if (!StringHelper.isNullOrEmpty(attr1.reationChildKey)) {// 二层级联
					if (!StringHelper.isNullOrEmpty(attr1.reationParentKey)) {
						ddl.addAttr("onchange",
								"onTwoCasca('DDL_" + attr1.reationParentKey + "','DDL_" + attr.getKey() + "','DDL_"
										+ attr1.reationChildKey + "','" + ensName + "','" + attr.getKey() + "','"
										+ attr1.reationChildKey + "')");
					} else {
						ddl.BindTable(attr.getUIBindKey(), attr.getUIRefKeyValue(), attr.getUIRefKeyText(), false,
								AddAllLocation.TopAndEnd);
						ddl.Items.get(0).setText(">>" + desc);
				...
				if ("BP.Port.Depts".equals(attr.getUIBindKey())) {// 如果是部门
					...
					if (depts.size() > SystemConfig.getMaxDDLNum())
						toolBar.AddLab("lD", "<a href=\"javascript:onDDLSelectedMore('DDL_" + key + "', '" + ensName
	 ...
	 * 构造查询条件
	 */
	public static QueryObject GetnQueryObject(Entities ens, Entity en, HttpServletRequest request) {
		Map map = en.getEnMap();
		QueryObject qo = InitQueryObjectByEns(ens, map.IsShowSearchKey, map.DTSearchWay, map.DTSearchKey,
				map.getAttrs(), map.getAttrsOfSearch(), map.getSearchAttrs(), request);
	...
	public static QueryObject InitQueryObjectByEns(Entities ens, boolean IsShowSearchKey, DTSearchWay searchWay,
			String dtKey, Attrs attrs, AttrsOfSearch attrsOfSearch, AttrSearchs searchAttrs,
		...
		for (AttrOfSearch attr : attrsOfSearch) {
			...
			if (!"".equals(str)) {
				qo.addAnd();
				qo.addLeftBracket();
				qo.AddWhere(attr.getRefAttrKey(), opkey, request.getParameter("TB_" + attr.getKey()));
				qo.addRightBracket();
			}
		...
		switch (searchWay) {// 日期查询
		case ByDate:
			String dtFrom1 = StringHelper.isEmpty(request.getParameter("TB_S_From"), "");
			String dtTo1 = StringHelper.isEmpty(request.getParameter("TB_S_To"), "");
			if (!"".equals(dtFrom1)) {
				qo.addAnd();
				qo.addLeftBracket();
			...
			break;
		case ByDateTime:
			String dtFrom2 = StringHelper.isEmpty(request.getParameter("TB_S_From"), "");
			String dtTo2 = StringHelper.isEmpty(request.getParameter("TB_S_To"), "");
			if (!"".equals(dtFrom2)) {
				qo.addAnd();
				qo.addLeftBracket();
	...
	public static void DataPanelDtl(StringBuilder sb, Entities ens, int pageIdx) {
		...
		try {
			...
			// isShowOpenICON = ens.GetEnsAppCfgByKeyBoolen("IsEnableOpenICON");
			// // 是否启用 OpenICON
		...
		for (Entity en : Entities.convertEntities(ens)) {
			...
			urlExt = "\"javascript:ShowEn('" + basePath + "WF/App/Comm/Creat.jsp?EnsName=" + ens.toString() + "&PK="
					+ en.GetValByKey(myen.getPK()) + url + "', 'cd','" + WinCardH + "','" + WinCardW + "');\"";
			...
			if (IsEnableRefFunc) {
				...
				for (RefMethod func : myreffuncs) {
					if (func.Visable == false || func.IsForEns == false)
						continue;
					str += "<a style='cursor:hand;' nowrap='true' onclick=\"javascript:RefMethod1('" + basePath + "', '"
							+ func.Index + "', '" + func.Warning + "', '" + func.Target + "', '" + ens.toString()
							+ "','" + url + "') \"  > " + func.GetIcon(basePath) + "" + func.Title + "</a>";
				}
				// #endregion
				...
				for (EnDtl enDtl : enDtls) {
					str += "[<a onclick=\"javascript:EditDtl1('" + basePath + "', '" + myen.ToStringAtParas() + "',  '"
							+ enDtl.getEnsName() + "', '" + enDtl.getRefKey() + "', '" + url + "&IsShowSum=1')\" >"
							+ enDtl.getDesc() + "</a>]";
				}
				// #endregion
				...
				// href="
				// + urlExt +
				// " ><img src='"+getBasePath()+"WF/Img/Btn/open.gif'
				// border=0/></a></TD>");
			}
	...
	public static void DataPanelDtlAdd(StringBuilder sb, Entity en, Attr attr, Searchs cfgs, String url, String cardUrl,
			String focusField) {
		 ...
		 * sb.append(BaseModel.AddTDDoc("...", "...")); return; }
		 */
		...
		case DataType.AppDouble:
			...
			if (cfgurl == "") {
				...
				sb.append(BaseModel
						.AddTDNum("<a href=\"javascript:WinOpen('" + cfgurl + "','dtl1');\" >" + str + "</a>"));
			}
			break;
		case DataType.AppMoney:
			cfgurl = "";
			...
			if (cfgurl == "") {
				...
				cfgurl = cfgurl.replace("@Keys", url);
				sb.append(BaseModel.AddTDNum("<a href=\"javascript:WinOpen('" + cfgurl + "','dtl1');\" >"
	...
	public static void BindPageIdxEasyUi(StringBuilder sb, int totalRecords, int pageIdx, int pageSize, String layout,
			boolean showParentPanel) {
		sb.append("    <style type='text/css'>" + "        #eupage table,#eupage td" + "        {"
