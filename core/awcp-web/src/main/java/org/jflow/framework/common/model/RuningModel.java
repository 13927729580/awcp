...
//            this.Pub1.append(BaseModel.AddCaption("在途工作"));
        ...
        if (!"FlowName".equals(GroupBy))
            this.Pub1.append(BaseModel.AddTDTitle("<a href='"+basePath+"WF/"+ PageID +".jsp?GroupBy=FlowName&FK_Flow=" + FK_Flow + "' >流程</a>"));

        if (!"NodeName".equals(GroupBy))
            this.Pub1.append(BaseModel.AddTDTitle("<a href='"+basePath+"WF/"+ PageID +".jsp?GroupBy=NodeName&FK_Flow=" + FK_Flow + "' >当前节点</a>"));

        if (!GenerWorkFlowAttr.StarterName.equals(GroupBy))
            this.Pub1.append(BaseModel.AddTDTitle("<a href='"+basePath+"WF/"+ PageID +".jsp?GroupBy=StarterName&FK_Flow=" + FK_Flow + "' >发起人</a>"));
