...
public class CondController {
	 ...
	 * 日志对象
	 */
	private static Log logger = LogFactory.getLog(CondController.class);

	@RequestMapping(value = "/cond_ddl_SelectedIndexChanged", method = RequestMethod.GET)
	public ModelAndView ddl_SelectedIndexChanged(HttpServletRequest request, HttpServletResponse response)
			throws IOException {
	...
	public ModelAndView btn_Save_Click(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		if (NamesOfBtn.getEnumByCode(btnId) == NamesOfBtn.Delete) {
			DBAccess.RunSQL("DELETE FROM WF_Cond WHERE  ToNodeID=" + ToNodeID + " AND DataFrom="
					+ ConnDataFrom.Form.getValue());
		...
		DBAccess.RunSQL("DELETE FROM WF_Cond WHERE (" + CondAttr.NodeID + "=" + FK_Node + "  AND ToNodeID=" + ToNodeID
				+ ") AND DataFrom!=" + ConnDataFrom.Form.getValue());
		...
		String sql = "UPDATE WF_Cond SET DataFrom=" + ConnDataFrom.Form.getValue() + " WHERE NodeID=" + cond.getNodeID()
				+ "  AND FK_Node=" + cond.getFK_Node() + " AND ToNodeID=" + ToNodeID;
		...
		case Node:
			...
			mv.setViewName("redirect:" + "/WF/Admin/Cond.jsp");
			return mv;
		// response.sendRedirect("Cond.jsp?MyPK=" + cond.getMyPK() + "&FK_Flow="
		// + cond.getFK_Flow() + "&FK_Node=" + cond.getFK_Node() +
		// "&FK_MainNode=" + cond.getNodeID() + "&CondType=" +
		// cond.getHisCondType().getValue() + "&FK_Attr=" + cond.getFK_Attr() +
		// "&ToNodeID=" + ToNodeID);
		case Dir:
