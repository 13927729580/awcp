...
public class WorkflowTaskControl extends BaseController {
    ...
    private Map executeAct(final HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            if (store != null) {
                ...
                if (act == null)
                    throw new PlatformException("动作脚本未找到");
            } else
                throw new PlatformException("动作ID未找到");
            // 初始化脚本解释执行器,加载全局工具类
     ...
     * 增加消息推送
     */
    private void addPush(DocumentVO vo) {
        ...
        if (!emps.isEmpty()) {
            if(preset.equals(vo.getFlowTempleteId())){
                pushByDD(vo, emps);
            }else{
                String gotoUrl;
                ...
                for (Map<String, Object> map : emps) {
                    ...
                    gotoUrl = baseUrl + "wf/openTask.do?FK_Flow=" + vo.getFlowTempleteId() + "&WorkID="
    ...
    public void pushByDD(DocumentVO vo, List<Map<String, Object>> emps) {
        ...
        Map<String, Object> data = jdbcTemplate.queryForMap("select * from " + vo.getTableName() + " where id=? ",
                vo.getRecordId());
        // 去app表中查找需要显示的字段
        String FIELDS = this.jdbcTemplate.queryForObject(
                "SELECT FIELDS FROM dd_apps WHERE dynamicPageId=? or pcDynamicPageId=?",String.class, dynamicPageId, dynamicPageId);
    ...
    public Map<String, Object> shiftWork(Long userId) {
        ...
        PunUserBaseInfoVO user = (PunUserBaseInfoVO) SessionUtils.getObjectFromSession(SessionContants.CURRENT_USER);
        Map<String, Object> map = query.getUntreatedData(10000, 0, null, null,null, user.getUserIdCardNumber(), true);
