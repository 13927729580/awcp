...
import cn.org.awcp.formdesigner.utils.MyFontFactory;
import cn.org.awcp.formdesigner.utils.MyImageFactory;
import cn.org.awcp.formdesigner.utils.PdfHeaderFooter;
...
public class PrintServiceImpl implements PrintService, ServletContextAware {
	...
	public void printPDF(StoreVO printManageVO, Map dataMap, OutputStream os) {
		...
		if (objectList != null && objectList.size() > 0) {
			for (int i = 0; i < objectList.size(); i++) {
				pageList.add(objectList.getJSONObject(i).getString("id"));
			}
		}
		...
		try {
			...
			for (int i = 0; i < pageList.size(); i++) {
				...
				PdfPTable mainTable = geneaatePTableByPageId(pageList.get(i), dataMap, null,
	...
	public void printPDFByTemplate(StoreVO printManageVO, Map dataMap, OutputStream os, List<JSONObject> components) {
		...
		while (pageOfCurrentReaderPDF < reader.getNumberOfPages()) {
			...
			if (StringUtils.isNotBlank(oo.getString("addPageId"))) { // 如果在模板里面要合并上页面
				String addPageId = oo.getString("addPageId");
	...
	private PdfPTable geneaatePTableByPageId(String id, Map dataMap, Map configures, float totalWidth)
		...
		if (pageVO.getPageType() == 1002) { // 表单页面
			...
			if (StringUtils.isNoneBlank(pageVO.getPdfTemplatePage())) { // 如果列表页面配置了pdf的显示模板
				...
				String pdfTemplatePage = pageVO.getPdfTemplatePage();
	...
	private PdfPTable generateDataGridTable(JSONObject component, Map dataMap, float totalWidth)
		...
		for (int i = 0; i < columns.size(); i++) {
			...
			Paragraph finalContent = new Paragraph();
			finalContent.add(chunkFont.process(o.getString("columnTitle")));
			PdfPCell cell = new PdfPCell(finalContent);
		...
		if (dataList != null && dataList.size() > 0) {
			...
			for (int i = 0; i < maxLine; i++) {
				...
				for (int k = 0; k < columns.size(); k++) {
					...
					finalContent.add(chunkFont.process(value));
					PdfPCell cell = new PdfPCell(finalContent);
				...
				if (Integer.parseInt(minLineCount) > dataList.size()) { // 如果最小条数大于数据条数，则补上空白
					...
					for (int i = 0; i < addLine; i++) {
						for (int j = 0; j < columns.size(); j++) {
							PdfPCell cell = new PdfPCell(chunkFont.process(" "));
	...
	private PdfPTable createListTableWithTemplate(String pdfTemplatePage, List<Map<String, String>> dataList,
		...
		if (compList != null && compList.size() > 0) {
			...
			for (int i = 0; i < compList.size(); i++) {
				...
				if ("1012".equals(comType)) { // 包含组件
					String containPageId = component.getString("relatePageId");
