...
public class DdDocController {
	...
	private void sendMsg(String dataSource, DocumentUtils docUtils,String id,String dynamicPageId,
		...
		String sql = "select fields,agentId from dd_apps where dynamicPageId=? or pcDynamicPageId=? limit 1";
		Map<String,Object> ddApp = jdbcTemplate.queryForMap(sql,dynamicPageId,dynamicPageId); 
		String fields = ddApp.get("fields") + "";	//消息推送需要的字段
		String agentId = ddApp.get("agentId") + "";
		...
		for(int i=0;i<fieldArr.length;i++){
			...
			if(ControllerHelper.getLang()==Locale.SIMPLIFIED_CHINESE){
				obj.put(tempArr[1], data.get(dataSource + "." + tempArr[0]));
			}
			else{
				if(tempArr.length==2){
					obj.put(tempArr[0], data.get(dataSource + "." + tempArr[0]));
				}
				else if((tempArr.length==3)){
					obj.put(tempArr[2], data.get(dataSource + "." + tempArr[0]));
				}
			}
		}
		...
		if(StringUtils.isNotBlank(ddUserIds)){
			docUtils.sendMessage(url,"0",obj,title,ddUserIds.replaceAll(",","|"),agentId,null,null);
		}
		if(StringUtils.isNotBlank(groupIds)){	
			docUtils.sendMessage(url,"1",obj,title,groupIds.replaceAll(",","|"),agentId,null,null);
		}
	}
