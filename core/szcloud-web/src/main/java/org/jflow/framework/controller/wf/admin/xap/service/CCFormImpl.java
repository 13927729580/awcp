...
@WebService(endpointInterface = "org.jflow.framework.controller.wf.admin.xap.service.CCForm")
public class CCFormImpl implements CCForm {
	...
	public String UploadFile(byte[] FileByte, String fileName) throws IOException {
		// 创建临时目录.
		String pathTemp = BP.Sys.SystemConfig.getPathOfTemp() + File.separator + "Temp";
	...
	public String SaveImageAsFile(byte[] img, String pkval, String fk_Frm_Ele) {
		...
		// fe.FK_MapData + "\\");
		//
		// string saveTo = fe.HandSigantureSavePath + File.separator+
		// fe.FK_MapData +
		// File.separator+ pkval + ".jpg";
	...
	public String LoadFrmTemplete(byte[] fileByte, String fk_mapData, boolean isClear) {
		try {
			...
			String file = File.separator + "Temp" + File.separator + fk_mapData + ".xml";
	...
	// FtpSupport.FtpConnection("192.168.1.138", "administrator", "jiaozi");
	// switch (doType)
	// {
	// case "ShareFrm": /*共享模板*/
	// MapData md = new MapData();
	// DataSet ds = md.GenerHisDataSet();
	// string file = BP.Sys.SystemConfig.PathOfTemp + v1 + "_" + v2 + "_" +
	// DateTime.Now.ToString("MM-dd hh-mm") + ".xml";
	// ds.WriteXml(file);
	...
	//
	// /*下载文件到指定的目录: */
	// string tempFile = BP.Sys.SystemConfig.PathOfTemp + File.separator+ v2 +
	/// ".xml";
