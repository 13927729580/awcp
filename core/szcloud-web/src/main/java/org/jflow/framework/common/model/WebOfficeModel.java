...
public class WebOfficeModel {
	 ...
	 * 日志对象
	 */
	private static Log logger = LogFactory.getLog(WebOfficeModel.class);
	...
	public WebOfficeModel(String basePath, int FK_Node, int FID, long WorkID, String FK_Flow, boolean IsTrueTH,
			String IsTrueTHTemplate, String UserName, String HeBing, boolean ReadOnly, boolean IsCheckInfo,
			String NodeInfo, boolean IsSavePDF, boolean IsMarks, String OfficeTemplate, boolean IsLoadTempLate,
	...
	private void LoadMenu(boolean isMenu) {
		...
		if (!isCompleate) {
			try {
				isCompleate = !Dev2Interface.Flow_IsCanDoCurrentWork(node.getFK_Flow(), this.FK_Node, this.WorkID,
						WebUser.getNo());
		...
		if (isMenu && !isCompleate) {
			...
			if (btnLab.getOfficeNodeInfo())
				IsCheckInfo = true;
			//
			// if (btnLab.getOfficeIsTrueTH()){
			// this.IsTrueTH = true;
			// try{
			// String thTemplate = btnLab.getOfficeTHTemplate();
			// if (!StringHelper.isNullOrEmpty(thTemplate)){
			// File fileDir = new
			// File(this._request.getSession().getServletContext().getRealPath("/"+thTemplate));
			// if (fileDir.exists()) {
			// DataSet ds = new DataSet();
			// ds.readXml(this._request.getSession().getServletContext().getRealPath("/"+thTemplate));
			//
			// if (ds.Tables.size() > 0){
			// for (DataRow row : ds.Tables.get(0).Rows){
			// boolean isTrue = false;
			// String attrKey = row.getValue("AttrKey") + "";
			// String attrValue = row.getValue("AttrValue") + "";
			// String fileKey = row.getValue("FileKey") + "";
			// String fileValue = row.getValue("FileValue") + "";
			//
			// if (!StringHelper.isNullOrEmpty(attrKey)){
			// String sql = String.format("select %1$s from %2$s where
			// OID='%3$s'", attrKey, node.getPTable(), this.WorkID);
			// String realAttrValue =
			// BP.DA.DBAccess.RunSQLReturnStringIsNull(sql,"");
			// isTrue = true;
			// if (!attrValue.equals(realAttrValue))
			// continue;
			// }
			//
			// if (!StringHelper.isNullOrEmpty(fileKey)){
			// isTrue = true;
			// String sql = String.format("select %1$s from %2$s where
			// OID='%3$s'", fileKey, node.getPTable(), this.WorkID);
			// String realFileValue =
			// BP.DA.DBAccess.RunSQLReturnStringIsNull(sql, "");
			// if (!fileValue.equals(realFileValue))
			// continue;
			//
			// }
			//
			// if (isTrue){
			// File fileBir = new
			// File(this._request.getSession().getServletContext().getRealPath("/DataUser/OfficeOverTemplate/"+row.getValue("FileName")));
			// if (fileBir.exists()) {
			// this.IsTrueTHTemplate = fileBir.getName();
			// this.setSealName(fileBir.getName());
			// this.setSealType(FileAccess.getExtensionName(fileBir.getName()));
			// }
			// break;
			// }
			//
			...
			// }
			// }
			// OfficeTemplate = btnLab.getOfficeTemplate();

			// if (!isCompleate){
			//
			// if (btnLab.getOfficeReadOnly()){
			// this.ReadOnly = true;
			// }
			// if (btnLab.getOfficeNodeInfo()){
			// IsCheckInfo = true;
			// }
			// if (btnLab.getOfficeReSavePDF()){
			// IsSavePDF = true;
			// }
			//
			// this.IsMarks = btnLab.getOfficeIsMarks();
			// }else{
			// this.ReadOnly = true;
		}
	...
	private void ReadFile() {
		...
		if (!isHave) {
			...
			// {
			// fileInfo.CopyTo(path + File.separator+ this.WorkID +
			// fileInfo.Extension);
			...
			// if (node.HisNodeWorkType == NodeWorkType.WorkHL ||
			// node.HisNodeWorkType == NodeWorkType.WorkFHL)
			// {

			// GenerWorkFlows generWorksFlows = new GenerWorkFlows();
			// generWorksFlows.RetrieveByAttr(GenerWorkFlowAttr.FID,
			// this.WorkID);
			// string tempH = "";
	...
	private void LoadFile() {
		...
		try {
			String loadType = this._request.getParameter("LoadType") == null ? ""
					: this._request.getParameter("LoadType");
			...
			if ("1".equals(loadType)) {
				path = this._request.getSession().getServletContext()
						.getRealPath("/DataUser/OfficeFile/" + this.FK_Flow + "/" + name);
			} else {
			...
			this._response.getOutputStream().write(result.getBytes());
	...
	private void LoadAttachment() {
		...
		for (FrmAttachment ath : FrmAttachments.convertFrmAttachments(attachments)) {
			String src = this.basePath + "WF/CCForm/AttachmentUpload.jsp?PKVal=" + this.WorkID + "&Ath="
					+ ath.getNoOfObj() + "&FK_MapData=" + EnName + "&FK_FrmAttachment=" + ath.getMyPK() + "&FK_Node="
					+ this.FK_Node;
	...
	private void SaveFile() {
		...
		try {
			...
			MultipartHttpServletRequest multipartRequest = (MultipartHttpServletRequest) this._request;
			...
			for (Map.Entry<String, MultipartFile> entity : fileMap.entrySet()) {
				...
				if (IsSavePDF) {
					...
					try {
						...
						strBuf.append("Content-Disposition: form-data; name=\"" + fileExtension + "\"; filename=\""
								+ fileStart + "\"\r\n");
