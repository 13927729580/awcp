...
public class AccepterController {
	...
	private BP.WF.Template.Node _HisNode = null;

	/**
	 * 它的节点
	 * 
	 */
	public final BP.WF.Template.Node getHisNode() {
		if (_HisNode == null) {
			_HisNode = new BP.WF.Template.Node(this.getFK_Node());
		}
		return _HisNode;
	}
	...
	public final boolean getIsMFZ() {
		...
		int num = 0;
		for (BP.WF.Template.Node mynd : Nodes.convertNodes(nds)) {
			Cond cond = new Cond();
	...
	protected final void execute(HttpServletRequest request, HttpServletResponse response) {
		...
		DoWhat = _request.getSession().getAttribute("DoWhat") == null ? ""
				: _request.getSession().getAttribute("DoWhat").toString(); // 读取
		...
		if (this.getToNode() == 0) {
			...
			String sql = "SELECT COUNT(*) FROM WF_Node WHERE FK_Flow='" + this.getHisNode().getFK_Flow() + "' AND "
					+ NodeAttr.DeliveryWay + "=" + DeliveryWay.BySelected.ordinal() + " AND " + NodeAttr.DeliveryParas
			...
			if (nds.size() == 0) {
				try {
					_response.getOutputStream().write(
							"<script type=\"text/javascript\">alert('当前点是最后的一个节点,不能使用此功能!')</script> ".getBytes());
				} catch (IOException e) {
					// TODO Auto-generated catch block
					logger.info("ERROR", e);
				}
				...
				BP.WF.Template.Node toND = (BP.WF.Template.Node) ((nds.get(0) instanceof BP.WF.Template.Node)
						? nds.get(0) : null);
			...
			if (tempToNodeID == 0) {
				try {
					_response.getOutputStream()
							.write("<script type=\"text/javascript\">alert('@流程设计错误：\n\n 当前节点的所有分支节点没有一个接受人员规则为按照选择接受!')</script> "
									.getBytes());
				} catch (IOException e) {
					// TODO Auto-generated catch block
					logger.info("ERROR", e);
			...
			if (dirSend.equals("yes")) {
				try {
					_response.sendRedirect("Accepter.jsp?FK_Flow=" + this.getFK_Flow() + "&FK_Node=" + this.getFK_Node()
							+ "&ToNode=" + tempToNodeID + "&FID=" + this.getFID() + "&type=1&WorkID=" + this.getWorkID()
							+ "&IsWinOpen=" + this.getIsWinOpen() + "&dirSend=" + dirSend + "&DoWhat=" + DoWhat);
				} catch (IOException e) {
					// TODO Auto-generated catch block
					logger.info("ERROR", e);
				}
				...
				try {
					_response.sendRedirect("Accepter.jsp?FK_Flow=" + this.getFK_Flow() + "&FK_Node=" + this.getFK_Node()
							+ "&ToNode=" + tempToNodeID + "&FID=" + this.getFID() + "&type=1&WorkID=" + this.getWorkID()
							+ "&IsWinOpen=" + this.getIsWinOpen() + "&dirSend=no" + "&DoWhat=" + DoWhat);
				} catch (IOException e) {
					// TODO Auto-generated catch block
					logger.info("ERROR", e);
				}
	...
	private void returnValue() {
		...
		if (StringHelper.isNullOrEmpty(WebUser.getNo())
				|| StringHelper.isNullOrEmpty(_request.getParameter("method"))) {
			addEmps();
			return;
		}
	...
	/**
	 * 这是一个清空全部数据的操作 由于此前可能更改配置规则--因此删除时不必再判断，直接清空相关数据
	 * 
	 * @return
	 */
	public final String clearDataMet() {
		String delHadEmps = String.format("delete from WF_SelectAccper where FK_Node=%1$s and WorkID=%2$s)",
				this.getToNode(), this.getWorkID());
	...
	public final void saveMet() {
		...
		if (_request.getParameter("IsEUI") == null) {
			try {
				_response.getOutputStream()
						.write("<script type=\"text/javascript\">window.close();</script> ".getBytes());
			} catch (IOException e) {
				// TODO Auto-generated catch block
				logger.info("ERROR", e);
			}
	...
	public final String addEmps() {
		...
		for (int i = 0; i < dt.Rows.size(); i++) {
			addLi += String.format(
					"<li style='text-align:center;vertical-align:middle;width:100px;' id='%1$s' class='liNode' onmouseout='javascript:removeCss(this);'"
							+ "onmouseover='javascript:addCss(this);'onclick='javascript:RemoveLiList(this);' title='%1$s'>"
							+ "<img src='../Style/themes/icons/emp.png' />%2$s<img src='../Style/themes/icons/close.png' /></li>",
					dt.getValue(i, "No").toString(), dt.getValue(i, "Name").toString());
		}
	...
	public final void ToMsg(String msg, String type) {
		...
		try {
			...
			_response.sendRedirect("./../MyFlowInfo" + BP.WF.Glo.getFromPageType() + ".jsp?FK_Flow=" + this.getFK_Flow()
					+ "&FK_Type=" + type + "&FK_Node=" + this.getFK_Node() + "&WorkID=" + this.getWorkID());
		} catch (Exception e) {
			// TODO Auto-generated catch block
			logger.info("ERROR", e);
		}
