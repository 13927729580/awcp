...
public class AllotTaskController extends BaseController {
	...
	public static void addCheckBox(GenerWorkerList wl, StringBuilder html) {
		...
		// type=checkbox checked="+wl.getIsEnable()+"/>");
		// html.append("<label
		// for=\""+idAndName+"\">"+BP.WF.Glo.DealUserInfoShowModel(wl.getFK_Emp(),
		// wl.getFK_EmpText())+"</label>");
		// html.append("</li>");
	}
	...
	public static void addButton(StringBuilder html, String cb_ids) {
		...
		html.append("&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type='button' id=\"" + idAndName + "\" name='" + idAndName
				+ "' value=\"" + "  确定  " + "\" class=\"am-btn am-btn-primary am-btn-xs\" onclick='checkSubmit(\""
				+ cb_ids + "\")'/>");
	}
	...
	public final void Confirm() {
		...
		try {
			...
			// for (GenerWorkerList wl :
			// GenerWorkerLists.convertGenerWorkerLists(wls))
			// {
			// boolean cbChecked = _request.getParameter("CB_" +
			// wl.getFK_Emp())==null?false:Boolean.valueOf(_request.getParameter("CB_"
			// + wl.getFK_Emp()));
			// if (cbChecked)
			// {
			// isHave0 = false;
			...
			for (GenerWorkerList wl : GenerWorkerLists.convertGenerWorkerLists(wls)) {
				...
				if (rm.getObjs().indexOf(wl.getFK_Emp()) != -1) {
					rm.setEmpsExt(rm.getEmpsExt() + "<font color=green>"
							+ BP.WF.Glo.DealUserInfoShowModel(wl.getFK_Emp(), wl.getFK_EmpText())
							+ "</font>&nbsp;&nbsp;");
				} else {
					rm.setEmpsExt(rm.getEmpsExt() + "<strike><font color=red>("
							+ BP.WF.Glo.DealUserInfoShowModel(wl.getFK_Emp(), wl.getFK_EmpText())
							+ "</font></strike>&nbsp;&nbsp;");
				}
			}
			...
			if (WebUser.getIsWap()) {
				...
				builder.append(
						BaseModel.AddLi("<a href='./WAP/Home.jsp' ><img src='/WF/Img/Home.gif' border=0/>主页</a>"));
				builder.append(
						BaseModel.AddLi("<a href='./WAP/Start.jsp' ><img src='/WF/Img/Start.gif' border=0/>发起</a>"));
				builder.append(
						BaseModel.AddLi("<a href='./WAP/Runing.jsp' ><img src='/WF/Img/Runing.gif' border=0/>待办</a>"));
	...
	public final void DealWithFHLFlow(ArrayList al, GenerWorkerLists wlSeles) {
		...
		for (Object obj : al) {
			emps += obj.toString() + ",";
			myemp = obj.toString();
			DBAccess.RunSQL("UPDATE  WF_GenerWorkerlist SET IsEnable=1  WHERE FID=" + this.getFID() + " AND FK_Emp='"
					+ obj + "'");
		}
	...
	public final void DealWithPanelFlow(ArrayList al, GenerWorkerLists wlSeles) {
		...
		DBAccess.RunSQL("UPDATE  WF_GenerWorkerlist SET IsEnable=0  WHERE WorkID=" + this.getWorkID() + " AND FK_Node="
				+ NodeID);
		// string vals = "";
		String emps = "";
		String myemp = "";
		for (Object obj : al) {
			emps += obj.toString() + ",";
			myemp = obj.toString();
			DBAccess.RunSQL("UPDATE  WF_GenerWorkerlist SET IsEnable=1  WHERE WorkID=" + this.getWorkID()
					+ " AND FK_Node=" + NodeID + " AND fk_emp='" + obj + "'");
		}

		BP.WF.Template.Node nd = new BP.WF.Template.Node(NodeID);
