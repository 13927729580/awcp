...
public class TestFlowModel extends BaseModel{
	...
	public final void loadPage() throws IOException
	{
		...
		// 让admin 登录.
		BP.WF.Dev2Interface.Port_Login(SystemConfig.getAppSettings().get("Admin").toString());
		...
		if (this.getFK_Flow() == null)
		{
			...
			appendPub(AddLi("提供此功能的目的是，快速的让各个角色人员登陆，以便减少登陆的繁琐麻烦。"));
			appendPub(AddLi("点左边的流程列表后，系统自动显示能够发起此流程的工作人员，点一个工作人员就直接登陆了。"));
			appendPub(AddULEnd());
...
//		appendPub(AddTDTitle("AmazeUI模式"));
		appendPub(AddTH("table_type", "Dept"));
		appendPub(AddTREnd());
		boolean is1 = false;
		int idx = 0;
...
//			appendPub(AddTD("<a href='./../Port.jsp?DoWhat=StartLigerUI&UserNo=" + emp.getNo() + "&FK_Flow=" + this.getFK_Flow() + "&Lang=" +  WebUser.getSysLang() + "&Type=" + getParameter("Type") + "'  ><img src='./../Img/IE.gif' border=0 />LigerUI模式</a>"));
		   ...
		   // appendPub(AddTD("<a href='./../Port.aspx?DoWhat=Start&UserNo=" + emp.No + "&FK_Flow=" + this.FK_Flow + "&Lang=" + BP.Web.WebUser.SysLang + "&Type=" + getParameter(["Type"] + "'  ><img src='./../Img/IE.gif' border=0 />Blog模式</a>");                
			//appendPub(AddTD("<a href='./../Port.aspx?DoWhat=StartSmallSingle&UserNo=" + emp.No + "&FK_Flow=" + this.FK_Flow + "&Lang=" + BP.Web.WebUser.SysLang + "&Type=" + getParameter(["Type"] + "'  ><img src='./../Img/IE.gif' border=0 />Internet Explorer</a>");
...
//			appendPub(AddTD("<a href='./../Port.jsp?DoWhat=Amaze&UserNo=" + emp.getNo() + "&FK_Flow=" + this.getFK_Flow() + "&Lang=" + WebUser.getSysLang() + "&Type=" + getParameter("Type") + "'  ><img src='./../Img/IE.gif' border=0 />AmazeUI模式</a>"));
...
//
		...
		appendPub(AddFieldSetEnd());
		String buttom = "<div><a href=\"javascript:ShowIt('BP.WF.DTS.ClearDB');\"> <img src='../Img/Btn/Delete.gif' border=0 />清除所有流程运行的数据(此功能要在测试环境里运行)</a> <br> <font size=2 color=\"Green\">清除所有流程运行的数据，包括待办工作。</font> <br><br> <a href=\"javascript:Open('"+getFK_Flow()+"');\"> <img src='../Img/Btn/Delete.gif' border=0 />删除本流程数据(此功能要在测试环境里运行)</a> <br> <font size=\"2\" color=\"Green\">清除本流程运行的数据，包括待办工作。</font> <br></div>";
		appendPub(buttom);
	}
}
