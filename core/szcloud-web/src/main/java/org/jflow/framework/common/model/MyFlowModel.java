...
public class MyFlowModel{
	...
	public void initFlow() {
		...
		if (currND.getHisFormType() == NodeFormType.SheetTree || currND.getHisFormType() == NodeFormType.WebOffice){
			...
			if (workId == 0){
				if (StringHelper.isNullOrEmpty(pFlowNo)){
					...
					workId = Dev2Interface.Node_CreateBlankWork(fk_flow, null, null, WebUser.getNo(), null, Long.parseLong(pWorkID), pFlowNo, Integer.parseInt(pNodeID), null);
				}
		...
		if (currND.getHisFormType() == NodeFormType.SelfForm){
            ...
            selfFromPub = "<iframe ID='SelfForm' src='" + url + "' frameborder=0  style='width:100%; height:800px' leftMargin='0' topMargin='0' />";
            selfFromPub += "\t\n</iframe>";
            return;
		}
		// endregion 处理表单类型.
		...
		if (!isCC && currND.getIsStartNode() == false && 
				Dev2Interface.Flow_IsCanDoCurrentWork(fk_flow, Integer.parseInt(fk_node), workId, WebUser.getNo()) == false){
