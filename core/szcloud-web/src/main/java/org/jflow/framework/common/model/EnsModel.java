...
//		Entity en = BP.En.ClassFactory.GetEns(this.getEnsName()).getGetNewEntity();
	...
	public final void Bind()
	{
		...
		///#region 生成数据
		int i = 0;
		for (Entity dtl : Entities.convertEntities(ens))
		{
			String urlExt = "\"javascript:ShowEn('"+Glo.getCCFlowAppPath()+"WF/Comm/RefFunc/UIEn.jsp?EnsName=" + ens.toString() + "&PK=" + dtl.getPKVal() + "', 'cd');\"";
			i++;
			...
			if (isShowOpenICON)
			{
				this.ucsys1.append("<TD class='TD' style='cursor:hand;' nowrap=true><a href=" + urlExt + " ><img src='"+Glo.getCCFlowAppPath()+"WF/Img/Btn/open.gif' border=0/></a></TD>");
			}
			else
			{
				this.ucsys1.append(AddTD());
