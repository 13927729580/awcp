...
import BP.WF.Template.Form.Flow.FlowFormTrees;
...
public class SDKBaseController extends BaseController {
	...
	private String GetAppToolBar() {
		...
		if (btnLab.getCCRole() != CCRole.UnCC) {
			toolsBar.append("{");
	...
	private String SendCase() {
		...
		try {
			...
			if (UnSendindex != -1) {
				...
				resultMsg = kj
						+ "<a href='javascript:UnSend();'><img src='../../Img/UnDo.gif' border=0/>Undo this send</a>"
						+ kp;
			}

			resultMsg = resultMsg.replace("指定特定的处理人处理", "指定人员");
			resultMsg = resultMsg.replace("发手机短信提醒他(们)", "短信通知");
			resultMsg = resultMsg.replace("Undo this send", "撤销案件");
			resultMsg = resultMsg.replace("New process", "发起案件");
	...
	private String GetFlowFormTree() {
		...
		for (FrmNode frmNode : FrmNodes.convertFrmNodes(frmNodes)) {
	...
	// string nodeId = getUTF8ToString("nodeId");
	// //流程表单树父节点
	// BP.WF.Template.FlowFormTrees flowFormTree = new
	// BP.WF.Template.FlowFormTrees();
	...
	// private void GetChildTreeNode(string flowId, string parentNo, FlowForms
	// flowForms, NodeForms nodeForms)
	// {
	// //流程表单树父节点
	// BP.WF.Template.FlowFormTrees flowFormTree = new
	// BP.WF.Template.FlowFormTrees();
	 ...
	 * 初始节点表单树
	 * 
	 * //private void InitFlowFormTree(string flowId) //{ // BP.WF.Flow flow =
	 * new Flow(flowId); // //获取模版 // SysFormTree sysFormTree = new
	 * SysFormTree(); // sysFormTree.RetrieveByAttr(SysFormTreeAttr.Name,
	 * flow.Name);
	 */
	...
	// {
	// //添加到流程
	// BP.WF.Template.FlowFormTree flowFormTree = new
	// BP.WF.Template.FlowFormTree();
	...
	// private void InitChildNode(string flowId, string parentNo, string
	// wfParetNo)
	// {
	// //获取模版
	// SysFormTrees formTrees = new SysFormTrees();
	...
	// BP.WF.Template.FlowFormTree pFlowFormTree = new
	// BP.WF.Template.FlowFormTree(wfParetNo);

	// foreach (SysFormTree item in formTrees)
	// {
	// string subNo = pFlowFormTree.DoCreateSubNode().No;
	// BP.WF.Template.FlowFormTree flowFormTree = new
	// BP.WF.Template.FlowFormTree(subNo);
	...
	// DataTable dt_Forms = sysForm.RunSQLReturnTable("SELECT No,Name FROM
	// Sys_MapData WHERE FK_FormTree='" + parentNo + "' ORDER BY Name");
	// //流程表单
	...
	public final void TansEntitiesToGenerTree(Entities ens, String rootNo, String checkIds) {
		...
		if (formTree != null) {
			...
			url = url.replace("/", "|");
			appendMenus.append(
					",\"attributes\":{\"NodeType\":\"" + formTree.getNodeType() + "\",\"Url\":\"" + url + "\"}");
		}
		// 增加它的子级.
	...
	public final void AddChildren(EntityMultiTree parentEn, Entities ens, String checkIds) {
		...
		appendMenuSb.append("[");
		for (Entity entity : Entities.convertEntities(ens)) {
