...
import org.jflow.framework.designer.model.ListSelectionMode;
...
public class MExtController extends BaseController {
	...
	public String btn_SaveJiLian_Click(HttpServletResponse response, String FK_MapData, String ExtType,
			TempObject object) throws IOException {
		...
		try {
			...
			BaseModel.Alert("没有与之对应的下拉框");
			return "redirect:/WF/MapDef/MapExt.jsp?s=3&FK_MapData=" + FK_MapData + "&ExtType=" + ExtType + "&RefNo = "
					+ this.getRefNo();
		}
		...
		if (me.getAttrsOfActive() == me.getAttrOfOper()) {
			this.printAlert(response, "两个项目不能相同.");
			// return ;
		}
		try {
	...
	public String rbCheckChange(TempObject object, String DoType, String FK_MapData, String OperAttrKey,
			String ExtType) {
		HashMap<String, BaseWebControl> controls = HtmlUtils.httpParser(object.getFormHtml(), true);
		String path = BP.Sys.SystemConfig.getPathOfData() + File.separator + "JSLib" + File.separator;
		RadioButton rb = (RadioButton) controls.get("RB_0"); // sender as
		...
		mes.Retrieve(MapExtAttr.FK_MapData, FK_MapData, MapExtAttr.AttrOfOper, OperAttrKey, MapExtAttr.ExtType,
				ExtType);
		...
		return "redirect:/WF/MapDef/MapExt.jsp?FK_MapData=" + FK_MapData + "&ExtType=" + ExtType + "&RefNo = "
				+ this.getRefNo() + "&OperAttrKey=" + OperAttrKey + "&DoType=" + DoType;
	}

	@RequestMapping(value = "/SaveInputCheck", method = RequestMethod.POST)
	public @ResponseBody void saveInputCheck(HttpServletRequest request, TempObject object, String DoType,
			String FK_MapData, String OperAttrKey, String ExtType) throws IOException {
		HashMap<String, BaseWebControl> controls = HtmlUtils.httpParser(object.getFormHtml(), true);
		ListBox lb = (ListBox) controls.get("LB1");

		// 检查路径. 没有就创建它。
		String pathDir = BP.Sys.SystemConfig.getPathOfDataUser() + File.separator + "JSLibData" + File.separator;
		File file = new File(pathDir);
		if (!file.exists())
			file.mkdirs();

		logger.debug("测试" + request.getParameter("LB1"));

		// 删除已经存在的数据.
		MapExt me = new MapExt();
		me.Retrieve(MapExtAttr.FK_MapData, FK_MapData, MapExtAttr.ExtType, ExtType, MapExtAttr.AttrOfOper, OperAttrKey);
		...
		MapExts mes = new MapExts();
		mes.Retrieve(MapExtAttr.FK_MapData, FK_MapData, MapExtAttr.ExtType, ExtType);

		String js = "";
		for (MapExt me1 : MapExts.convertMapExts(mes)) {
			js += "\r\n" + BP.DA.DataType.ReadTextFile(me1.getTag());
		}
		File f = new File(pathDir + File.separator + FK_MapData + ".js");
		if (f.exists())
			f.delete();

		BP.DA.DataType.WriteFile(pathDir + File.separator + FK_MapData + ".js", js.trim());
		// #endregion 把所有的js 文件放在一个文件里面。

		//
		this.getResponse().sendRedirect(Glo.getCCFlowAppPath() + "WF/MapDef/MapExt.jsp?FK_MapData=" + FK_MapData
				+ "&ExtType=" + ExtType + "&RefNo = " + this.getRefNo());
	}

	@RequestMapping(value = "/ExpressionEdit", method = RequestMethod.POST)
	public void expressionSave(HttpServletResponse response, HttpServletRequest request, String FK_MapData,
			String ExtType, String OperAttrKey, TempObject object) throws IOException {
		...
		response.sendRedirect(
				Glo.getCCFlowAppPath() + "WF/MapDef/MapExt.jsp?s=3&FK_MapData=" + FK_MapData + "&ExtType=" + ExtType);
	}

	@RequestMapping(value = "/SavePopVal", method = RequestMethod.POST)
	public void savePopVal(HttpServletResponse response, HttpServletRequest request, String FK_MapData, String ExtType,
			String OperAttrKey, TempObject object, String RefNo) throws IOException {
		...
		me.Save();
		response.sendRedirect(Glo.getCCFlowAppPath() + "WF/MapDef/MapExt.jsp?FK_MapData=" + FK_MapData + "&ExtType="
				+ ExtType + "&RefNo = " + RefNo);
	}

	public void BindRegularExpressionEdit_ClickSave(String myEvent, String FK_MapData, String ExtType,
			String OperAttrKey, HashMap<String, BaseWebControl> controls, HttpServletRequest request) {
		MapExt me = new MapExt();
		me.setFK_MapData(FK_MapData);
		me.setExtType(ExtType);
		me.setTag(myEvent);
	...
	public void SaveAutoFull(HttpServletResponse response, HttpServletRequest request, String FK_MapData,
			String ExtType, String MyPK, TempObject object) throws IOException {
	...
	public void SaveAutoFullDtl(HttpServletResponse response, HttpServletRequest request, String RefNo,
			String FK_MapData, String ExtType, String MyPK, TempObject object) throws IOException {
		...
		// {
		// this.Response.Redirect("MapExt.aspx?FK_MapData=" +
		...
		String info = "";
		String error = "";
		for (MapDtl dtl : dtls.convertMapDtls(dtls)) {
			...
			try {
				...
				// {
				// if (attrs.IsExits(MapAttrAttr.KeyOfEn, dc.ColumnName) ==
				// false)
				// {
		...
		if (error != "") {
			// this.Pub2.AddEasyUiPanelInfo("错误", "设置错误,请更正:<br />" + error,
			// "icon-no");
		...
		myme.Update();
		this.getResponse().sendRedirect(Glo.getCCFlowAppPath() + "WF/MapDef/MapExt.jsp?FK_MapData=" + FK_MapData
				+ "&ExtType=" + ExtType + "&MyPK=" + MyPK + "&RefNo=" + RefNo);
	}

	@RequestMapping(value = "/SaveAutoFullM2M", method = RequestMethod.POST)
	public void SaveAutoFullM2M(HttpServletResponse response, HttpServletRequest request, String RefNo,
			String FK_MapData, String ExtType, String MyPK, TempObject object) throws IOException {
		...
		String info = "";
		String error = "";
		for (MapM2M m2m : m2ms.convertMapM2Ms(m2ms)) {
		...
		myme.Update();
		response.sendRedirect(Glo.getCCFlowAppPath() + "WF/MapDef/MapExt.jsp?FK_MapData=" + FK_MapData + "&ExtType="
				+ ExtType + "&MyPK=" + MyPK + "&RefNo=" + RefNo);
	}

	@RequestMapping(value = "/AutoFullDLL", method = RequestMethod.POST)
	public String AutoFullDLL(HttpServletResponse response, HttpServletRequest request, String RefNo, String FK_MapData,
			String ExtType, String MyPK, TempObject object) throws IOException {
		...
		try {
			...
			return "redirect:/WF/MapDef/MapExt.jsp?FK_MapData=" + this.getFK_MapData() + "&ExtType="
	...
	public String BindLinkEdit(HttpServletResponse response, HttpServletRequest request, String RefNo,
			String FK_MapData, String ExtType, String OperAttrKey, String MyPK, TempObject object) throws IOException {
		...
		// me.Delete();
		// this.Response.Redirect("MapExt.jsp?s=3&FK_MapData=" +
	...
	public String BindLinkDelete(HttpServletResponse response, HttpServletRequest request, String RefNo,
			String FK_MapData, String ExtType, String OperAttrKey, String MyPK, TempObject object) throws IOException {
		MapExt me = new MapExt();
		me.setMyPK(MyPK);
		me.Delete();
	...
	public void SaveReTemplete(HttpServletResponse response, HttpServletRequest request, String RefNo,
			String FK_MapData, String ExtType, String OperAttrKey, String MyPK, TempObject object) throws IOException {
		...
		exts.Delete(MapExtAttr.AttrOfOper, OperAttrKey, MapExtAttr.ExtType, MapExtXmlList.RegularExpression);
		String str = "";
		for (ListItem li : lb.Items) {
			if (!li.getSelected())
				continue;
		...
		for (RegularExpressionDtl dtl : RegularExpressionDtls.convertRegularExpressionDtls(reDtls)) {
			try {
				...
				// 小周鹏修改 Start
				response.sendRedirect(Glo.getCCFlowAppPath() + "WF/MapDef/MapExt.jsp?FK_MapData=" + this.getFK_MapData()
						+ "&ExtType=" + ExtType + "&MyPK=" + newMyPk + "&OperAttrKey=" + OperAttrKey + "&DoType=New");
				// response.sendRedirect(Glo.getCCFlowAppPath()+"WF/MapDef/MapExt.jsp?FK_MapData="
				// + this.getFK_MapData() + "&ExtType=" + ExtType + "&MyPK=" +
				// newMyPk + "&OperAttrKey=" + OperAttrKey + "&DoType=New");
				// 小周鹏修改 End
			}

			MapExt ext = new MapExt();
			ext.setMyPK(
					FK_MapData + "_" + OperAttrKey + "_" + MapExtXmlList.RegularExpression + "_" + dtl.getForEvent());
		...
		response.sendRedirect(Glo.getCCFlowAppPath() + "WF/MapDef/MapExt.jsp?FK_MapData=" + this.getFK_MapData()
				+ "&ExtType=" + ExtType + "&MyPK=" + newMyPk + "&OperAttrKey=" + OperAttrKey + "&DoType=New");
		// response.sendRedirect(Glo.getCCFlowAppPath()+"WF/MapDef/MapExt.jsp?FK_MapData="
		// + this.getFK_MapData() + "&ExtType=" + ExtType + "&MyPK=" + newMyPk +
		// "&OperAttrKey=" + OperAttrKey + "&DoType=New");
		// 小周鹏修改 End
	}

	@RequestMapping(value = "/SaveAutoFullJilian", method = RequestMethod.POST)
	public String SaveAutoFullJilian(HttpServletResponse response, HttpServletRequest request, String RefNo,
			String FK_MapData, String ExtType, String OperAttrKey, String MyPK, TempObject object) throws IOException {
		...
		for (MapAttr attr : MapAttrs.convertMapAttrs(attrs)) {
			...
			try {
				...
				// this.Alert("SQL ERROR: " + ex.getMessage());
				System.err.println("SQL ERROR: " + ex.getMessage());
				return "/WF/MapDef/MapExt.jsp?FK_MapData=" + FK_MapData + "&ExtType=" + ExtType + "&MyPK=" + MyPK
						+ "&RefNo=" + RefNo;
			}
		...
		myme.Update();
		// this.Alert("保存成功.");
		return "/WF/MapDef/MapExt.jsp?FK_MapData=" + FK_MapData + "&ExtType=" + ExtType + "&MyPK=" + MyPK + "&RefNo="
				+ RefNo;
	}

	@RequestMapping(value = "/SavePageLoadFull", method = RequestMethod.POST)
	public String SavePageLoadFull(HttpServletResponse response, HttpServletRequest request, String RefNo,
			String FK_MapData, String ExtType, String OperAttrKey, String MyPK, TempObject object, String url)
			throws IOException {
