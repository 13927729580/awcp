...
public class FrmController extends BaseController {
	...
	public void Btn_Save_Click(HttpServletRequest request, HttpServletResponse response) {
		try {
			...
			if (md.getIsHaveCA() == true) {
				if (!StringHelper.isNullOrEmpty(getTB_SealData())) {
					...
					if (data.length != 0) {
						...
						// System.Drawing.Bitmap(MS);
						// image.Save(getTB_SealFile(),
			...
			// this.get_response().sendRedirect("Frm.jsp?OID=" +
			// en.GetValStringByKey("OID") + "&FK_Node=" + this.FK_Node +
			// "&FID=" + this.FID + "&FK_MapData=" + this.FK_MapData, true);
	...
	public final void SaveNode() {
		...
		try {
			...
			// AlertMsg_Warning("错误", ex.getMessage() +
			// "@有可能此错误被系统自动修复,请您从新保存一次.");
			return;
		}

		// this.get_response().sendRedirect("Frm.jsp?OID=" +
		// wk.GetValStringByKey("OID") + "&FK_Node=" + this.FK_Node + "&WorkID="
		// + this.OID + "&FID=" + this.FID + "&FK_MapData=" + this.FK_MapData,
		// true);
		return;
	}
}
