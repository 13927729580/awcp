...
public class EnModel extends BaseModel {
	...
	public void BindCCForm(Entity en, MapData md, MapAttrs mattrs,
		...
		// 根据宽度计算出来微调.
		wtX = MapData.GenerSpanWeiYi(md, srcWidth)/2;
		...
		for (FrmAttachment ath : FrmAttachments.convertFrmAttachments(aths)) {
			...
			if (ath.getUploadType().equals(AttachmentUploadType.Single)) {
				...
				tempStr.append("<iframe id='F")
						...
						.append(h)
						.append("px' leftMargin='0'  topMargin='0' scrolling=auto");
	...
	private void printFrmRBs() {
		...
		for (MapAttr attr : MapAttrs.convertMapAttrs(mattrs)) {
			if (attr.getUIContralType().equals(UIContralType.RadioBtn)) {
				String id = "RB_" + attr.getKeyOfEnToLowerCase() + "_"
						+ HisEn.GetValStrByKey(attr.getKeyOfEnToLowerCase());
		...
		for (FrmRB rb : FrmRBs.convertFrmRBs(myrbs)) {
			...
			rbCtl.setId("RB_" + rb.getKeyOfEnToLowerCase() + "_"
					+ rb.getIntKey());
			rbCtl.setName("RB_" + rb.getKeyOfEnToLowerCase());
	...
	private void printBaseDataUI() {
		for (MapAttr attr : MapAttrs.convertMapAttrs(mattrs)) {
			if (!attr.getUIVisible()) {
				...
				tbH.setId("TB_" + attr.getKeyOfEnToLowerCase());
				tbH.setName("TB_" + attr.getKeyOfEnToLowerCase());
			...
			if (!attr.getUIIsEnable()
					&& LinkFields.contains("," + attr.getKeyOfEnToLowerCase()
							+ ",")) {
			...
			if (attr.getIsSigan()) {
				...
				if (!fk_node.equals("0") && !getIsReadonly()) {
			...
			if (attr.getUIContralType().equals(UIContralType.TB)) {
				...
				tb.setName("TB_" + attr.getKeyOfEnToLowerCase());
			...
			case Normal:
				...
				case BP.DA.DataType.AppBoolean:
					...
					cb.setId("CB_" + attr.getKeyOfEnToLowerCase());
					cb.setName("CB_" + attr.getKeyOfEnToLowerCase());
			...
			case Enum:
				if (attr.getUIContralType().equals(UIContralType.DDL)) {
					DDL ddle = new DDL();
					ddle.setId("DDL_" + attr.getKeyOfEnToLowerCase());
					ddle.setName("DDL_" + attr.getKeyOfEnToLowerCase());
			...
			case FK:
				DDL ddl1 = new DDL();
				ddl1.setId("DDL_" + attr.getKeyOfEnToLowerCase());
				ddl1.setName("DDL_" + attr.getKeyOfEnToLowerCase());
		...
		for (GroupField gf : GroupFields.convertGroupFields(gfs)) {
			...
			for (MapAttr attr : MapAttrs.convertMapAttrs(mattrs)) {
				...
				if (attr.getIsBigDoc() && attr.getUIIsLine()) {
					...
					mytbLine.setId("TB_" + attr.getKeyOfEnToLowerCase());
					mytbLine.setName("TB_" + attr.getKeyOfEnToLowerCase());
					if (attr.getIsBigDoc()) {
						// mytbLine = 5;
						// mytbLine.Columns = 30;
					}
	...
	private void LoadData(MapAttrs mattrs, Entity en) {
		...
		for (MapExt myitem : MapExts.convertMapExts(mes)) {
			if (myitem.getExtType().equals(MapExtXmlList.Link)) {
				LinkFields += "," + myitem.getAttrOfOperToLowerCase() + ",";
			}
		}
				...
				case Enum:
					if (attr.getUIContralType().equals(UIContralType.DDL)) {
						...
						for (SysEnum item : SysEnums.convertSysEnums(ses)) {
							...
							rb.setId("RB_" + attr.getKeyOfEnToLowerCase() + "_"
									+ item.getIntKey());
			...
			for (MapExt me : MapExts.convertMapExts(mes)) {
				...
				if (me.getExtType().equals(MapExtXmlList.DDLFullCtrl)) {
					...
					DDL ddlChild = (DDL) ctrlMap.get("DDL_"
							+ me.getAttrsOfActiveToLowerCase());
					...
					if (tbExp == null) {
						String rbName = "RB_" + me.getAttrOfOperToLowerCase();
		...
		for (MapExt ext : MapExts.convertMapExts(mes)) {
			...
			for (DataRow dr : dt.Rows) {
				...
				mytb.addAttr("onkeyup",
						"javascript:Auto" + ext.getAttrOfOperToLowerCase()
								+ "();");
			...
			// 判断类型
			for (MapAttr attr : MapAttrs.convertMapAttrs(mattrs)) {
				if (attr.getKeyOfEnToLowerCase().equals(
						ext.getAttrOfOperToLowerCase())) {
					myDataType = attr.getMyDataType();
				}
			}

			js.append("\t\n function Auto" + ext.getAttrOfOperToLowerCase()
					+ "() { ");
	...
	public final void AddRichTextBox(Entity en, MapAttr attr) {
		...
		tbd.setId("TB_" + attr.getKeyOfEnToLowerCase());
		tbd.setName("TB_" + attr.getKeyOfEnToLowerCase());
		...
		strs.append("\t\n var tbID='TB_").append(attr.getKeyOfEnToLowerCase())
				.append("'; ");
