...
public class WorkCheckModel {
	...
	*/
	public final void BindFreeModel(FrmWorkCheck wcDesc)
	{
		...
		for (BP.WF.Entity.Track tk : Tracks.convertTracks(tks))
		{
...
//C# TO JAVA CONVERTER TODO TASK: There is no preprocessor in Java:
					...
					if (tk.getHisActionType() == ActionType.Forward)
					{
						...
						if (fwc.getHisFrmWorkCheckSta() == FrmWorkCheckSta.Disable)
						{
							...
							{
								this.Pub1.append(BaseModel.AddTD(BP.WF.Glo.GenerUserSigantureHtml(ContextHolderUtils.getRequest().getSession().getServletContext().getRealPath("") , tk.getEmpFrom(), tk.getEmpFromT())));
							}
							else
							{
...
//C# TO JAVA CONVERTER TODO TASK: There is no preprocessor in Java:
	...
	public final void BindFreeModelV1_del(FrmWorkCheck wcDesc)
	{
		...
		{
			this.Pub1.append(BaseModel.AddTR());
			this.Pub1.append(BaseModel.AddTD("<div style='float:right'><img src='/WF/Img/Btn/Save.gif' border=0 />保存</div>"));
			this.Pub1.append(BaseModel.AddTREnd());
