	...
	public void execute(HttpServletRequest request, HttpServletResponse response, BindException errors)
		...
		if (doType.equals("Delete")) {
			...
			if (db.RetrieveFromDBSources() == 0) {
				db.Retrieve(FrmAttachmentDBAttr.FK_MapData, en.getClassID(), FrmAttachmentDBAttr.RefPKVal,
						en.getPKVal().toString(), FrmAttachmentDBAttr.FK_FrmAttachment,
			...
			if (id == 0) {
				...
				logger.debug("---" + db.getFileFullName());
				File file = new File(db.getFileFullName());
				if (file.isFile() && file.exists()) {
					file.delete();
					logger.debug("删除单个文件" + db.getFileName() + "成功！");
				} else {
					logger.debug("删除单个文件" + db.getFileName() + "失败！");
				}
			}
			String url = Glo.getCCFlowAppPath() + "WF/CCForm/SingleAttachmentUpload.jsp?" + request.getQueryString();
			this.windowReload(response, url);

		} else if (doType.equals("Upload")) {
			MultipartHttpServletRequest multipartRequest = (MultipartHttpServletRequest) request;
			CommonsMultipartFile item = (CommonsMultipartFile) multipartRequest.getFile("file_single");
			if (item.getOriginalFilename() != null && !item.getOriginalFilename().equals("")) {// 判断是否选择了文件
				long upFileSize = item.getSize(); // 上传文件的大小

				// 检查格式是否符合要求.
				if (frmAth.getExts().equals("") || frmAth.getExts().equals("*.*")) {
					// 任何格式都可以上传.
				} else {
					String fileExt = item.getOriginalFilename()
					...
					if (frmAth.getExts().toLowerCase().contains(fileExt) == false) {
						String url = Glo.getCCFlowAppPath() + "WF/CCForm/SingleAttachmentUpload.jsp?"
								+ request.getQueryString();
						this.printAlertReload(response, "您上传的文件格式不符合要求,要求格式为:" + frmAth.getExts(), url);
						return;
					}
				}
				...
				saveTo = saveTo.replace("\\\\", File.separator);
				try {
					File file = new File(saveTo);
					if (!file.exists()) {
						saveTo = ContextHolderUtils.getRequest().getSession().getServletContext()
				...
				saveTo = saveTo + File.separator + athDBPK + "."
				...
				try {
					...
					this.printAlertReload(response, "@文件存储失败,有可能是路径的表达式出问题,导致是非法的路径名称:" + ex.getMessage(), url);
					return;
				}
			...
			if (dbDown.RetrieveFromDBSources() == 0) {
				dbDown.Retrieve(FrmAttachmentDBAttr.FK_MapData, en.getClassID(), FrmAttachmentDBAttr.RefPKVal,
						en.getPKVal().toString(), FrmAttachmentDBAttr.FK_FrmAttachment,
						frmAth.getFK_MapData() + "_" + frmAth.getNoOfObj());
			}
			...
			if (file.exists()) {
				...
				String url = Glo.getCCFlowAppPath() + "WF/CCForm/SingleAttachmentUpload.jsp?"
						+ request.getQueryString();
				this.printAlertReload(response, "【" + dbDown.getFileName() + "】，该文件已不存在！", url);

			}

		} else if (doType.equals("Open")) {
			String url = BP.WF.Glo.getCCFlowAppPath() + "WF/WebOffice/AttachOffice.jsp?DoType=EditOffice&DelPKVal="
					+ athDBPK + "&FK_FrmAttachment=" + frmAth.getMyPK() + "&PKVal=" + pkVal + "&FK_Node="
					+ en.GetValStringByKey("FK_Node") + "&FK_MapData=" + frmAth.getFK_MapData() + "&NoOfObj="
					+ frmAth.getNoOfObj();
