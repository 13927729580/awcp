			...
			if (attr.getHisEditType() == EditType.UnDel || attr.getHisEditType() == EditType.Edit)
			{
				switch (attr.getLGType())
				{
					...
					case Enum:
						...
						break;
					case FK:
						appendPub("<a href=\"javascript:EditTable('" + this.getMyPK() + "','" + attr.getMyPK() + "','" + attr.getMyDataTypeS() + "');\"  alt='" + attr.getKeyOfEn() + "'>" + attr.getName() + "</a>");
						break;
				}
			}
			else
...
//						}
		...
		for (MapExt ext : MapExts.convertMapExts(exts))
		{
	...
	*/
	public final String GenerAutoFull(String pk, MapAttrs attrs, MapExt ext)
	{
		try
		{
			...
			for (MapAttr mattr : MapAttrs.convertMapAttrs(attrs))
			{
