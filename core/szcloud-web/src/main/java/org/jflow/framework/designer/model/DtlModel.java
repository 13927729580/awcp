...
public class DtlModel {
	...
	public void Bind() {
		...
		this.UCSys1.append(
				BaseModel.AddTable("class='Table' cellSpacing='1' cellPadding='1' border='1' style='width:100%'"));
		...
		try {
			// BaseModel.BindPageIdx_ver1(this.UCSys2, qo.GetCount(),
			// this.PageSize, this.getPageIdx(),
			// this.basePath+"WF/Comm/RefFunc/Dtl.jsp?EnName=" +
			// this.getEnName() + "&PK=" + this.getRefVal() + "&EnsName=" +
			// this.getEnsName() + "&RefVal=" + this.getRefVal() + "&RefKey=" +
			// this.getRefKey() + "&MainEnsName=" + this.getMainEnsName());
			BaseModel.BindPageIdxEasyUi(this.UCSys2, qo.GetCount(),
					this.basePath + "WF/Comm/RefFunc/Dtl.jsp?EnName=" + this.getEnName() + "&PK=" + this.getRefVal()
							+ "&EnsName=" + this.getEnsName() + "&RefVal=" + this.getRefVal() + "&RefKey="
							+ this.getRefKey() + "&MainEnsName=" + this.getMainEnsName(),
					this.getPageIdx(), this.PageSize, "'first','prev','sep','manual','sep','next','last'", false);

			qo.DoQuery(en.getPK(), this.PageSize, this.getPageIdx(), false);
		} catch (Exception e) {
			dtls.getGetNewEntity().CheckPhysicsTable();
			// this.Response.Redirect("Ens.aspx?EnsName=" + this.EnsName +
			// "&RefPKVal=" + this.RefPKVal, true);
			return;
		}
		// #endregion 生成翻页
		...
		for (Entity dtl : Entities.convertEntities(dtls)) {
			...
			if ("".equals(dtl.getPKVal().toString()) || "0".equals(dtl.getPKVal().toString())) {
				...
				// is1 = this.ucsys1.AddTR(is1, "ondblclick=\"WinOpen(
				// 'UIEn.aspx?EnsName=" + this.EnsName + "&PK=" + dtl.PKVal +
				// "', 'cd' )\"");
				...
				case TB:
					...
					if (attr.getIsNum() && !attr.getIsFKorEnum()) {
						if (tb.getEnabled()) {
							...
							// tb.Attributes["OnKeyDown"] = "javascript:return
							// VirtyNum(this);";
							// tb.Attributes["onkeyup"] += "javascript:C" +
							// dtl.PKVal + "();C" + attr.Key + "();";
							tb.addAttr("class", "TBNum");
						} else {
							// tb.Attributes["onpropertychange"] += "C" +
							// attr.Key + "();";
				...
				case DDL:
					...
					if (!attr.getUIIsReadonly()) {
						...
						if (attr.getIsEnum()) {
							...
							ddl.BindEntities(attr.getHisFKEns(), attr.getUIRefKeyValue(), attr.getUIRefKeyText());
						}
					}
			...
			if (isFJ) {
				...
				if (!ext.equals("")) {
					this.UCSys1.append(BaseModel.AddTD("<img src='../Images/FileType/" + ext + ".gif' border=0/>"
							+ dtl.GetValStrByKey("MyFileName")));
				}
			}
			this.UCSys1.append(BaseModel.AddTREnd());
		}
	...
	public static void main(String[] args) {
		...
		// cb.setText("选择全部");
		// logger.debug(((CheckBox)dot.ui.getTmpList().get(0)).getText());

		logger.debug(NamesOfBtn.Save.getDesc());
	}
}
