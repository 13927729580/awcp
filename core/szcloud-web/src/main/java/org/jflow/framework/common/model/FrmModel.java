...
public class FrmModel extends EnModel {
	...
	public final int getOID() {
		...
		if (StringHelper.isNullOrEmpty(cworkid) == false && Integer.parseInt(cworkid) != 0) {
	...
	public final void loadModel() throws IOException {
		Btn_Save = new Button();
		...
		// if (get_request().getParameter("IsLoadData").equals("1"))
		// {
		...
		if (md.RetrieveFromDBSources() == 0 && md.getName().length() > 3) {
			...
			if (md.getHisFrmType() == FrmType.Url) {
				...
				String urlParas = "OID=" + this.getOID() + "&NO=" + no + "&WorkID=" + this.getWorkID() + "&FK_Node="
						+ this.getFK_Node() + "&UserNo=" + WebUser.getNo() + "&SID=" + this.getSID();
				// 如果是URL.
				if (md.getUrl().contains("?") == true) {
					this.get_response().sendRedirect(md.getUrl() + "&" + urlParas);
				} else {
					this.get_response().sendRedirect(md.getUrl() + "?" + urlParas);
				}
				return;
			...
			if (dtlEn.getEnMap().getAttrs().size() <= 0) {
				md.RepairMap();
				this.get_response().sendRedirect(get_request().getRequestURL() + "?" + get_request().getQueryString());
				return;
			}

			int i = dtlEn.RetrieveFromDBSources();

			String[] paras = this.get_request().getQueryString().split("[&]", -1);
			...
			appendPub("<div id=divCCForm style='width:" + getWidth() + "px;height:" + getHeight() + "px' >");
			Dimension dimension = Toolkit.getDefaultToolkit().getScreenSize();
			if (md.getHisFrmType() == FrmType.FreeFrm) {
				BindCCForm(dtlEn, this.getFK_MapData(), !this.getIsEdit(), dimension.width);
			}
			...
			if (md.getHisFrmType() == FrmType.Url) {
				String no = get_request().getParameter("NO");
				String urlParas = "OID=" + this.getOID() + "&NO=" + no + "&WorkID=" + this.getWorkID() + "&FK_Node="
						+ this.getFK_Node() + "&IsEdit=" + (new Boolean(this.getIsEdit())).toString() + "&UserNo="
						+ WebUser.getNo() + "&SID=" + this.getSID();
				// 如果是URL.
				if (md.getUrl().contains("?") == true) {
					this.get_response().sendRedirect(md.getUrl() + "&" + urlParas);
				} else {
			...
			if (md.getHisFrmType() == FrmType.WordFrm) {
				...
				String urlParas = "OID=" + this.getOID() + "&NO=" + no + "&WorkID=" + this.getWorkID() + "&FK_Node="
						...
						+ this.getFK_MapData() + "&OIDPKVal=" + this.getOIDPKVal() + "&FID=" + this.getFID()
						+ "&FK_Flow=" + this.getFK_Flow();
				// 如果是URL.
				String requestParas = this.get_request().getQueryString();
				String[] parasArrary = requestParas.split("[&]", -1);
				...
				if (md.getUrl().contains("?") == true) {
					this.get_response().sendRedirect("WordFrm.jsp?1=2" + "&" + urlParas);
				} else {
					this.get_response().sendRedirect("WordFrm.jsp" + "?" + urlParas);
				}
				return;
			}

			if (md.getHisFrmType() == FrmType.ExcelFrm) {
				this.get_response().sendRedirect("FrmExcel.jsp?1=2" + "&" + get_request().getQueryString());
				return;
			}
			...
			if (!nodeid.equals("0") && StringHelper.isNullOrEmpty(this.getFK_Flow()) == false) {
			...
			if (en.getEnMap().getAttrs().size() <= 0) {
				...
				this.get_response()
						.sendRedirect(get_request().getRequestURL().append(get_request().getQueryString()).toString());
				return;
			}
			...
			String[] paras = get_request().getQueryString().split("[&]", -1);
			for (String str : paras) {
				if (StringHelper.isNullOrEmpty(str) || str.contains("=") == false) {
					continue;
				}
			...
			if (!StringHelper.isNullOrEmpty(nodeid)) {
				// UCEn1.fFK_Node = this.getFK_Node();
				// 处理表单权限控制方案
				this.setHisFrmNode(new FrmNode());
				int ii = this.getHisFrmNode().Retrieve(FrmNodeAttr.FK_Frm, this.getFK_MapData(), FrmNodeAttr.FK_Node,
						Integer.parseInt(nodeid));

				if (ii == 0 || this.getHisFrmNode().getFrmSln() == 0) {
					// 说明没有配置,或者方案编号为默认就不用处理,
					BindCCForm(en, this.getFK_MapData(), !this.getIsEdit(), 0);
				} else {
					FrmFields fls = new FrmFields(this.getFK_MapData(), this.getHisFrmNode().getFrmSln());
					...
					for (FrmField item : FrmFields.convertFrmFields(fls)) {
						for (MapAttr attr : MapAttrs.convertMapAttrs(mattrs)) {
							if (!attr.getKeyOfEn().equalsIgnoreCase(item.getKeyOfEn())) {
								continue;
							}
							...
							if (attr.getSignType() == SignType.CA) {
								...
								info.AddWhere(FrmFieldAttr.FK_Node, this.getFK_Node());
								info.addAnd();
								...
								if (info.DoQuery() > 0) {
									sealName = en.GetValStrByKey(attr.getKeyOfEn());
								}
							}
					...
					// 按照当前方案绑定表单.
					BindCCForm(en, md, mattrs, this.getFK_MapData(), !this.getIsEdit(), 0);
					...
					// 查询出来，需要不为空的
					Paras ps = new Paras();
					ps.SQL = "SELECT KeyOfEn, Name FROM Sys_FrmSln WHERE FK_MapData=" + ps.getDBStr()
							+ "FK_MapData AND FK_Node=" + ps.getDBStr() + "FK_Node AND IsNotNull=" + ps.getDBStr()
							+ "IsNotNull";
					...
					// 获取当前所有控件
					HashMap<String, BaseWebControl> ctrlMap = HtmlUtils.httpParser(Pub.toString(), false);
					// 查询
					...
					for (DataRow dr : dtKeys.Rows) {
						...
						if (TB_NotNull != null) {
							scriptCheckFrm += "\t\n try{  ";
							scriptCheckFrm += "\t\n var element = document.getElementById('" + TB_NotNull.getId()
									+ "');";
							// 验证输入的正则格式
			...
			if (md.getIsHaveCA()) {
				...
				if (md.getIsHaveCA() == true) {
					...
					String basePath = Glo.getIntallPath() + "/DataUser/Siganture/" + getWorkID();
					...
					this.TB_SealFile_Value = basePath + File.separator + sealName + ".jpg";
					/// #region 获取存储的 签名信息

					BP.Tools.WFSealData sealData = new BP.Tools.WFSealData();
					sealData.RetrieveByAttrAnd(BP.Tools.WFSealDataAttr.OID, getWorkID(),
							BP.Tools.WFSealDataAttr.FK_Node, getFK_Node());
		...
		if (curNd.getFormType() == NodeFormType.SheetTree) {
			...
			Btn_Print.addAttr("onclick",
					"window.open('Print.jsp?FK_Node=" + this.getFK_Node() + "&FID=" + this.getFID() + "&FK_MapData="
							+ this.getFK_MapData() + "&WorkID=" + this.getOID()
