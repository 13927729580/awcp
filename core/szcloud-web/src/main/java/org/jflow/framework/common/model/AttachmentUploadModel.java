...
public class AttachmentUploadModel {
	...
	public void init() {
		...
		if (this.getDoType().equals("Del")) {
			...
			if (file.isFile() && file.exists()) {
				file.delete();
				logger.debug("删除单个文件" + delDB.getFileName() + "成功！");
			} else {
				logger.debug("删除单个文件" + delDB.getFileName() + "失败！");
			}
		}
		...
		if (this.getDoType().equals("WinOpen")) {
			...
			String downpath = GetRealPath(downDB.getFileFullName());
			// ContextHolderUtils.getResponse().getWriter().(downpath);
			try {
		...
		if (this.getFK_Node() == 0 || this.getFK_Flow() == null) {
			...
			athDesc.setMyPK(this.getFK_FrmAttachment());
			if (athDesc.RetrieveFromDBSources() == 0 && StringHelper.isNullOrEmpty(this.getFK_Flow()) == false) {
				...
				// 存储路径.
				String path = ContextHolderUtils.getRequest().getSession().getServletContext()
						.getRealPath("/DataUser/UploadFile/");
				path += File.separator + "F" + this.getFK_Flow() + "MultiAth";
			...
			if (dt.Rows.size() == 0) {
				...
				if (sln == 0) {
					...
					int result = athDesc.Retrieve(FrmAttachmentAttr.FK_MapData, this.getFK_MapData(),
					...
					// throw new
					// Exception("@该流程表单在该节点("+this.FK_Node+")使用的是自定义的权限控制，但是没有定义该附件的权限。");
				}
			}
		...
		if (athDesc.getHisCtrlWay() == AthCtrlWay.PWorkID) {
			...
			int num = dbs.Retrieve(FrmAttachmentDBAttr.FK_FrmAttachment, this.getFK_FrmAttachment(),
					FrmAttachmentDBAttr.RefPKVal, this.getPKVal(), "RDT");
			if (num == 0 && this.getIsCC().equals("1")) {
				CCList cc = new CCList();
				int nnn = cc.Retrieve(CCListAttr.FK_Node, this.getFK_Node(), CCListAttr.WorkID, this.getWorkID(),
						CCListAttr.CCTo, WebUser.getNo());
				if (cc.getNDFrom() != 0) {
					this._fk_node = cc.getNDFrom();

					dbs.Retrieve(FrmAttachmentDBAttr.FK_FrmAttachment, this.getFK_FrmAttachmentExt(),
							FrmAttachmentDBAttr.FK_MapData, "ND" + cc.getNDFrom(), FrmAttachmentDBAttr.RefPKVal,
							(new Long(this.getWorkID())).toString());

					// 重新设置文件描述。
					athDesc.Retrieve(FrmAttachmentAttr.FK_MapData, this.getFK_MapData(), FrmAttachmentAttr.NoOfObj,
							"DocMultiAth");
				}
			}
		}
		...
		if (athDesc.getFileShowWay() == FileShowWay.Pict) {
			...
			this.Pub1.append("<div class='slideBox' id='" + athDesc.getMyPK() + "' style='width:" + athDesc.getW()
					+ "px;height:" + athDesc.getH() + "px; position:relative;  overflow:hidden;'>");
			this.Pub1.append("<ul class='items'>");
			for (FrmAttachmentDB db : FrmAttachmentDBs.convertFrmAttachmentDBs(dbs)) {
				if (db.getFileExts().toUpperCase().equals("JPG") || db.getFileExts().toUpperCase().equals("JPEG")
						|| db.getFileExts().toUpperCase().equals("GIF") || db.getFileExts().toUpperCase().equals("PNG")
						|| db.getFileExts().toUpperCase().equals("BMP")) {
					this.Pub1.append("<li> <a  title='" + db.getMyNote() + "'><img src = '" + db.getFileFullName()
							+ "' width=" + athDesc.getW() + " height=" + athDesc.getH() + "/></a></li>");
				}
			}

			this.Pub1.append("</ul>");
			this.Pub1.append("</div>");
			this.Pub1.append("<script>$(function(){$('#" + athDesc.getMyPK()
					+ "').slideBox({duration : 0.3,easing : 'linear',delay : 5,hideClickBar : false,clickBarRadius : 10});})</script>");
		...
		if (dbs.size() == 0 && athDesc.getIsWoEnableTemplete()) {
			...
			String templetePath = BP.Sys.SystemConfig.getPathOfDataUser() + "AthTemplete" + File.separator
			...
			for (File fl : fls) {
				...
				String saveTo = athDesc.getSaveTo() + File.separator + oid + "."
			...
			for (FrmAttachmentDB db : FrmAttachmentDBs.convertFrmAttachmentDBs(dbs)) {
				...
				if (CanEditor(db.getFileExts())) {
					if (athDesc.getIsWoEnableWF()) {
						this.Pub1.append(BaseModel.AddTD("<a href=\"javascript:OpenOfiice('"
								+ this.getFK_FrmAttachment() + "','" + this.getWorkID() + "','" + db.getMyPK() + "','"
								+ this.getFK_MapData() + "','" + this.getAth() + "','" + this.getFK_Node()
								+ "')\"><img src='../Img/FileType/" + db.getFileExts()
								+ ".gif' border=0 onerror=\"src='../Img/FileType/Undefined.gif'\" />" + db.getFileName()
								+ "</a>"));
					} else {
						/*
						 * warning this.Pub1.append(BaseModel.
						 * AddTD("<a href=\"javascript:OpenFileView('" +
						 * this.getPKVal() + "','" + db.getMyPK() +
						 * "')\"><img src='../Img/FileType/" + db.getFileExts()
						 * +
						 * ".gif' border=0 onerror=\"src='../Img/FileType/Undefined.gif'\" />"
						 * + db.getFileName() + "</a>"));
						 */
						this.Pub1.append(BaseModel.AddTD("<img src='../Img/FileType/" + db.getFileExts()
								+ ".gif' border=0 onerror=\"src='../Img/FileType/Undefined.gif'\" />"
								+ db.getFileName()));
					}
				} else if (db.getFileExts().toUpperCase().equals("JPG") || db.getFileExts().toUpperCase().equals("JPEG")
						|| db.getFileExts().toUpperCase().equals("GIF") || db.getFileExts().toUpperCase().equals("PNG")
						|| db.getFileExts().toUpperCase().equals("BMP") || db.getFileExts().toUpperCase().equals("PDF")
						|| db.getFileExts().toUpperCase().equals("CEB")) {
					/*
					 * warning this.Pub1.append(BaseModel.
					 * AddTD("<a href=\"javascript:OpenFileView('" +
					 * this.getPKVal() + "','" + db.getMyPK() +
					...
					this.Pub1.append(BaseModel.AddTD("<a href='AttachmentUpload.jsp?DoType=Down&MyPK=" + db.getMyPK()
							+ "' target=_blank ><img src='../Img/FileType/" + db.getFileExts()
							+ ".gif' border=0 onerror=\"src='../Img/FileType/Undefined.gif'\" />" + db.getFileName()
							+ "</a>"));
				}

				if (athDesc.getIsDownload()) {
					this.Pub1.append(BaseModel.AddTD("<a href=\"javascript:Down('" + this.getFK_FrmAttachment() + "','"
							+ this.getPKVal() + "','" + db.getMyPK() + "')\">下载</a>"));
				} else {
				...
				if (!this.getIsReadonly().equals("1")) {
					if (athDesc.getIsDelete()) {
						this.Pub1.append(BaseModel.AddTD("style='border:0px'",
								"<a href=\"javascript:Del('" + this.getFK_FrmAttachment() + "','" + this.getPKVal()
										+ "','" + db.getMyPK() + "')\">删除</a>"));
					} else {
						this.Pub1.append(BaseModel.AddTD(""));
					}
		...
		for (FrmAttachmentDB db : FrmAttachmentDBs.convertFrmAttachmentDBs(dbs)) {
			...
			if (athDesc.getIsDownload()) {
				if (athDesc.getIsWoEnableWF() && CanEditor(db.getFileExts())) {
					this.Pub1.append(BaseModel.AddTD("<a href=\"javascript:OpenOfiice('" + this.getFK_FrmAttachment()
							+ "','" + this.getWorkID() + "','" + db.getMyPK() + "','" + this.getFK_MapData() + "','"
							+ this.getAth() + "','" + this.getFK_Node() + "')\"><img src='../Img/FileType/"
							+ db.getFileExts() + ".gif' border=0 onerror=\"src='../Img/FileType/Undefined.gif'\" />"
							+ db.getFileName() + "</a>"));
				} else if (db.getFileExts().toUpperCase().equals("JPG") || db.getFileExts().toUpperCase().equals("JPEG")
						|| db.getFileExts().toUpperCase().equals("GIF") || db.getFileExts().toUpperCase().equals("PNG")
						|| db.getFileExts().toUpperCase().equals("BMP") || db.getFileExts().toUpperCase().equals("PDF")
			...
			if (!this.getIsReadonly().equals("1")) {
				String op = null;
				if (isDel == true) {
					op = "<a href=\"javascript:Del('" + this.getFK_FrmAttachment() + "','" + this.getPKVal() + "','"
							+ db.getMyPK() + "')\">删除</a>";
				}
	...
	private void AddFileUpload(boolean isUpdate, FrmAttachment athDesc) {
		if (isUpdate == true && !this.getIsReadonly().equals("1")) {
			...
			this.Pub1.append(
					"<input type=\"file\" name=\"file\" id=\"file\" onchange=\"UploadChange('Btn_Upload');\" />");
			// System.Web.UI.WebControls.FileUpload fu = new
			...
			if (athDesc.getIsNote()) {
				...
				// this.Pub1.append("&nbsp;备注:");
				this.Pub1.append(
						"<input name=\"TB_Note\" type=\"text\" size=\"30\" id=\"TB_Note\" Width=\"90%\" style=\"display:none;\" />");
			}
	...
	private String GetRealPath(String fileFullName) {
		...
		try {
			...
			String tmpStr = ContextHolderUtils.getRequest().getSession().getServletContext()
