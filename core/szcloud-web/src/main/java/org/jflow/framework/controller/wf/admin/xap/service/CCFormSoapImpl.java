...
//@WebService(endpointInterface = "org.jflow.framework.controller.wf.admin.xap.service.CCFormSoap")
public class CCFormSoapImpl implements CCFormSoap {
	...
	public String AttachmentFiles(String WorkID, String FK_Node, String FK_FrmAttachment, String UserNo,
			String UserName, byte[] FileByte, String fileName, String MyNote, String sort) {
	...
	public String SaveEnumField(String fk_mapData, String fieldKey, String fieldName, String enumKey, double x,
			double y) {
		try {
	...
	public String SaveImageAsFile(byte[] img, String pkval, String fk_Frm_Ele) {
		...
		// fe.FK_MapData + "\\");
		//
		// string saveTo = fe.HandSigantureSavePath + "\\" + fe.FK_MapData +
		// "\\" + pkval + ".jpg";
	...
	public String LoadFrmTemplete(byte[] fileByte, String fk_mapData, boolean isClear) {
		try {
			...
			String path = ContextHolderUtils.getRequest().getSession().getServletContext().getRealPath("/")
					+ File.separator + "DataUser" + File.separator + "Temp" + File.separator + file;
		...
		try {
			if (dotype.equals("CreateCheckGroup")) {
				...
				for (DataRow dr : dtEnum.Rows) {
					msgDelEnum += "\n 表单编号:" + dr.get("FK_MapData") + " , 字段:" + dr.get("KeyOfEn") + ", 名称:"
							+ dr.get("Name");
				}
				...
				String file = ContextHolderUtils.getRequest().getSession().getServletContext().getRealPath("/")
						+ File.separator + "Temp" + File.separator + v1 + ".xml";
				...
				// this.DownLoadFile(System.Web.HttpContext.Current, file,
				// mdfrmtem.Name);
				return null;
			} else if (dotype.equals("FrmTempleteImp")) {
				DataSet dsImp = new DataSet();
				String fileImp = ContextHolderUtils.getRequest().getSession().getServletContext().getRealPath("/")
						+ File.separator + "Temp" + File.separator + v1 + ".xml";
				...
				if (fn.Retrieve(FrmNodeAttr.FK_Frm, fk_frm, FrmNodeAttr.FK_Node, fk_Node) == 1) {
					fn.setIsEdit(!isReadonly);
					fn.setIsPrint(isPrint);
					fn.setFK_Flow(fk_flow);
					fn.Update();
					BP.DA.DBAccess
							.RunSQL("UPDATE Sys_MapData SET FK_FrmSort='01',AppType=1  WHERE No='" + fk_frm + "'");
					return fk_frm;
				}
	...
	public String FtpMethod(String doType, String v1, String v2, String v3) {
		try {
			// FtpSupport.FtpConnection conn = new
			// FtpSupport.FtpConnection("192.168.1.138", "administrator",
			// "jiaozi");
			// switch (doType)
			// {
			// case "ShareFrm": /*共享模板*/
			...
			// string file = BP.Sys.SystemConfig.PathOfTemp + v1 + "_" + v2 +
			// "_" + DateTime.Now.ToString("MM-dd hh-mm") + ".xml";
			// ds.WriteXml(file);
			...
			//
			// /*下载文件到指定的目录: */
			// string tempFile = BP.Sys.SystemConfig.PathOfTemp + "\\" + v2 +
			// ".xml";
			// conn.GetFile(v1, tempFile, false, FileAttributes.Archive,
			// FtpSupport.FtpTransferType.Ascii);
	...
	public String SaveFrm(String fk_mapdata, String xml, String sqls, String mapAttrKeyName) {
		...
		for (DataTable dt : ds.Tables) {
			try {
				...
				if (dt.TableName.toLowerCase() == "wf_node".toUpperCase() && dt.Columns.size() > 0
						&& dt.Rows.size() == 1) {
					/* 更新审核组件状态. */
		...
		// string sql = "SELECT KeyOfEn FROM Sys_MapAttr WHERE Name='' AND
		// FK_MapData='' ";
		...
		// sqls += "@ UPDATE Sys_MapAttr SET Name='"+kv[2]+"' WHERE
		// FK_MapData='' AND KeyOfEn=''";
		// }
		// }
		// // 执行sql.
		...
		// 备份文件
		// DoPortModel.w.WriteToXmlMapData(fk_mapdata, false);
