...
public class JFlowAPI implements JFlowSoap {
	...
	public String Flow_DoPress(long workid, String msg, String userNo) {
		...
		DataTable dataTable = new DataTable(Dev2Interface.Flow_DoPress(workid, msg, true));
	...
	public String Port_SMS(String userNo, String lastTime) {
		...
		Paras ps = new Paras();
		ps.SQL = "SELECT MsgType , Count(*) as Num FROM Sys_SMS WHERE SendTo='" + userNo + "' AND  RDT >'" + lastTime
	...
	public String DB_MobileMenu(String userNo) {
		DataSet ds = new DataSet();
		ds.readXml(Glo.getIntallPath() + File.separator + "Xml" + File.separator + "Mobile.xml");
		return BP.DA.DataType.ToJson(ds.Tables.get(0));
	}
	...
	public String Address_List() {
		...
		// string sql =
		// " select A.No, A.Name as UserName,B.Name as DeptName,A.Tel, A.Email
		// from WF_Emp as A,Port_Dept as B where A.FK_Dept=B.No order by B.No ";
		...
		for (DataRow dr : dt.Rows) {
			...
			dr.setValue("Img", Glo.getCCFlowAppPath() + "DataUser/UserIcon/" + dr.getValue("No") + "Smaller.png");
	...
	public String UserInfoByNo(String userNo) {
		String sql = " select A.No, A.Name as UserName,B.Name as DeptName,A.Tel,A.Email from Port_Emp as A,Port_Dept as B where A.FK_Dept=B.No and A.NO='"
	...
	public String UserInfoChange(String userNo, String userName, String tel, String email) {
		...
		String sSql = "Update Port_Emp set Name='" + userName + "',Tel='" + tel + "',Email='" + email + "' where No='"
				+ userNo + "'";
	 ...
	 * @throws IOException
	 */
	public String WriteUserMsg(String userNo, String msg) throws IOException {
		...
		String filePath = path + File.separator + userNo + "_" + DataType.getCurrentDateByFormart("yyyy_MM_dd_HH_mm_ss")
				+ ".txt";
	...
	public String DB_GenerWillReturnNodes(int nodeID, long workid, long fid, String userNo) {
		try {
			...
			DataSet ds = new DataSet();
			DataTable table = Dev2Interface.DB_GenerWillReturnNodes(nodeID, workid, fid);
			ds.Tables.add(table);
			return BP.DA.DataType.ToJson(ds.Tables.get(0));
			// return Connector.ToXml(ds);
		} catch (RuntimeException ex) {
	...
	public String DB_FlowComplete(String userNo, String sid, String fk_flow, int pageSize, int pageIdx) {
		try {
			...
			DataTable table = Dev2Interface.DB_FlowComplete(userNo, fk_flow, pageSize, pageIdx);
	 ...
	 * @return
	 */
	public String DataTable_DB_GenerWillReturnNodes(int nodeID, long workid, long fid, String userNo) {
		try {
			Emp emp = new Emp(userNo);
			WebUser.SignInOfGener(emp);
			DataSet ds = new DataSet();
	...
	public String DB_TaskPool(String userNo) {
		...
		ds.WriteXml("c:" + File.separator + "DB_TaskPool获得任务池的工作列表.xml");
		return BP.DA.DataType.ToJson(ds.Tables.get(0));
		// return Connector.ToXml(ds);
	}
	 ...
	 *            填写意见
	 */
	public String Node_DoCCCheckNote(String userNo, String sid, String fk_flow, int fk_node, long workID, long fid,
			String msge) {
		if (!userNo.equals(WebUser.getNo())) {
			Dev2Interface.Port_Login(userNo);
		}
	...
	public String DB_Truck(String flowNo, long WorkID, long FID) {
		...
		sql = "SELECT MyPK,ActionType,ActionTypeText,FID,WorkID,NDFrom,NDFromT,NDTo,NDToT,EmpFrom,EmpFromT,EmpTo,EmpToT,RDT,WorkTimeSpan,Msg,NodeData,Exer FROM ND"
				+ Integer.parseInt(flowNo) + "Track " + sqlOfWhere1 + " ORDER BY RDT";
		prs.SQL = sql;
	...
	public String Node_CreateBlankWork(String flowNo, String starter, String title) {
		...
		DataSet ds = new DataSet();
		long tableName = Dev2Interface.Node_CreateBlankWork(flowNo, null, null, starter, title);
	 ...
	 * @return
	 */
	public String Node_CC(String userNo, String sid, String fk_flow, int fk_node, long workID, String toEmpNos,
		...
		for (String item : toEmps) {
			...
			Dev2Interface.Node_CC(fk_flow, fk_node, workID, emp.getNo(), emp.getName(), msgTitle, msgDoc, pFlowNo,
					pWorkID);
		}
	...
	public String Node_HungUpWork(String fk_flow, long workid, int wayInt, String reldata, String hungNote) {
		return Dev2Interface.Node_HungUpWork(fk_flow, workid, wayInt, reldata, hungNote);
	}
	 ...
	 *            移交消息
	 */
	public String Node_Shift(String flowNo, int nodeID, long workID, long fid, String toEmp, String msg, String userNo,
			String sid) {
		if (!userNo.equals(WebUser.getNo())) {
			Dev2Interface.Port_Login(userNo);
		}
		return Dev2Interface.Node_Shift(flowNo, nodeID, workID, fid, toEmp, msg);
	}
	...
	public String Un_Node_Shift(String userNo, String sid, String fk_flow, long workID) {
	...
	public String Node_ReturnWork(String fk_flow, long workID, long fid, int currentNodeID, int returnToNodeID,
			String returnToEmp, String msg, boolean isBackToThisNode, String userNo, String sid) throws IOException {
		try {
			// 让用户登录.
			LetUserLogin(userNo, sid);
			return Dev2Interface.Node_ReturnWork(fk_flow, workID, fid, currentNodeID, returnToNodeID, returnToEmp, msg,
					isBackToThisNode);
		} catch (RuntimeException ex) {
			return "err@" + ex.getMessage();
		}
	...
	public String DoIt(String flag, String userNo, String sid, String fk_flow, String workID, String msg,
			String delModel, String val4, String val5) {
		LetUserLogin(userNo, sid);
		try {

			if (flag.equals("UnSend")) // 撤销发送..
			{
				return Dev2Interface.Flow_DoUnSend(fk_flow, Long.parseLong(workID));
			} else if (flag.equals("EndWorkFlow")) // 结束流程.
			{
				return Dev2Interface.Flow_DoFlowOver(fk_flow, Long.parseLong(workID), msg);
			} else if (flag.equals("DelWorkFlow")) // 删除流程.
			{
				String model = delModel;
				if (model.equals("1")) {
					// 逻辑删除
					return Dev2Interface.Flow_DoDeleteFlowByFlag(fk_flow, Long.parseLong(workID), msg, false);
				}
				if (model.equals("2")) {
					// 写入日志方式删除
					return Dev2Interface.Flow_DoDeleteFlowByWriteLog(fk_flow, Long.parseLong(workID), msg, false);
				}
				if (model.equals("3")) {
					// 彻底删除
					return Dev2Interface.Flow_DoDeleteFlowByReal(fk_flow, Long.parseLong(workID), false);
				}
	...
	public String GenerFlowTrack_Josn(String fk_flow, long workID, long fid, String userNo, String sid) {
	...
	public String GenerWorkNode_JSON(String fk_flow, int fk_node, long workID, long fid, String userNo, String sid) {
		this.LetUserLogin(userNo, sid);
		DataSet ds = this.GenerWorkNode(fk_flow, fk_node, workID, fid, userNo);
		return BP.Tools.FormatToJson.ToJson(ds);
	}

	private DataSet GenerWorkNode(String fk_flow, int fk_node, long workID, long fid, String userNo) {
		if (fk_node == 0) {
			fk_node = Integer.parseInt(fk_flow + "01");
		}

		if (workID == 0) {
			workID = Dev2Interface.Node_CreateBlankWork(fk_flow, null, null, userNo, null);
		}
		...
		try {
			...
			if (md.getMapDtls().size() > 0) {
				ArrayList<MapDtl> mapDtls = MapDtls.convertMapDtls(md.getMapDtls());
				for (MapDtl dtl : mapDtls) {
					...
					for (MapAttr attr : MapAttrs.convertMapAttrs(dtlAttrs)) {
						// 处理它的默认值.
			...
			DataTable dtMapAttr = myds.Tables.get(myds.Tables.indexOf("Sys_MapAttr"));
			for (DataRow dr : dtMapAttr.Rows) {
				...
				if (StringHelper.isNullOrEmpty(uiBindKey) == true) {
					...
					// 如果是空的
					throw new RuntimeException("@属性字段数据不完整，流程:" + fl.getNo() + fl.getName() + ",节点:" + nd.getNodeID()
							+ nd.getName() + ",属性:" + myPK + ",的UIBindKey IsNull ");
				}
			...
			// 放入track信息.
			Paras ps = new Paras();
			ps.SQL = "SELECT * FROM ND" + Integer.parseInt(fk_flow) + "Track WHERE WorkID="
			...
			// 工作人员列表，用于审核组件.
			ps = new Paras();
			ps.SQL = "SELECT * FROM  WF_GenerWorkerlist WHERE WorkID=" + BP.Sys.SystemConfig.getAppCenterDBVarStr()
					+ "WorkID";
			ps.Add("WorkID", wfid);
			...
			ps = new Paras();
			ps.SQL = "SELECT * FROM WF_SelectAccper WHERE WorkID=" + BP.Sys.SystemConfig.getAppCenterDBVarStr()
					+ "WorkID";
			ps.Add("WorkID", wfid);
			...
			// 放入所有的节点信息. 用于审核组件.
			ps = new Paras();
			ps.SQL = "SELECT * FROM WF_Node WHERE FK_Flow=" + BP.Sys.SystemConfig.getAppCenterDBVarStr()
			...
			Log.DebugWriteError(ConvertTools.getStackTraceString(ex.getStackTrace()));
	...
	public String GenerWorkNode_JSONV2(String fk_flow, int fk_node, long workID, long fid, boolean isCc, float srcWidth,
	...
	private DataSet GenerWorkNodeV2(String fk_flow, int fk_node, long workID, long fid, boolean iscc, float srcWidth,
			float srcHeight) {
		if (fk_node == 0) {
			fk_node = Integer.parseInt(fk_flow + "01");
		}
		...
		try {
			...
			// 流程数据. 计算出来表单的位移.
			String sql = "SELECT  Ver as FlowVer, '" + md.getVer() + "' as FormVer, "
					+ MapData.GenerSpanWeiYi(md, srcWidth) + " as WeiYi, " + MapData.GenerSpanHeight(md, srcHeight)
					+ " as SrcH, " + MapData.GenerSpanWidth(md, srcWidth) + " as SrcW  FROM WF_Flow WHERE No='"
					+ fk_flow + "'";
			DataTable dt = BP.DA.DBAccess.RunSQLReturnTable(sql);
			...
			dt.Rows.get(0).setValue("WeiYi2", MapData.GenerSpanWeiYi(md, srcHeight));
			dt.Rows.get(0).setValue("SrcH2", MapData.GenerSpanHeight(md, srcWidth));
			dt.Rows.get(0).setValue("SrcW2", MapData.GenerSpanWidth(md, srcHeight));
			...
			// sql =
			// "@SELECT
			// UIVisible,UIIsEnable,FK_MapData,MyPK,KeyOfEn,Name,DefVal,UIContralType,MyDataType,LGType,X,Y,UIBindKey,UIWidth,UIHeight
			// "
			...
			DataTable dtMapAttr = md.GenerHisDataSet().Tables.get(md.GenerHisDataSet().Tables.indexOf("Sys_MapAttr"));
			...
			for (DataRow dr : dtMapAttr.Rows) {
				...
				if (StringHelper.isNullOrEmpty(uiBindKey) == true) {
					String myPK = dr.getValue("MyPK").toString();
					// 如果是空的
					throw new RuntimeException("@属性字段数据不完整，流程:" + fk_flow + ",节点:" + nd.getNodeID() + nd.getName()
							+ ",属性:" + myPK + ",的UIBindKey IsNull ");
				}
			...
			if (dtGenerWorkerlist.Rows.size() != 0 && nd.getIsStartNode() == false && iscc == false) {
				for (DataRow dr : dtGenerWorkerlist.Rows) {
					if (dr.getValue(GenerWorkerListAttr.IsRead).toString().equals("1")
							&& dr.getValue(GenerWorkerListAttr.FK_Emp).toString().equals(WebUser.getNo())) {
						Dev2Interface.Node_SetWorkRead(fk_node, workID);
						break;
					}
				}
			...
			throw new RuntimeException(
					"Para:FK_Node=" + fk_node + ",workid:" + workID + ",UserNo  Ext:" + ex.getMessage());
		}
	}
	...
	// public string GetFlowTemplete(string fk_flow, string ver)
	{
		...
		Flow fl = new Flow(fk_flow);
		String path = BP.Sys.SystemConfig.getPathOfDataUser() + File.separator + "FlowDesc" + File.separator
				+ fl.getNo() + "." + fl.getName();

		// string fileName = path+ "\\Flow.xml";
		Node node = new Node(fk_node);
		String fileName = path + File.separator + fk_node + "." + node.getName() + ".xml";
		// 小周鹏修改-------------------------------END
		File file = new File(path);
		if (file.exists() == false) {
			// 如果不存在, 就要生成他。
	...
	public String WorkOpt_GetToNodes(String userNo, String sid, String fk_flow, int fk_node, long workID, long fid) {
		this.LetUserLogin(userNo, sid);

		Nodes nodes = Dev2Interface.WorkOpt_GetToNodes(fk_flow, fk_node, workID, fid);
	 ...
	 * @return 发送结果
	 */
	public String WorkOpt_SendToNodes(String userNo, String sid, String fk_flow, int fk_node, long workID, long fid,
			String to_node) {
		this.LetUserLogin(userNo, sid);
		...
		String msg = "";
		try {
			msg = Dev2Interface.WorkOpt_SendToNodes(fk_flow, fk_node, workID, fid, to_node).ToMsgOfText();
		} catch (RuntimeException ex) {
	...
	public String WorkOpt_AccepterDB(String userNo, String sid, String fk_flow, int fk_node, long workID, long fid) {
		try {
			this.LetUserLogin(userNo, sid);

			// 获取接收人DataSet
			DataSet ds = Dev2Interface.WorkOpt_AccepterDB(fk_flow, fk_node, workID, fid);
	...
	public String WorkOpt_SetAccepter(String userNo, String sid, int fk_node, long workID, long fid, String emps,
			boolean isNextTime) {
		this.LetUserLogin(userNo, sid);

		Dev2Interface.WorkOpt_SetAccepter(fk_node, workID, fid, emps, isNextTime);
	...
	public String Flow_DoDeleteFlowByReal(String userNo, String sid, String fk_flow, long workID,
	...
	public String Node_AskforReply(String userNo, String sid, String fk_flow, int fk_node, long workID, long fid,
			String replyNote) {
		this.LetUserLogin(userNo, sid);

		String info = Dev2Interface.Node_AskforReply(fk_flow, fk_node, workID, fid, replyNote);
	...
	private DataSet GenerWorkNode_FlowDataOnly(String fk_flow, int fk_node, long workID, long fid, String userNo) {
		if (fk_node == 0) {
			fk_node = Integer.parseInt(fk_flow + "01");
		}
		...
		try {
			...
			myds.Tables.add(wk.ToDataTableField(md.getNo()));
			if (md.getMapDtls().size() > 0) {
				for (MapDtl dtl : MapDtls.convertMapDtls(md.getMapDtls())) {
					GEDtls dtls = new GEDtls(dtl.getNo());
					QueryObject qo = null;
					try {
						qo = new QueryObject(dtls);
			...
			for (DataRow dr : dtMapAttr.Rows) {
				...
				if (StringHelper.isNullOrEmpty(uiBindKey)) {
					String myPK = dr.getValue("MyPK").toString();
					// 如果是空的
					throw new RuntimeException("@属性字段数据不完整，流程:" + nd.getFK_Flow() + nd.getFlowName() + ",节点:"
							+ nd.getNodeID() + nd.getName() + ",属性:" + myPK + ",的UIBindKey IsNull ");
				}
	...
	public String GenerFlowTemplete_Json(String fk_flow) throws IOException {
		Flow fl = new Flow(fk_flow);

		String path = BP.Sys.SystemConfig.getPathOfDataUser() + File.separator + "FlowDesc" + File.separator;

		DataSet myds = fl.DoExpFlowXmlTemplete(path);
		myds.WriteXml("c:" + File.separator + "GenerFlowTemplete_Json.xml");

		String strs = BP.Tools.FormatToJson.ToJson(myds);
		DataType.WriteFile("c:" + File.separator + "GenerFlowTemplete_Json.txt", strs);
		return strs;
	}
	...
	public String Node_SaveWork(String fk_flow, int fk_node, long workID, String userNo, String dsXml) {
		try {
			...
			return Dev2Interface.Node_SaveWork(fk_flow, fk_node, workID, htMain, null);
		} catch (RuntimeException ex) {
	...
	public String Node_SaveWork_Json(String fk_flow, int fk_node, long workID, long fid, String userNo, String sid,
			String jsonStr) {
		try {
			if (!userNo.equals(WebUser.getNo())) {
				Emp emp = new Emp(userNo);
			...
			for (DataColumn dc : dtMain.Columns) {
				htMain.put(dc.ColumnName, dtMain.Rows.get(0).getValue(dc.ColumnName).toString());
			}

			// 判断是否有审核数据表.
			if (ds.Tables.contains("FrmCheck")) {
				DataTable dtfrm = ds.Tables.get(ds.Tables.indexOf("FrmCheck"));
				String note = (String) ((dtfrm.Rows.get(0).getValue(0) instanceof String)
						? dtfrm.Rows.get(0).getValue(0) : null);
				String opName = (String) ((dtfrm.Rows.get(0).getValue(1) instanceof String)
						? dtfrm.Rows.get(0).getValue(1) : null);
				if (note != null) {
					Dev2Interface.WriteTrackWorkCheck(fk_flow, fk_node, workID, fid, note, opName);
				}
			}
	...
	public String Node_SendWork_Json(String fk_flow, int fk_node, long workID, long fid, String userNo, String sid,
			String jsonStr) {
		this.LetUserLogin(userNo, sid);
		try {
	...
	public String Node_SendWork(String fk_flow, int fk_node, long workID, String dsXml, String currUserNo) {
		try {
			...
			if (dsXml != null) {
				...
				DataSet ds = new DataSet();
				ds.readXml(dsXml);
				ds.WriteXml("c:" + File.separator + "GenerSendXml.xml");

				java.util.Hashtable htMain = new java.util.Hashtable();
				DataTable dtMain = ds.Tables.get(ds.Tables.indexOf("ND" + fk_node));
				for (DataRow dr : dtMain.Rows) {
					htMain.put(dr.getValue(0).toString(), dr.getValue(1).toString());
				}
				objs = Dev2Interface.Node_SendWork(fk_flow, workID, htMain, ds);
	...
	public String Node_Askfor(String userNo, String sid, long workID, int _askforHelpSta, String toEmpNo, String note) {
		if (!userNo.equals(WebUser.getNo())) {
			Dev2Interface.Port_Login(userNo);
		}
	...
	public boolean Upload(String fileName, long offSet, byte[] intoBuffer) {
		// 指定上传文件夹+文件名(相对路径)
		String strPath = "D:" + File.separator + "value-added" + File.separator + "CCFlow" + File.separator + "DataUser"
		...
		if (buffer != null) {
			...
			try {
				FileOutputStream fileOutputStream = new FileOutputStream(new File(strPath));
	...
	public String GetCheckInfo(String userNo, String sid, String fk_flow, int fk_node, long workID) {
		if (!userNo.equals(WebUser.getNo())) {
			Dev2Interface.Port_Login(userNo);
		}
	...
	public String WriteTrackWorkCheck(String userNo, String sid, String flowNo, int nodeFrom, long workid, long fid,
			String msg, String optionName) {
		if (!userNo.equals(WebUser.getNo())) {
			Dev2Interface.Port_Login(userNo);
		}

		Dev2Interface.WriteTrackWorkCheck(flowNo, nodeFrom, workid, fid, msg, optionName);

		// 设置审核完成.
		Dev2Interface.Node_CC_SetSta(nodeFrom, workid, WebUser.getNo(), CCSta.CheckOver);
