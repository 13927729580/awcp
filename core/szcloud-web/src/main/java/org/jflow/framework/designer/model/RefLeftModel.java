...
public class RefLeftModel{
	...
	public void init() {
		...
		try{
			...
			if (StringHelper.isNullOrEmpty(getPK())) return;
			if (en == null)
				throw new Exception(this.getEnsName() + " " + this.getEnName());
			if (en.getEnMap().getAttrsOfOneVSM().size() + en.getEnMap().getDtls().size() + en.getEnMap().getHisRefMethods().size() == 0)
                return;
	        ...
	        this.Pub1.append(BaseModel.AddLi(
	        		String.format("<div><a href='"+basePath+"WF/Comm/RefFunc/UIEn.jsp?EnName=%1$s&PK=%2$s'>%5$s<span class='nav'>%3$s</span></a></div>%4$s", this.getEnName(), this.getPK(), "Title".equals(titleKey) ? "主页" : desc, "\r\n", GetIcon(IconFirstDefault))));
            ...
            if (oneVsM.size() > 0){
            	 for(AttrOfOneVSM vsM : oneVsM){
            		 String url = basePath+"WF/Comm/RefFunc/Dot2Dot.jsp?EnsName=" + en.getGetNewEntities().getClass().getName() + "&EnName=" + this.getEnName() + "&AttrKey=" + vsM.getEnsOfMM().getClass().getName() + keys;
            		 try{
            		 ...
            		 if (i == 0){
            			 if ((this.getAttrKey()).equals(vsM.getEnsOfMM().getClass().getName())){
            				  this.Pub1.append(BaseModel.AddLi(
            						  String.format("<div style='font-weight:bold'><a href='%1$s'>%4$s<span class='nav'>%2$s</span></a></div>%3$s",
            			 ...
            			 if ((this.getAttrKey()).equals(vsM.getEnsOfMM().getClass().getName())){
            				 this.Pub1.append(BaseModel.AddLi(
            						 String.format("<div style='font-weight:bold'><a href='%1$s'>%5$s<span class='nav'>%2$s[%3$s]</span></a></div>%4$s",
            	                             url, vsM.getDesc(), i, "\r\n", GetIcon(IconM2MDefault))));
            				 this.ItemCount++;
            			 }else{
            				 this.Pub1.append(BaseModel.AddLi(
            ...
            for (RefMethod func : myreffuncs){
            	if(!func.Visable || null != func.RefAttrKey)continue;
            	...
            	if(func.refMethodType != RefMethodType.Func){
            		...
            		if (func.refMethodType == RefMethodType.RightFrameOpen){
            			 this.Pub1.append(BaseModel.AddLi(
            					 String.format("<div><a href='javascript:void(0)' onclick=\"javascript:OpenUrlInRightFrame(this,'%1$s')\" title='%2$s'>%5$s<span class='nav'>%3$s</span></a></div>%4$s",
            		...
            		if (func.Target == null || "".equals(func.Target)){
            			...
            			this.Pub1.append(BaseModel.AddLi(
            					String.format("<div><a href='%1$s' title='%2$s'>%5$s<span class='nav'>%3$s</span></a></div>%4$s", 
            							myurl, StringHelper.isEmpty(func.ToolTip, ""), func.Title, "\r\n", GetBasePathIcon(func.Icon))));
                        ItemCount++;
            		}else{
            			 //this.AddLi(func.GetIcon(path) + "<a href=\"javascript:WinOpen('" + myurl + "','" + func.Target + "')\" ToolTip='" + func.ToolTip + "' >" + func.Title + "</a>");
            			this.Pub1.append(BaseModel.AddLi(
            					String.format("<div><a href=\"javascript:WinOpen('%1$s','%2$s')\" title='%3$s'>%6$s<span class='nav'>%4$s</span></a></div>%5$s", 
                ...
                String url = basePath+"WF/Comm/RefMethod.jsp?Index=" + func.Index + "&EnsName=" + en.getGetNewEntities().getClass().getName() + keys;
                ...
                if (func.Warning == null || "".equals(func.Warning)){
                	 if (func.Target == null || "".equals(func.Target)){
                		 ...
                		 this.Pub1.append(BaseModel.AddLi(
                				 String.format("<div><a href='%1$s' title='%2$s'>%5$s<span class='nav'>%3$s</span></a></div>%4$s", 
                						 url, StringHelper.isEmpty(func.ToolTip, ""), func.Title, "\r\n", GetBasePathIcon(func.Icon))));
                         ItemCount++;
                     }else{
                	 ...
                	 if (func.Target == null || "".equals(func.Target)){
                		//this.AddLi(func.GetIcon(path) + "<a href=\"javascript: if ( confirm('" + func.Warning + "') ) { window.location.href='" + url + "' }\" ToolTip='" + func.ToolTip + "' >" + func.Title + "</a>");
                		 this.Pub1.append(BaseModel.AddLi(
                				 String.format("<div><a href=\"javascript: if ( confirm('%1$s')){{ window.location.href='%2$s' }}\" title='%3$s'>%6$s<span class='nav'>%4$s</span></a></div>%5$s",
                		 ...
                		 this.Pub1.append(BaseModel.AddLi(
                				 String.format("<div><a href=\"javascript: if ( confirm('%1$s')){{ WinOpen('%2$s','%3$s') }}\" title='%4$s'>%7$s<span class='nav'>%5$s</span></a></div>%6$s",
            ...
            for (EnDtl enDtl : enDtls){
            	String url = basePath+"WF/Comm/RefFunc/Dtl.jsp?EnName=" + this.getEnName() + "&PK=" + this.getPK() + "&EnsName=" + enDtl.getEns().getClass().getName() + "&RefKey=" + enDtl.getRefKey()+ "&RefVal=" + en.getPKVal().toString() + "&MainEnsName=" + en.getClass().getName() + keys;
    ...
    private String GetIcon(String imgPath){
        if (!this.ShowIconDefault) return "";
