...
public class EnsAppCfgController extends BaseController {
	...
	public void btn_Click(HttpServletRequest request, HttpServletResponse response, TempObject object) {
		...
		if (!StringHelper.isNullOrEmpty(this.getDoType())) {
			...
			HashMap<String, BaseWebControl> controlMap = HtmlUtils.httpParser(html, true);
		...
		if (WebUser.getNo().equals(SystemConfig.getAppSettings().get("Admin").toString())
				&& StringHelper.isNullOrEmpty(this.getDoType())) {
			...
			for (Attr attr : attrs) {
				...
				case CheckBok:
					if (request.getParameter("CB_" + attr.getKey()) != null
							&& request.getParameter("CB_" + attr.getKey()).equals("on")) {
						val = "1";
					} else {
						val = "0";
					}
	...
	// if (request.getParameter("CB_" +
	// xml.getNo())!=null&&request.getParameter("CB_" +
	// xml.getNo()).equals("on")) {
	// val = "1";
	// } else {
	// val = "0";
	...
	// Entity en1 =
	// BP.En.ClassFactory.GetEns(this.getEnsName()).getGetNewEntity();
	...
	// switch (attr.getUIContralType()) {
	// case DDL:
	//// val = this.UCSys1.GetDDLByID("DDL_" +
	...
	// if (request.getParameter("CB_" +
	// attr.getKey())!=null&&request.getParameter("CB_" +
	// attr.getKey()).equals("on")) {
	// val = "1";
	// } else {
	// val = "0";
	// }
