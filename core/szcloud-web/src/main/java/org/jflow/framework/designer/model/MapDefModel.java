...
public class MapDefModel {
	...
	public MapDefModel(String basePath, HttpServletRequest req, HttpServletResponse res, String MyPK, String FK_Flow,
			boolean IsEditMapData) {
		this.MyPK = MyPK;
	...
	public void BindLeft() {
		...
		// #region bindleft
		this.Left.append("<a href='http://ccflow.org' target=_blank ><img src='../../DataUser/ICON/"
				+ SystemConfig.getCompanyID() + "/LogBiger.png' border=0/></a>");
		this.Left.append(BaseModel.AddHR());
		this.Left.append(BaseModel.AddUL());
		for (int i = 0; i < xmls.size(); i++) {
			MapMenu item = (MapMenu) xmls.get(i);
			this.Left.append(BaseModel.AddLi("<a href=\""
					+ item.getJS().replace("@MyPK", "'" + this.FK_MapData + "'").replace("@FK_Flow",
							"'" + this.FK_Flow + "'")
					+ "\" ><img src='" + item.getImg() + "' width='16px' /><b>" + item.getName()
					+ "</b></a><br><font color=green>" + item.getNote() + "</font>"));
		}
		// for (MapMenu item : xmls)
		// {
		// this.Left.AddLi("<a href=\"" + item.getJS().Replace("@MyPK", "'" +
		// this.FK_MapData + "'").Replace("@FK_Flow", "'" + this.FK_Flow + "'")
		// + "\" ><img src='" + item.Img + "' width='16px' /><b>" + item.Name +
		// "</b></a><br><font color=green>" + item.Note + "</font>");
		// }
	...
	public void Page_Load() throws IOException {
		...
		this.Pub1.append(BaseModel.AddTD("colspan=" + md.getTableCol(),
				"<div style='float:left' ><img src='../../DataUser/ICON/Smaller.png' border=0/></div><h3><div style='float:center' >"
						+ md.getName() + "</div></h3>"));
		...
		for (int i = 0; i < this.get_gfs().size(); i++) {
			...
			if (this.get_gfs().size() == 1)
				this.Pub1.append(BaseModel.AddTD(
						"colspan=" + md.getTableCol()
								+ " class=GroupField valign='top' align:left style='height: 24px;align:left' ",
						"<div style='text-align:left; float:left'>&nbsp;<a href=\"javascript:GroupField('"
								+ this.FK_MapData + "','" + gf.getOID() + "')\" >" + gf.getLab()
								+ "</a></div><div style='text-align:right; float:right'></div>"));
			else
				this.Pub1.append(BaseModel.AddTD(
						"colspan=" + md.getTableCol()
								+ " class=GroupField valign='top'  style='height: 24px;align:left' ",
						"<div style='text-align:left; float:left'><img src='./Style/Min.gif' alert='Min' id='Img"
								+ gf.getIdx() + "'  border=0 />&nbsp;<a href=\"javascript:GroupField('"
								+ this.FK_MapData + "','" + gf.getOID() + "')\" >" + gf.getLab()
								+ "</a></div><div style='text-align:right; float:right'> <a href=\"javascript:GFDoUp('"
								+ gf.getOID()
								+ "')\" ><img src='../Img/Btn/Up.gif' class='Arrow' border=0/></a> <a href=\"javascript:GFDoDown('"
								+ gf.getOID()
								+ "')\" ><img src='../Img/Btn/Down.gif' class='Arrow' border=0/></a></div>"));
			this.Pub1.append(BaseModel.AddTREnd());
			// #endregion 输出分组栏.
			...
			for (int a = 0; a < mattrs.size(); a++) {
				...
				if (attr.getIsBigDoc() && (attr.getColSpan() == md.getTableCol() || attr.getColSpan() == 0)) {
					...
					this.Pub1.append("<TD colspan=" + md.getTableCol() + " width='100%' height='"
							+ Float.toString(attr.getUIHeight()) + "px' >");
					this.Pub1.append("<span style='float:left' height='" + Float.toString(attr.getUIHeight()) + "px' >"
							+ this.GenerLab(attr, 0, count) + "</span>");
					this.Pub1.append(
							"<span style='float:right' height='" + Float.toString(attr.getUIHeight()) + "px'  >");
					...
					lab = ui.creatLabel("Lab" + attr.getKeyOfEn());// this.Pub1.GetLabelByID("Lab"
																	// +
																	// attr.getKeyOfEn());
					String ctlID = mytbLine.getId();
					lab.setText("<a href=\"javascript:TBHelp('" + ctlID + "','" + basePath + "','" + md.getNo() + "','"
							+ attr.getKeyOfEn() + "')\">默认值</a>");
				...
				if (attr.getIsBigDoc()) {
					...
					this.Pub1.append("<TD  colspan=" + attr.getColSpan() + " width='50%' height='"
							+ Float.toString(attr.getUIHeight()) + "px' >");
					this.Pub1.append("<span height='" + Float.toString(attr.getUIHeight()) + "px' style='float:left'>"
							+ this.GenerLab(attr, 0, count) + "</span>");
					this.Pub1
							.append("<span height='" + Float.toString(attr.getUIHeight()) + "px' style='float:right'>");
					Label lab = new Label();
					lab.setId("Lab" + attr.getKeyOfEn());
					lab.setText("默认值");
					this.Pub1.append(lab);
					...
					lab = ui.creatLabel("Lab" + attr.getKeyOfEn());// this.Pub1.GetLabelByID("Lab"
																	...
																	// attr.KeyOfEn);
					String ctlID = mytbLine.getId();
					lab.setText("<a href=\"javascript:TBHelp('" + ctlID + "','"
							+ req.getSession().getServletContext().getRealPath("/") + "','" + md.getNo() + "','"
							+ attr.getKeyOfEn() + "')\">默认值</a>");
					this.Pub1.append(BaseModel.AddTDEnd());
				...
				if (attr.getLGType() == BP.En.FieldTypeS.Normal) {
					...
					case BP.DA.DataType.AppString:
						...
						if (attr.getIsSigan())
							this.Pub1.append(BaseModel.AddTD("colspan=" + attr.getColSpan(),
									"<img src='/DataUser/Siganture/" + WebUser.getNo()
											+ ".jpg' border=0 onerror=\"this.src='../../DataUser/Siganture/UnName.jpg'\"/>"));
						else
					...
					if (attr.getLGType() == FieldTypeS.Enum) {
						if (attr.getUIContralType() == UIContralType.DDL) {
							...
							// {
							// RadioButton rb = new RadioButton();
							// rb.setId("RB_" + attr.getKeyOfEn() + "_" +
							// item.getIntKey());
							// rb.setText(item.getLab());
							// if (Integer.toString(item.getIntKey()) ==
							// attr.getDefVal())
							// rb.setChecked(true);
							// else
							// rb.setChecked(false);
							// //有错
							// rb.setGroupName(item.getEnumKey() +
							// attr.getKeyOfEn());
							// this.Pub1.append(rb);
							// }
							this.Pub1.append(BaseModel.AddTDEnd());
				...
				// {
				// case BP.DA.DataType.AppString:
				// this.Pub1.append(BaseModel.AddTDDesc(this.GenerLab(attr, i,
				// count)));
				// tb.setShowType(TBType.TB);
				// tb.setText(attr.getDefVal());
				//
				// if (attr.getIsSigan())
				// this.Pub1.append(BaseModel.AddTD("colspan=" +
				// attr.getColSpan(), "<img src='/DataUser/Siganture/" +
				// WebUser.getNo() + ".jpg' border=0
				// onerror=\"this.src='../../DataUser/Siganture/UnName.jpg'\"/>"));
				// else
				// this.Pub1.append(BaseModel.AddTD("colspan=" +
				// attr.getColSpan(), tb));
				//
				...
				// if (attr.getUIIsEnable())
				// {
				// tbDT.addAttr("onfocus", "WdatePicker({dateFmt:'yyyy-MM-dd
				// HH:mm'});");
				// tbDT.addAttr("class", "TBcalendar");
				// //tbDT.Attributes["onfocus"] =
				// "WdatePicker({dateFmt:'yyyy-MM-dd HH:mm'});";
		...
		if (fwc.getHisFrmWorkCheckSta() != FrmWorkCheckSta.Disable) {
			...
			src += "&r=q" + paras;
			this.Pub1
					.append("<iframe ID='FWC' frameborder=0 style='padding:0px;border:0px;'  leftMargin='0'  topMargin='0' src='"
							+ src + "'  width='100%' height='150px' scrolling=auto  /></iframe>");
			this.Pub1.append(BaseModel.AddTDEnd());
			this.Pub1.append(BaseModel.AddTREnd());
		}
		// #endregion
		...
		if (mes.size() != 0) {
			...
			// "<script language='JavaScript' src='../CCForm/MapExt.js'
			// defer='defer' type='text/javascript' ></script>");

			// this.Page.RegisterClientScriptBlock("dC",
			// "<script language='JavaScript' src='" + appPath +
			// "DataUser/JSLibData/" + this.FK_MapData + ".js' ></script>");
		...
		for (int i = 0; i < mes.size(); i++) {
			...
			if (me.getExtType() != null) {
				if (me.getExtType() == MapExtXmlList.DDLFullCtrl) {
					DDL ddlOper = ui.creatDDL("DDL_" + me.getAttrOfOper());// this.Pub1.GetDDLByID("DDL_"
																			// +
																			// me.AttrOfOper);
					if (ddlOper == null)
						continue;
					ddlOper.addAttr("onchange",
							"DDLFullCtrl(this.value,\'" + ddlOper.getId() + "\', \'" + me.getMyPK() + "\')");// Attributes["onchange"]
																												// =
				...
				if (me.getExtType() == MapExtXmlList.ActiveDDL) {
					DDL ddlPerant = ui.creatDDL("DDL_" + me.getAttrOfOper());// this.Pub1.GetDDLByID("DDL_"
																				// +
																				// me.AttrOfOper);
					...
					ddlPerant.addAttr("onchange",
							"DDLAnsc(this.value,\'" + ddlChild.getId() + "\', \'" + me.getMyPK() + "\')");// Attributes["onchange"]
																											// =
																											// "DDLAnsc(this.value,\'"
																											// +
																											// ddlChild.ClientID
																											...
																											// +
																											// "\')";
					// ddlChild.Attributes["onchange"] =
					// "ddlCity_onchange(this.value,'" + me.MyPK + "')";
				}
				if (me.getExtType() == MapExtXmlList.TBFullCtrl) {
					TextBox tbAuto = ui.creatTextBox("TB_" + me.getAttrOfOper());// this.Pub1.GetTBByID("TB_"
																					// +
																					// me.AttrOfOper);
					if (tbAuto == null) {
						me.Delete();
				...
				if (me.getExtType() == MapExtXmlList.InputCheck) {
					...
					if (tbJS != null)
						tbJS.addAttr(me.getTag2(), me.getTag1() + "(this);");// Attributes[me.Tag2]
																				// +=
																				// me.Tag1
																				// +
																				// "(this);";
				...
				if (me.getExtType() == MapExtXmlList.PopVal) {
					TextBox tbPop = ui.creatTextBox("TB_" + me.getAttrOfOper());// this.Pub1.GetTBByID("TB_"
																				// +
																				// me.AttrOfOper);
				}
				if (me.getExtType() == MapExtXmlList.AutoFull) {
					String js = "\t\n <script type='text/javascript' >";
					TextBox tb = ui.creatTextBox("TB_" + me.getAttrOfOper());// this.Pub1.GetTBByID("TB_"
																				// +
																				// me.AttrOfOper);
					if (tb == null)
						continue;
					...
					for (int j = 0; j < mattrs.size(); j++) {
						...
						// this.Pub1.GetTBByID(tbID).Attributes["onkeyup"] =
						// "javascript:Auto" + me.AttrOfOper + "();";
					...
					js += left + right + ";";
					js += " \t\n  document.forms[0]." + tb.getId() + ".value= VirtyMoney(document.forms[0]."
							+ tb.getId() + ".value ) ;";
					js += "\t\n } ";
					js += "\t\n</script>";
					this.Pub1.append(js);
			...
			// DDL ddlOper =ui.creatDDL("DDL_" + me.getAttrOfOper());//
			// this.Pub1.GetDDLByID("DDL_" + me.AttrOfOper);
			// if (ddlOper == null)
			// continue;
			// ddlOper.addAttr("onchange", "DDLFullCtrl(this.value,\'" +
			// ddlOper.getId() + "\', \'" + me.getMyPK() +
			// "\')");//Attributes["onchange"] = "DDLFullCtrl(this.value,\'" +
			// ddlOper.ClientID + "\', \'" + me.MyPK + "\')";
			// break;
			// case MapExtXmlList.ActiveDDL:
			// DDL ddlPerant =ui.creatDDL("DDL_" + me.getAttrOfOper());//
			// this.Pub1.GetDDLByID("DDL_" + me.AttrOfOper);
			// DDL ddlChild = ui.creatDDL("DDL_" + me.getAttrsOfActive());
			// if (ddlChild == null || ddlPerant == null)
			// {
			...
			// }
			//
			// ddlPerant.addAttr("onchange", "DDLAnsc(this.value,\'" +
			// ddlChild.getId() + "\', \'" + me.getMyPK() +
			// "\')");//Attributes["onchange"] = "DDLAnsc(this.value,\'" +
			// ddlChild.ClientID + "\', \'" + me.MyPK + "\')";
			// // ddlChild.Attributes["onchange"] =
			// "ddlCity_onchange(this.value,'" + me.MyPK + "')";
			// break;
			// case MapExtXmlList.TBFullCtrl: // 自动填充.
			// TextBox tbAuto = ui.creatTextBox("TB_" +
			// me.getAttrOfOper());//this.Pub1.GetTBByID("TB_" + me.AttrOfOper);
			// if (tbAuto == null)
			// {
			// me.Delete();
			// continue;
			// }
			// tbAuto.addAttr("onkeyup", "DoAnscToFillDiv(this,this.value,\'" +
			...
			// TextBox tbJS =ui.creatTextBox("TB_" + me.getAttrOfOper());
			// if (tbJS != null)
			// tbJS.addAttr(me.getTag2(), me.getTag1() +
			// "(this);");//Attributes[me.Tag2] += me.Tag1 + "(this);";
			// else
			// me.Delete();
			// break;
			// case MapExtXmlList.PopVal: //弹出窗.
			// TextBox tbPop =ui.creatTextBox("TB_" + me.getAttrOfOper());//
			// this.Pub1.GetTBByID("TB_" + me.AttrOfOper);
			// //tb.Attributes["ondblclick"] = "ReturnVal(this,'" + me.Doc +
			// "','sd');";
			// break;
			// case MapExtXmlList.AutoFull: //自动填充.
			// String js = "\t\n <script type='text/javascript' >";
			// TextBox tb =ui.creatTextBox("TB_" + me.getAttrOfOper());//
			// this.Pub1.GetTBByID("TB_" + me.AttrOfOper);
			// if (tb == null)
			// continue;
			//
			...
			// continue;
			//
			// ui.creatTextBox(tbID).addAttr("onkeyup", "javascript:Auto" +
			// me.getAttrOfOper() + "();");
			// //this.Pub1.GetTBByID(tbID).Attributes["onkeyup"] =
			// "javascript:Auto" + me.AttrOfOper + "();";
			// right = right.replace("@" + mattr.getName(), " parseFloat(
			// document.forms[0]." + mytb.getId() + ".value.replace( ',' , '' )
			// ) ");
			// right = right.replace("@" + mattr.getKeyOfEn(), " parseFloat(
			// document.forms[0]." + mytb.getId() + ".value.replace( ',' , '' )
			// ) ");
			// }
			// js += "\t\n function Auto" + me.getAttrOfOper() + "() { ";
			// js += left + right + ";";
			// js += " \t\n document.forms[0]." + tb.getId() + ".value=
			// VirtyMoney(document.forms[0]." + tb.getId() + ".value ) ;";
			// js += "\t\n } ";
			// js += "\t\n</script>";
			// this.Pub1.append(js);
			// break;
		...
		for (int j = 0; j < mattrs.size(); j++) {
			...
			if (attr.getUIIsEnable() == false || attr.getUIVisible() == false)
				continue;
			UiFatory ui = new UiFatory();
			ui.creatTextBox("TB_" + attr.getKeyOfEn()).addAttr("onblur",
					"checkLength(this,'" + attr.getMinLen() + "','" + attr.getMaxLen() + "')");
		}
		// for (MapAttr attr : mattrs)
		// {
		//
		// //this.Pub1.GetTextBoxByID("TB_" + attr.KeyOfEn).Attributes["onblur"]
		// = "checkLength(this,'" + attr.MinLen + "','" + attr.MaxLen + "')";
		// }
		...
		// M2M.getNoOfObj() + "','TD" + M2M.getNoOfObj() + "')\", 200);";
		// }
		for (int i = 0; i < _aths.size(); i++) {
			FrmAttachment ath = (FrmAttachment) _aths.get(i);
			if (ath.getIsAutoSize())
				myjs += "\t\n window.setInterval(\"ReinitIframe('F" + ath.getMyPK() + "','TD" + ath.getMyPK()
						+ "')\", 200);";
		}
		...
		// {
		// myjs += "\t\n window.setInterval(\"ReinitIframe('F" + fr.getMyPK() +
		// "','TD" + fr.getMyPK() + "')\", 200);";
		// }
		...
		// #region 处理隐藏字段。
		DataTable dt = DBAccess.RunSQLReturnTable("SELECT * FROM Sys_MapAttr WHERE FK_MapData='" + this.FK_MapData
				+ "' AND GroupID NOT IN (SELECT OID FROM Sys_GroupField WHERE EnName='" + this.FK_MapData + "')");
		if (dt.Rows.size() != 0) {
	...
	public void InsertObjects(boolean isJudgeRowIdx) {
		...
		for (int i = 0; i < this.get_dtls().size(); i++) {
			...
			this.Pub1.append("<TD colspan=" + md.getTableCol()
					+ " class=TRSum  ><div style='text-align:left; float:left'><a href=\"javascript:EditDtl('"
					+ this.FK_MapData + "','" + dtl.getNo() + "')\" >" + dtl.getName()
					+ "</a></div><div style='text-align:right; float:right'><a href=\"javascript:document.getElementById('F"
					+ dtl.getNo() + "').contentWindow.AddF('" + dtl.getNo()
					+ "');\"><img src='../Img/Btn/New.gif' border=0/>插入列</a><a href=\"javascript:document.getElementById('F"
					+ dtl.getNo() + "').contentWindow.AddFGroup('" + dtl.getNo()
					+ "');\"><img src='../Img/Btn/New.gif' border=0/>插入列组</a><a href=\"javascript:document.getElementById('F"
					+ dtl.getNo() + "').contentWindow.CopyF('" + dtl.getNo()
					+ "');\"><img src='../Img/Btn/Copy.gif' border=0/>复制列</a><a href=\"javascript:document.getElementById('F"
					+ dtl.getNo() + "').contentWindow.HidAttr('" + dtl.getNo()
					+ "');\"><img src='../Img/Btn/Copy.gif' border=0/>隐藏列</a><a href=\"javascript:document.getElementById('F"
					+ dtl.getNo() + "').contentWindow.DtlMTR('" + dtl.getNo()
					+ "');\"><img src='../Img/Btn/Copy.gif' border=0/>多表头</a> <a href='Action.aspx?FK_MapData="
					+ dtl.getNo() + "' >从表事件</a> <a href=\"javascript:DtlDoUp('" + dtl.getNo()
					+ "')\" ><img src='../Img/Btn/Up.gif' border=0/></a> <a href=\"javascript:DtlDoDown('" + dtl.getNo()
					+ "')\" ><img src='../Img/Btn/Down.gif' border=0/></a></div></td>");
			this.Pub1.append(BaseModel.AddTREnd());

			myidx++;
			this.Pub1.append(BaseModel.AddTR(" ID='" + currGF.getIdx() + "_" + myidx + "' "));
			this.Pub1.append("<TD colspan=" + md.getTableCol() + " ID='TD" + dtl.getNo() + "' height='50px' width='"
					+ this.md.getTableWidth() + "' > ");
			String src = "MapDtlDe.aspx?DoType=Edit&FK_MapData=" + this.FK_MapData + "&FK_MapDtl=" + dtl.getNo();
			this.Pub1.append("<iframe ID='F" + dtl.getNo()
					+ "' frameborder=0 style='padding:0px;border:0px;'  leftMargin='0'  topMargin='0' src='" + src
					+ "'  width='" + this.md.getTableWidth() + "' height='10px' scrolling=no  /></iframe>");
			this.Pub1.append(BaseModel.AddTDEnd());
			this.Pub1.append(BaseModel.AddTREnd());
		}
		// #endregion 增加从表
		...
		for (int i = 0; i < this.get_aths().size(); i++) {
			...
			this.Pub1.append(BaseModel.AddTR(" ID='" + currGF.getIdx() + "_" + myidx + "' "));
			this.Pub1.append("<TD colspan=" + md.getTableCol()
					+ " class=TRSum  ><div style='text-align:left; float:left'><a href=\"javascript:EditAth('"
					+ this.FK_MapData + "','" + dtl.getNoOfObj() + "')\" >" + dtl.getName()
					+ "</a></div><div style='text-align:right; float:right'><a href=\"javascript:AthDoUp('"
					+ dtl.getMyPK()
					+ "')\" ><img src='../Img/Btn/Up.gif' border=0/></a> <a href=\"javascript:AthDoDown('"
					+ dtl.getMyPK() + "')\" ><img src='../Img/Btn/Down.gif' border=0/></a></div></td>");
			this.Pub1.append(BaseModel.AddTREnd());

			myidx++;
			this.Pub1.append(BaseModel.AddTR(" ID='" + currGF.getIdx() + "_" + myidx + "' "));
			this.Pub1.append(
					"<TD colspan=" + md.getTableCol() + " ID='TD" + dtl.getMyPK() + "' height='50px' width='1000px'>");

			String src = "../CCForm/AttachmentUpload.aspx?PKVal=0&Ath=" + dtl.getNoOfObj() + "&FK_MapData="
					+ this.FK_MapData + "&FK_FrmAttachment=" + dtl.getMyPK();
			if (dtl.getIsAutoSize())
				this.Pub1.append("<iframe ID='F" + dtl.getMyPK()
						+ "' frameborder=0 style='padding:0px;border:0px;'  leftMargin='0'  topMargin='0' src='" + src
						+ "' width='100%' height='10px' scrolling=no  /></iframe>");
			else
				this.Pub1.append("<iframe ID='F" + dtl.getMyPK()
						+ "' frameborder=0 style='padding:0px;border:0px;'  leftMargin='0'  topMargin='0' src='" + src
						+ "' width='" + dtl.getW() + "' height='" + dtl.getH() + "' scrolling=auto  /></iframe>");
		...
		for (int i = 0; i < this.get_dot2dots().size(); i++) {
			...
			this.Pub1.append(BaseModel.AddTR(" ID='" + currGF.getIdx() + "_" + myidx + "' "));
			this.Pub1
					.append("<TD colspan=4 class=TRSum  ><div style='text-align:left; float:left'><a href=\"javascript:EditM2M('"
							+ this.FK_MapData + "','" + m2m.getNoOfObj() + "')\" >" + m2m.getName()
							+ "</a></div><div style='text-align:right; float:right'><a href=\"javascript:M2MDoUp('"
							+ m2m.getMyPK()
							+ "')\" ><img src='../Img/Btn/Up.gif' border=0/></a> <a href=\"javascript:M2MDoDown('"
							+ m2m.getMyPK() + "')\" ><img src='../Img/Btn/Down.gif' border=0/></a></div></td>");
			this.Pub1.append(BaseModel.AddTREnd());

			myidx++;
			String src = "";
			if (m2m.getHisM2MType() == M2MType.M2M)
				src = "../CCForm/M2M.aspx?FK_MapData=" + this.FK_MapData + "&NoOfObj=" + m2m.getNoOfObj() + "&OID=0";
			else
				src = "../CCForm/M2MM.aspx?FK_MapData=" + this.FK_MapData + "&NoOfObj=" + m2m.getNoOfObj() + "&OID=0";

			switch (m2m.getShowWay()) {
			case FrmAutoSize:
				// this.Pub1.AddTR(" ID='" + currGF.Idx + "_" + myidx + "'");
				// this.Pub1.Add("<TD colspan=4 ID='TD" + m2m.NoOfObj + "'
				// width='100%'>");
				// this.Pub1.Add("<iframe ID='F" + m2m.NoOfObj + "'
				// frameborder=0 style='padding:0px;border:0px;' leftMargin='0'
				// topMargin='0' src='" + src + "' width='100%' scrolling=no
				// /></iframe>");
				// this.Pub1.AddTDEnd();
				// this.Pub1.AddTREnd();
				...
				myidx++;
				this.Pub1.append(BaseModel.AddTR(" ID='" + currGF.getIdx() + "_" + myidx + "' "));
				this.Pub1.append("<TD colspan=" + md.getTableCol() + " ID='TD" + m2m.getNoOfObj()
						+ "' height='50px' width='1000px'>");
				this.Pub1.append("<iframe ID='F" + m2m.getNoOfObj()
						+ "' frameborder=0 style='padding:0px;border:0px;'  leftMargin='0'  topMargin='0' src='" + src
						+ "' width='100%' height='10px' scrolling=no  /></iframe>");
				this.Pub1.append(BaseModel.AddTDEnd());
				this.Pub1.append(BaseModel.AddTREnd());
			...
			case FrmSpecSize:
				this.Pub1.append(BaseModel.AddTR(" ID='" + currGF.getIdx() + "_" + myidx + "'"));
				this.Pub1.append("<TD colspan=" + md.getTableCol() + "ID='TD" + m2m.getNoOfObj() + "' height='"
						+ m2m.getH() + "' width='" + m2m.getW() + "'  >");
				this.Pub1.append("<iframe ID='F" + m2m.getNoOfObj() + "' src='" + src
			...
			case WinOpen:
				...
				this.Pub1.append("<a href=\"javascript:WinOpen('" + src + "&IsOpen=1','" + m2m.getW() + "','"
						+ m2m.getH() + "');\"  />" + m2m.getName() + "</a>");
				this.Pub1.append(BaseModel.AddTDEnd());
				this.Pub1.append(BaseModel.AddTREnd());
				break;
			default:
		...
		for (int i = 0; i < this.get_frams().size(); i++) {
			...
			int myidx = rowIdx + 20;
			this.Pub1.append(BaseModel.AddTR(" ID='" + currGF.getIdx() + "_" + myidx + "' "));
			// this.Pub1.Add("<TD colspan=4 class=TRSum ><div
			// style='text-align:left; float:left'><a
			// href=\"javascript:EditDtl('" + this.FK_MapData + "','" + dtl.No +
			// "')\" >" + dtl.Name + "</a></div><div style='text-align:right;
			// float:right'><a href=\"javascript:document.getElementById('F" +
			// dtl.No + "').contentWindow.AddF('" + dtl.No + "');\"><img
			// src='../Img/Btn/New.gif' border=0/>插入列</a><a
			// href=\"javascript:document.getElementById('F" + dtl.No +
			// "').contentWindow.CopyF('" + dtl.No + "');\"><img
			// src='../Img/Btn/Copy.gif' border=0/>复制列</a><a
			// href=\"javascript:DtlDoUp('" + dtl.No + "')\" ><img
			// src='../Img/Btn/Up.gif' border=0/></a> <a
			// href=\"javascript:DtlDoDown('" + dtl.No + "')\" ><img
			// src='../Img/Btn/Down.gif' border=0/></a></div></td>");
			this.Pub1.append("<TD colspan=" + md.getTableCol()
					+ " class=TRSum  ><div style='text-align:left; float:left'><a href=\"javascript:EditFrame('"
					+ this.FK_MapData + "','" + fram.getMyPK() + "')\" >" + fram.getName()
					+ "</a></div><div style='text-align:right; float:right'><a href=\"javascript:FrameDoUp('"
					+ fram.getMyPK()
					+ "')\" ><img src='../Img/Btn/Up.gif' border=0/></a> <a href=\"javascript:FrameDoDown('"
					+ fram.getMyPK() + "')\" ><img src='../Img/Btn/Down.gif' border=0/></a></div></td>");
			this.Pub1.append(BaseModel.AddTREnd());

			myidx++;
			this.Pub1.append(BaseModel.AddTR(" ID='" + currGF.getIdx() + "_" + myidx + "' "));
			if (fram.getIsAutoSize())
				this.Pub1.append("<TD colspan=" + md.getTableCol() + " ID='TD" + fram.getMyPK()
						+ "' height='50px' width='1000px'>");
			else
				this.Pub1.append("<TD colspan=" + md.getTableCol() + " ID='TD" + fram.getMyPK() + "' height='"
						+ fram.getH() + "' width='" + fram.getW() + "' >");

			String src = fram.getURL(); // "MapDtlDe.aspx?DoType=Edit&FK_MapData="
			...
			if (fram.getIsAutoSize())
				this.Pub1.append("<iframe ID='F" + fram.getMyPK()
						+ "' frameborder=0 style='padding:0px;border:0px;'  leftMargin='0'  topMargin='0' src='" + src
						+ "' width='100%' height='100%' scrolling=no  /></iframe>");
			else
				this.Pub1.append("<iframe ID='F" + fram.getMyPK()
						+ "' frameborder=0 style='padding:0px;border:0px;'  leftMargin='0'  topMargin='0' src='" + src
						+ "' width='" + fram.getW() + "' height='" + fram.getH() + "' scrolling=no  /></iframe>");

			// this.Pub1.Add("<iframe ID='F" + fram.No + "' frameborder=0
			// style='padding:0px;border:0px;' leftMargin='0' topMargin='0'
			// src='" + src + "' width='" + fram.W + "px' height='" + fram.H +
			// "px' scrolling=no /></iframe>");
	...
	public String GenerLab_arr(MapAttr attr, int idx, int i, int count) {
		String divAttr = " onmouseover=FieldOnMouseOver('" + attr.getMyPK() + "') onmouseout=FieldOnMouseOut('"
				+ attr.getMyPK() + "') ";
		...
		if (attr.getHisEditType() == EditType.Edit || attr.getHisEditType() == EditType.UnDel) {
			switch (attr.getLGType()) {
			case Normal:
				lab = "<a  href=\"javascript:Edit('" + this.FK_MapData + "','" + attr.getMyPK() + "','"
						+ attr.getMyDataType() + "');\">" + lab + "</a>";
				break;
			case FK:
				lab = "<a  href=\"javascript:EditTable('" + this.FK_MapData + "','" + attr.getMyPK() + "','"
						+ attr.getMyDataType() + "');\">" + lab + "</a>";
				break;
			case Enum:
				lab = "<a  href=\"javascript:EditEnum('" + this.FK_MapData + "','" + attr.getMyPK() + "','"
						+ attr.getMyDataType() + "');\">" + lab + "</a>";
				break;
			default:
				break;
		...
		if (idx == 0) {
			/* 第一个。 */
			return "<div " + divAttr + " >" + lab + "<a href=\"javascript:Down('" + this.FK_MapData + "','"
					+ attr.getMyPK()
					+ "','1');\" ><img src='../Img/Btn/Right.gif' class='Arrow' alt='向右动顺序' border=0/></a></div>";
		}

		if (idx == count - 1) {
			/* 到数第一个。 */
			return "<div " + divAttr + " ><a href=\"javascript:Up('" + this.FK_MapData + "','" + attr.getMyPK()
					+ "','1');\" ><img src='../Img/Btn/Left.gif' alt='向左移动顺序' class='Arrow' border=0/></a>" + lab
					+ "</div>";
		}
		return "<div " + divAttr + " ><a href=\"javascript:Up('" + this.FK_MapData + "','" + attr.getMyPK()
				+ "','1');\" ><img src='../Img/Btn/Left.gif' alt='向下移动顺序' class='Arrow' border=0/></a>" + lab
				+ "<a href=\"javascript:Down('" + this.FK_MapData + "','" + attr.getMyPK()
				+ "','1');\" ><img src='../Img/Btn/Right.gif' alt='向右移动顺序' class='Arrow' border=0/></a></div>";
	}
	...
	public String GenerLab(MapAttr attr, int i, int count) {
		...
		String divAttr = " onDragEnd=onDragEndF('" + attr.getMyPK() + "','" + attr.getGroupID()
				+ "');  onDrag=onDragF('" + attr.getMyPK() + "','" + attr.getGroupID() + "'); ";
		divAttr += " onDragOver=FieldOnMouseOver('" + attr.getMyPK() + "','" + attr.getGroupID()
				+ "');  onDragEnter=FieldOnMouseOver('" + attr.getMyPK() + "','" + attr.getGroupID() + "'); ";
		divAttr += " onDragLeave=FieldOnMouseOut();";

		// divAttr += " onDragLeave=FieldOnMouseOut('" + attr.MyPK + "','" +
		// attr.GroupID + "');";
	...
	public String GenerLab_bak(MapAttr attr, int idx, int i, int count) {
		String divAttr = " onDragEnd=onDragEndF('" + attr.getMyPK() + "','" + attr.getGroupID() + "')  onDrag=onDragF('"
				+ attr.getMyPK() + "','" + attr.getGroupID()
				+ "')  onMouseUp=alert('sss'); onmouseover=FieldOnMouseOver('" + attr.getMyPK() + "','"
				+ attr.getGroupID() + "') onmouseout=FieldOnMouseOut('" + attr.getMyPK() + "','" + attr.getGroupID()
				+ "') ";
		String lab = attr.getName();
		if (attr.getMyDataType() == DataType.AppBoolean && attr.getUIIsLine())
			lab = "编辑";
