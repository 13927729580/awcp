package org.jflow.framework.controller.wf.admin.xap.controller;
...
import BP.WF.Data.BillType;
import BP.WF.Data.BillTypes;
...
public class BillControlle {
	...
	public void btn_Gener_Click(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		String url = Glo.getCCFlowAppPath() + "WF/Admin/Bill.jsp?FK_Flow=" + FK_Flow + "&NodeID=" + FK_Node
				+ "&DoType=Edit&RefNo=" + RefNo;
		// this.Response.Redirect(url, true);
		response.sendRedirect(url);
	}

	@RequestMapping(value = "/btn_Click", method = RequestMethod.POST)
	public void btn_Click(@RequestParam("f") MultipartFile file, HttpServletRequest request,
			HttpServletResponse response) throws IllegalStateException, IOException {
		// private void btn_Click(HttpServletRequest request,HttpServletResponse
		// response) {
		...
		String RefNo = request.getParameter("RefNo");
		// String content =
		// ContextHolderUtils.getRequest().getParameter("FK_Node");
		...
		if (!StringHelper.isNullOrEmpty(RefNo)) {
			...
			// 0 bt.FK_BillType =
			// this.Ucsys1.GetDDLByID("DDL_BillType").SelectedItemStringVal;
			// 1
			// ((DDL)(controls.get("DDL_BillType"))).getSelectedItemStringVal();
			...
			if (bt.getHisBillFileType() == BillFileType.RuiLang) {
				tempName = "Temp.grf";
				temp = BP.Sys.SystemConfig.getPathOfCyclostyleFile() + File.separator + "Temp.grf";
				try {
				...
				tempName = "Temp.rtf";
				temp = BP.Sys.SystemConfig.getPathOfCyclostyleFile() + File.separator + "Temp.rtf";
				try {
					file.transferTo(new File(temp));
				} catch (IllegalStateException e) {
					// TODO Auto-generated catch block
			...
			// 检查文件是否正确。
			try {
				String[] paras = BP.DA.Cash.GetBillParas_Gener(tempName,
						nd.getHisFlow().getHisGERpt().getEnMap().getAttrs());
			} catch (RuntimeException ex) {
				BaseModel.ToErrorPage("错误信息" + ex.getMessage());
				// this.UCSys2.append(this.AddMsgOfWarning("错误信息",
				// ex.getMessage()));
				return;
			}
			String fullFile;
		...
		if (bt.getHisBillFileType() == BillFileType.RuiLang) {
			...
			tmp = SystemConfig.getPathOfCyclostyleFile() + File.separator + "Temp.grf";
			...
			tmpName = "Temp.rtf";
			tmp = SystemConfig.getPathOfCyclostyleFile() + File.separator + "Temp.rtf";
			try {
				file.transferTo(new File(tmp));
			} catch (IllegalStateException e) {
				e.printStackTrace();
		...
		// 检查文件是否正确。
		try {
			String[] paras1 = BP.DA.Cash.GetBillParas_Gener(tmpName,
					nd.getHisFlow().getHisGERpt().getEnMap().getAttrs());
		} catch (RuntimeException ex) {
			BaseModel.ToErrorPage("Error:" + ex.getMessage());
			// 增加错误信息在页面显示
			// this.UCSys2.append(this.AddMsgOfWarning("Error:",
			// ex.getMessage()));
			return;
		}

		try {
			String fullFile1 = FileFullPath(fileName, bt, request, response); // BP.Sys.SystemConfig.PathOfCyclostyleFile
																				// +
																				// "\\"
																				// +
																				// bt.No
		...
		for (BillTemplate Btmp : BillTemplates.convertBillTemplates(tmps)) {
			Billids += "@" + Btmp.getNo();
		}
	...
	public void btn_Del_Click(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		// C# TO JAVA CONVERTER TODO TASK: There is no preprocessor in Java:
		/// #endregion 更新节点信息。
		// this.Response.Redirect("Bill.aspx?FK_Flow=" + this.getFK_Flow() +
		// "&NodeID=" + this.getNodeID(), true);
	 ...
	 * 
	 */
	public final String FileFullPath(String fileName, BillTemplate bt, HttpServletRequest request,
		...
		String filePath = BP.Sys.SystemConfig.getPathOfCyclostyleFile() + File.separator + bt.getNo() + fileType;
		// String NodeID=ContextHolderUtils.getRequest().getParameter("NodeID");

		// String
		// FK_Flow=ContextHolderUtils.getRequest().getParameter("FK_Flow");
		String NodeID = request.getParameter("NodeID");
		String FK_Flow = request.getParameter("FK_Flow");
		Node curNode = new Node(NodeID);
		// 表单树时对存放路径进行修改
		if (curNode.getFormType() == NodeFormType.SheetTree) {
			bt.setUrl("FlowFrm" + File.separator + FK_Flow + File.separator + NodeID + File.separator
					+ fileName.replace(fileType, ""));
			filePath = BP.Sys.SystemConfig.getPathOfCyclostyleFile() + File.separator + "FlowFrm" + File.separator
					+ FK_Flow + File.separator + NodeID;
			...
			filePath = BP.Sys.SystemConfig.getPathOfCyclostyleFile() + File.separator + "FlowFrm" + File.separator
					+ FK_Flow + File.separator + NodeID + File.separator + fileName;
		}
		return filePath;
	}
