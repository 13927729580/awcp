...
import org.jflow.framework.system.ui.core.CheckBoxList;
...
public class GroupModel extends BaseModel {
	 ...
	 * 日志对象
	 */
	private static Log logger = LogFactory.getLog(GroupModel.class);
	...
	public GroupModel(HttpServletRequest request, HttpServletResponse response) {
	...
	public void initFXPoject() {
		...
		// 如果包含年度年月.
		if (HisEn.getEnMap().getAttrs().Contains("FK_NY") && HisEn.getEnMap().getAttrs().Contains("FK_ND"))
			IsContinueNDYF = "TRUE";
		else
	...
	private void BindNums() {
		...
		for (Attr attr : attrs) {
			...
			if (NDXRptBaseAttr.OID.equals(key) || NDXRptBaseAttr.FID.equals(key) || "MID".equals(key)
					|| NDXRptBaseAttr.PWorkID.equals(key) || NDXRptBaseAttr.FlowEndNode.equals(key)
					|| "WorkID".equals(key)) {
				continue;
			}
			// #endregion 排除不必要的字段。
	...
	public DataTable initPub() {
		...
		if (enMap.IsShowSearchKey && this.toolBar.GetTBByID("TB_Key").getText().trim().length() > 1) {
		...
		if (enMap.DTSearchWay != DTSearchWay.None) {
			...
			if (enMap.DTSearchWay == DTSearchWay.ByDate) {
				where += addAnd + "( " + field + ">='" + dtFrom + " 01:01' AND " + field + "<='" + dtTo
						+ " 23:59')     ";
			} else {
				where += addAnd + "(";
		...
		if (null != orderByReq && !StringHelper.isNullOrEmpty(getOrderBy())
		...
		sql.append(selectSQL).append(sqlOfGroupKey).append(" FROM ").append(this.currMapRpt.getPTable()).append(where)
			...
			for (DataRow dr : dt.Rows) {
				for (Attr attr : attrsOfNum) {
					...
					if (whereOfLJ.length() > 10) {
						sql.append("SELECT SUM(").append(attr.getKey()).append(") FROM ")
								.append(this.currMapRpt.getPTable()).append(whereOfLJ).append(" AND ");
					} else {
						sql.append("SELECT SUM(").append(attr.getKey()).append(") FROM ")
								.append(this.currMapRpt.getPTable()).append(" WHERE ");
					...
					for (Attr attr1 : AttrsOfGroup) {
						...
						if ("FK_NY".equals(key)) {
							sql.append(" FK_NY <= '").append(dr.getValue("FK_NY")).append("' AND FK_ND='")
									.append(dr.getValue("FK_NY").toString().substring(0, 4)).append("' AND ");
					...
					if (attr.getMyDataType() == DataType.AppInt)
						...
						dr.setValue(attr.getKey() + "Amount",
								DBAccess.RunSQLReturnValDecimal(sqlTemp, new BigDecimal(0), 2));
				}
			}
		}
		...
		if (StateNumKey.indexOf("=AMOUNT") != -1) {
			...
			for (Attr attr : attrsOfNum) {
				...
				UCSys1.append("<td class='GroupTitle'>本月</td>"); // 本月
																	// this.ToE("OrderCondErr")
			...
			for (Attr attr : AttrsOfGroup) {
				if (attr.getKey().equals(getOrderBy())) {
					if ("Down".equals(getOrderWay())) {
						UCSys1.append(AddTDGroupTitle("<a href='" + getPageID() + ".jsp?FK_Flow=" + getFkFlow()
								+ "&DoType=" + this.getDoType() + "&RptNo=" + getRptNo() + "&OrderBy=" + attr.getKey()
								+ "&OrderWay=Up' >" + attr.getDesc() + "<img src='" + basePath
								+ "WF/Img/ArrDown.gif' border=0/></a>"));
					} else {
						UCSys1.append(AddTDGroupTitle("<a href='" + getPageID() + ".jsp?FK_Flow=" + getFkFlow()
								+ "&DoType=" + this.getDoType() + "&RptNo=" + getRptNo() + "&OrderBy=" + attr.getKey()
								+ "&OrderWay=Down' >" + attr.getDesc() + "<img src='" + basePath
								+ "WF/Img/ArrUp.gif' border=0/></a>"));
					}
				} else {
					UCSys1.append(AddTDGroupTitle("<a href='" + getPageID() + ".jsp?FK_Flow=" + getFkFlow() + "&DoType="
							+ this.getDoType() + "&RptNo=" + getRptNo() + "&OrderBy=" + attr.getKey()
							+ "&OrderWay=Down' >" + attr.getDesc() + "</a>"));
				}
			}
				...
				if (key.equals(getOrderBy())) {
					if ("Down".equals(getOrderWay())) {
						if (key.equals(getNumKey()))
							UCSys1.append(AddTDGroupTitle(lab + "<a href='" + getPageID() + ".jsp?FK_Flow="
									+ getFkFlow() + "&DoType=" + getDoType() + "&RptNo=" + getRptNo() + "&NumKey=" + key
									+ "&OrderBy=" + key + "&OrderWay=Up'><img src='" + basePath
									+ "WF/Img/ArrDown.gif' border=0/></a>"));
						else
							UCSys1.append(AddTDGroupTitle("<a href=\"" + getPageID() + ".jsp?FK_Flow=" + getFkFlow()
									+ "&DoType=" + getDoType() + "&RptNo=" + getRptNo() + "&NumKey=" + key + "\" >"
									+ lab + "</a><a href='" + getPageID() + ".jsp?RptNo=" + getRptNo() + "&NumKey="
									+ key + "&OrderBy=" + key + "&OrderWay=Up&FK_Flow=" + getFkFlow() + "'><img src='"
									+ basePath + "WF/Img/ArrDown.gif' border=0/></a>"));
					} else {
						if (key.equals(getNumKey()))
							UCSys1.append(AddTDGroupTitle(lab + "<a href='" + getPageID() + ".jsp?FK_Flow="
									+ getFkFlow() + "&DoType=" + getDoType() + "&RptNo=" + getRptNo() + "&OrderBy="
									+ key + "&NumKey=" + key + "&OrderWay=Down'><img src='" + basePath
									+ "WF/Img/ArrUp.gif' border=0/></a>"));
						else
							UCSys1.append(AddTDGroupTitle("<a href=\"" + getPageID() + ".jsp?FK_Flow=" + getFkFlow()
									+ "&DoType=" + getDoType() + "&RptNo=" + getRptNo() + "&NumKey=" + key + "\" >"
									+ lab + "</a><a href='" + getPageID() + ".jsp?RptNo=" + getRptNo() + "&OrderBy="
									+ key + "&NumKey=" + key + "&OrderWay=Down&FK_Flow=" + getFkFlow() + "'><img src='"
									+ basePath + "WF/Img/ArrUp.gif' border=0/></a>"));
					}
				} else {
					if (key.equals(getNumKey()))
						UCSys1.append(AddTDGroupTitle(lab + "<a href='" + getPageID() + ".jsp?FK_Flow=" + getFkFlow()
								+ "&DoType=" + getDoType() + "&RptNo=" + getRptNo() + "&NumKey=" + key + "&OrderBy="
								+ key + "' ><img src='" + basePath + "WF/Img/ArrDownUp.gif' border=0/></a>"));
					else
						UCSys1.append(AddTDGroupTitle("<a href=\"" + getPageID() + ".jsp?FK_Flow=" + getFkFlow()
								+ "&DoType=" + getDoType() + "&RptNo=" + getRptNo() + "&NumKey=" + key + "\" >" + lab
								+ "</a><a href='" + getPageID() + ".jsp?RptNo=" + getRptNo() + "&NumKey=" + key
								+ "&OrderBy=" + key + "&FK_Flow=" + getFkFlow() + "' ><img src='" + basePath
								+ "WF/Img/ArrDownUp.gif' border=0/></a>"));
				}
			}
		...
		for (Attr attr : AttrsOfGroup) {
			...
			for (DataRow dr : dt.Rows) {
				...
				if (!attr.getUIBindKey().contains(".")) {
					try {
						dr.setValue(attr.getKey() + "T", DBAccess.RunSQLReturnStringIsNull(
								"SELECT Name FROM " + attr.getUIBindKey() + " WHERE No='" + val + "'", val));
					} catch (Exception e) {
		...
		for (DataRow dr : dt.Rows) {
			...
			for (Attr attr : attrsOfNum) {
				...
				try {
					// obj =
					// BigDecimal.valueOf(Double.parseDouble(dr.getValue(attr.getKey()).toString()));
					obj = BigDecimal.valueOf(Double.parseDouble(String.valueOf(dr.getValue(attr.getKey()))));
				} catch (Exception e) {
					System.err.println("GroupModel.java 905L");
					// throw new Exception(dr[attr.Key].ToString() +"@SQL="+ sql
					// +"@"+ex.Message +"@Attr="+attr.Key );
				}

				switch (attr.getMyDataType()) {
				case DataType.AppMoney:
				case DataType.AppRate:
					if (StateNumKey.indexOf(attr.getKey()
							+ "=AMOUNT") != -1) { /* 如果本数据列 包含累计 */
						UCSys1.append(AddTDJE(obj));
						// UCSys1.append(AddTDJE(BigDecimal.valueOf(Double.parseDouble(dr.getValue(attr.getKey()
						// + "Amount").toString()))));
						UCSys1.append(AddTDJE(BigDecimal
								.valueOf(Double.parseDouble(String.valueOf(dr.getValue(attr.getKey() + "Amount"))))));
					} else {
						UCSys1.append(AddTDJE(obj));
					}
					break;
				default:
					if (StateNumKey.indexOf(attr.getKey()
						...
						UCSys1.append(AddTDNum(BigDecimal
								.valueOf(Double.parseDouble(String.valueOf(dr.getValue(attr.getKey() + "Amount"))))));
					} else {
						UCSys1.append(AddTDNum(obj));
					}
			...
			UCSys1.append(AddTD("<a href=\"javascript:OpenEasyUiDialog('" + url
					+ "','eudlgframe','详细信息',700,432,'icon-table',false,null,null,document.getElementById('mainDiv'))\" class='easyui-linkbutton'>详细</a>"));
			UCSys1.append(AddTREnd());
		}
		...
		for (Attr attr : attrsOfNum) {
					...
					for (Attr myattr : attrsOfNum1) {
						if (exp.indexOf("@" + myattr.getKey() + "@") != -1) {
							...
							for (DataRow dr1 : dt.Rows) {
								try {
									d = d.add(BigDecimal
											.valueOf(Double.valueOf(String.valueOf(dr1.getValue(myattr.getKey())))));
								} catch (Exception e) {
									logger.info("ERROR", e);
								}
							}
			...
			case DataType.AppMoney:
			case DataType.AppRate:
				if (StateNumKey.indexOf(
						attr.getKey() + "=AMOUNT") != -1) { /* 如果本数据列 包含累计 */
					d = BigDecimal.valueOf(0);
					for (DataRow dr1 : dt.Rows)
					...
					for (DataRow dr1 : dt.Rows)
						d = d.add(BigDecimal
								.valueOf(Double.valueOf(String.valueOf(dr1.getValue(attr.getKey() + "Amount")))));
					UCSys1.append(AddTDJE(d));
				} else {
					d = BigDecimal.valueOf(0);
					for (DataRow dr1 : dt.Rows) {
						try {
							if (String.valueOf(dr1.getValue(attr.getKey())).equals(null)
									|| String.valueOf(dr1.getValue(attr.getKey())).equals("")) {
								d = d.add(BigDecimal
										.valueOf(Double.valueOf(String.valueOf(dr1.getValue(attr.getKey())))));
							} else {
		...
		if (CB_IsShowPict.checked) {
			...
			try {
				...
				// 增加柱状图
				UCSys4.append(
						"    <div id='column_chart_div'  title='柱状图' data-options=\"iconCls:'icon-columnchart'\" style='padding:5px;text-align:center'  >");
				UCSys4.append(GenerateColumnChart(dt, AttrsOfGroup.get(0).getKey() + "T", AttrsOfGroup.get(0).getDesc(),
						yfields, "分组分析", chartWidth, chartHeight));
				UCSys4.append("    </div>");

				// 增加饼图
				UCSys4.append(
						"    <div id='pie_chart_div' title='饼状图' data-options=\"iconCls:'icon-piechart'\" style='padding:5px;text-align:center'>");
				UCSys4.append(GeneratePieChart(dt, AttrsOfGroup.get(0).getKey() + "T", AttrsOfGroup.get(0).getDesc(),
						yfields, "分组分析 ", chartWidth, chartHeight));
				UCSys4.append("    </div>");

				// 增加拆线图
				UCSys4.append(
						"    <div id='line_chart_div' title='折线图' data-options=\"iconCls:'icon-linechart'\" style='padding:5px;text-align:center'>");
				UCSys4.append(GenerateLineChart(dt, AttrsOfGroup.get(0).getKey() + "T", AttrsOfGroup.get(0).getDesc(),
						yfields, "分组分析 ", chartWidth, chartHeight));
				UCSys4.append("    </div>");
				UCSys4.append("</div>");
