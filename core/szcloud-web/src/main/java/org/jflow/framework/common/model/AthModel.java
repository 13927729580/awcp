...
public class AthModel extends BaseModel {
	 ...
	 * 日志对象
	 */
	private static Log logger = LogFactory.getLog(AthModel.class);
	...
	public AthModel(HttpServletRequest request, HttpServletResponse response, String basePath, String FK_Flow, int OID,
			int FID, int FK_Node) {
	...
	public void init() {
		String sql = "SELECT * FROM Sys_FrmAttachmentDB WHERE FK_FrmAttachment IN (SELECT MyPK FROM Sys_FrmAttachment WHERE  "
				+ BP.WF.Glo.MapDataLikeKey(this.FK_Flow, "FK_MapData") + "  AND IsUpload=1) AND RefPKVal='" + this.OID
				+ "' ORDER BY RDT";

		// string sql = "SELECT * FROM Sys_FrmAttachmentDB WHERE
		// FK_FrmAttachment IN (SELECT MyPK FROM Sys_FrmAttachment WHERE
		// FK_MapData ='ND"+FK_Node+"' AND IsUpload=1) AND RefPKVal='" +
		// this.OID + "' ORDER BY RDT";
		DataTable dt = DBAccess.RunSQLReturnTable(sql);

		if (dt.Rows.size() > 0) {

			Pub1.append(this.AddTable(
					"class='am-table am-table-striped am-table-hover table-main' cellpadding='0' cellspacing='0' border='0' style='width: 100%'"));
			...
			for (DataRow dr : dt.Rows) {
				...
				Pub1.append(this.AddTD("<a href=" + basePath + "WF/CCForm/AttachmentUpload.jsp?DoType=Down&MyPK="
						+ dr.getValue("MyPK") + " target=_sd ><img src=" + basePath + "WF/Img/FileType/"
						+ dr.getValue("FileExts") + ".gif onerror=\"this.src='" + basePath
						+ "WF/Img/FileType/Undefined.gif'\" border=0/>" + dr.getValue("FileName").toString() + "</a>"));
		...
		if (bills.size() > 0) {
			...
			Pub1.append(this.AddTREnd());
			int idx = 0;
			for (Bill bill : Bills.convertBills(bills)) {
				idx++;
				...
				Pub1.append(this.AddTD("<a class='easyui-linkbutton' data-options=\"iconCls:'icon-print'\" href='"
						+ basePath + "WF/Rpt/Bill.aspx?MyPK=" + bill.getMyPK() + "&DoType=Print' >打印</a>"));// .AddTD("<a
																											// class='easyui-linkbutton'
																											// data-options=\"iconCls:'icon-print'\"
																											// href='"
																											// +
																											// basePath+
																											// "WF/Rpt/Bill.aspx?MyPK="
																											// +
																											// bill.getMyPK()
																											// +
																											// "&DoType=Print'
