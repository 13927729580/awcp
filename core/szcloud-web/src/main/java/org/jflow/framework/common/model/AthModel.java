	...
	public void init(){
		...
		if (bills.size() > 0)
		{
			...
			for(Bill bill: Bills.convertBills(bills))
			{
				...
				Pub1.append(this.AddTD("<a class='easyui-linkbutton' data-options=\"iconCls:'icon-print'\" href='" + basePath+ "WF/Rpt/Bill.aspx?MyPK=" + bill.getMyPK() + "&DoType=Print' >打印</a>"));//.AddTD("<a class='easyui-linkbutton' data-options=\"iconCls:'icon-print'\" href='" + basePath+ "WF/Rpt/Bill.aspx?MyPK=" + bill.getMyPK() + "&DoType=Print' >打印</a>");
				Pub1.append(this.AddTREnd());
			}
			Pub1.append(this.AddTableEnd());
		}
