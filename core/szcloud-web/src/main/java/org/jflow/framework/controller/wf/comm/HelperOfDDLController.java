package org.jflow.framework.controller.wf.comm;
...
public class HelperOfDDLController {
	...
	public void SetDataV2(HttpServletRequest request, HttpServletResponse response) {
		// UCSys1.Clear();
		// Entities ens =
		...
		if (selectName.equals("None")) {
			...
			if (isGrade) {
				...
				if (num > 1) {
					...
					UCSys1.append(BaseModel.AddTDTitle("<img src='../../images/Home.gif' border=0 />数据选择导航"));
					...
					for (Entity myen : Entities.convertEntities(ens)) {
						...
						UCSys1.append(BaseModel.AddTD("<a href='#ID" + myen.GetValStringByKey(refKey)
			...
			for (Entity myen : Entities.convertEntities(ens)) {
				...
				if (isGrade) {
					...
					switch (grade) {
					case 2:
						rb.setText("<a href='#top' name='ID" + myen.GetValStringByKey(refKey)
								+ "' ><Img src='../../images/Top.gif' border=0 /></a><b><font color=green>"
								+ myen.GetValStringByKey(refKey) + myen.GetValStringByKey(refText) + "</font></b>");
						break;
					case 3:
						rb.setText("<b>" + myen.GetValStringByKey(refKey) + myen.GetValStringByKey(refText) + "</b>");
						break;
					default:
						rb.setText(myen.GetValStringByKey(refKey) + myen.GetValStringByKey(refText));
						break;
					}
				} else {
				...
				rb.setName("RB_Group");
				String clientscript = "window.returnValue = '" + myen.GetValStringByKey(refKey) + "';window.close();";
		...
		if (attr.getMyFieldType() == FieldType.Enum || attr.getMyFieldType() == FieldType.PKEnum) {
			...
			for (SysEnum se : SysEnums.convertSysEnums(ses)) {
				...
				for (Entity myen : Entities.convertEntities(ens)) {
					...
					// rb.ID = "RB_" + myen.GetValStringByKey(refKey);
					rb.setId("RB_" + myen.GetValStringByKey(refKey));
					String clientscript = "window.returnValue = '" + myen.GetValStringByKey(refKey)
							+ "';window.close();";
		...
		if (attr.getKey().equals("FK_Dept")) {
			...
			for (BP.Port.Dept Dept : Depts.convertDepts(depts)) {
				...
				if (Dept.getGrade() == 2 || Dept.getGrade() == 1) {
					...
					UCSys1.append("<TR>");
					UCSys1.append(BaseModel.AddTDIdx(i));
					UCSys1.append("<a href='#ID_2" + Dept.getNo() + "' >&nbsp;&nbsp;" + Dept.getNo() + "&nbsp;&nbsp;"
							+ Dept.getName() + "</a><BR>");
					UCSys1.append("</TR>");
				}
			}
			...
			for (BP.Port.Dept Dept : Depts.convertDepts(depts)) {
				...
				if (Dept.getGrade() == 2) {
					...
					// border=0 /></a><BR>");
					UCSys1.append(BaseModel.AddTD("&nbsp;&nbsp;<a name='ID_2" + Dept.getNo() + "' >" + Dept.getNo()
					...
					UCSys1.append(BaseModel.AddTD("&nbsp;&nbsp;" + Dept.getNo() + "&nbsp;&nbsp;<a href='#ID"
							+ Dept.getNo() + "' name='#ID_2" + Dept.getNo() + "' >" + Dept.getName() + "</a><BR>"));
				}
			...
			for (BP.Port.Dept groupen : Depts.convertDepts(depts)) {
				 ...
				 */
				UCSys1.append("<TR><TD class='Toolbar' >");
				UCSys1.append("<a href='#ID_2" + groupen.getNo() + "' name='ID" + groupen.getNo()
						+ "' ><Img src='../../images/Top.gif' border=0 /></a>&nbsp;&nbsp;"
			...
			if (size > 19) {
				...
				for (Entity groupen : Entities.convertEntities(ens)) {
					...
					UCSys1.append(BaseModel.AddTR());
					UCSys1.append(BaseModel.AddTDIdx(i));
					UCSys1.append(BaseModel.AddTD("<a href='#ID" + groupen.GetValStringByKey(attr.getUIRefKeyValue())
			...
			for (Entity groupen : Entities.convertEntities(groupens)) {
				...
				UCSys1.append("<TR><TD class='Toolbar' >");
				UCSys1.append("<a href='#top' name='ID" + groupen.GetValStringByKey(attr.getUIRefKeyValue())
						+ "' ><Img src='../../images/Top.gif' border=0 /></a>&nbsp;&nbsp;"
						+ groupen.GetValStringByKey(attr.getUIRefKeyText()));
				UCSys1.append("</TD></TR>");
				UCSys1.append("<TR><TD>");
	...
	// ensOfM.RetrieveAll();
	// //for(Entity en : ensOfM) {
	// for(Entity en : Entities.convertEntities(ensOfM)) {
