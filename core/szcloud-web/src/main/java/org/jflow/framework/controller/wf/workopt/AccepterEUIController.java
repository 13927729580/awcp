...
public class AccepterEUIController {
		...
		try {
			...
			case Url:
				if (MySelector.getSelectorP1().contains("?")) {
					try {
						_response.sendRedirect(MySelector.getSelectorP1() + "&WorkID=" + this.getWorkID() + "&FK_Node="
								+ this.getFK_Node());
					} catch (IOException e) {
						// TODO Auto-generated catch block
						logger.info("ERROR", e);
	...
	public final String BindByEmp() {
		...
		if (nodeDepts != null && nodeDepts.size() > 0) {
			for (NodeDept item : NodeDepts.convertNodeDepts(nodeDepts)) {
				s_checkded += "," + item.getFK_Dept() + ",";
			}
		}
		String BindByEmpSql = String.format(
				"select A.* from (select No,Name,ParentNo  from Port_Dept   WHERE No IN (SELECT FK_Dept "
						+ "FROM Port_Emp WHERE No in(SELECT FK_EMP FROM WF_NodeEmp WHERE FK_Node=%s)) union "
						+ "select No,Name,FK_Dept as ParentNo  from Port_Emp  WHERE No in (SELECT FK_EMP FROM WF_NodeEmp WHERE FK_Node=%s)) A where 1=1 ",
				MySelector.getNodeID(), MySelector.getNodeID());
	 ...
	 * int col = 4; // this.Pub1.AddTable("style='border:0px;width:100%'"); //
	 * foreach (DataRow drGroup in dtGroup.Rows) // { // string ctlIDs = ""; //
	...
	public final String GetTreeJsonByTable(String sql, DataTable tabel, String idCol, String txtCol, String rela,
			Object pId, String CheckedString) {
		String treeJson = "";
