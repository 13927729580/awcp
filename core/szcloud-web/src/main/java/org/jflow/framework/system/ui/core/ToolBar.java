...
public class ToolBar extends PageBase {
	...
	private QueryObject InitQueryObjectByEns(Entities ens, boolean IsShowSearchKey, DTSearchWay dw, String dtKey,
	...
	public void InitFuncEn(UAC uac, Entity en) {
		...
		if (uac.IsInsert) {
			if (en.getEnMap().getEnType() != EnType.Dtl) {
				...
				// this.GetLinkBtnByID(NamesOfBtn.New).OnClientClick =
				// "this.disabled=true;";
			}
		}
		...
		if (uac.IsUpdate) {
			...
			// this.GetLinkBtnByID(NamesOfBtn.Save).OnClientClick =
			// "this.disabled=true;";
		}
		...
		if (uac.IsInsert && uac.IsUpdate) {
			if (en.getEnMap().getEnType() != EnType.Dtl) {
				...
				// this.GetLinkBtnByID(NamesOfBtn.SaveAndNew).OnClientClick =
				// "this.disabled=true;";
			}
		}
		...
		if (uac.IsDelete && pkval != "0" && pkval.length() >= 1) {
			...
			// this.GetLinkBtnByID(NamesOfBtn.Delete).OnClientClick =
			// "this.disabled=true;";
		}
		...
		if (uac.IsAdjunct) {
			...
			if (en.equals("") == false) {
				int i = DBAccess.RunSQLReturnValInt("select COUNT(*) from Sys_FileManager WHERE RefTable='"
						+ en.toString() + "' AND RefKey='" + en.getPKVal() + "'");
				if (i != 0) {
		...
		for (AttrSearch attr : searchs.subList(0, searchs.size())) {
			...
			if ("FK_NY".equals(tempKey)) {
				dt = DBAccess.RunSQLReturnTable(
						"SELECT DISTINCT FK_NY FROM " + currMapRpt.getPTable() + " WHERE FK_NY!='' ORDER BY FK_NY");
				mydll.Items.clear();
				...
				case MyDeptOnly: // 我的部门.
					sql = "SELECT No,Name FROM WF_Emp WHERE FK_Dept='" + WebUser.getFK_Dept()
							+ "' AND No IN (SELECT DISTINCT FlowStarter FROM " + currMapRpt.getPTable()
							+ " WHERE FlowStarter!='')";
					break;
				case MyDeptAndBeloneToMyDeptOnly: // 我的部门，或者隶属我部门下面的部门.
					// #warning 这里有错误，怎么递归循环出来?
					sql = "SELECT No,Name FROM WF_Emp WHERE FK_Dept IN (SELECT FK_Dept FROM  WF_DeptFlowSearch WHERE FK_Emp='"
							+ WebUser.getNo() + "'  AND FK_Flow='" + currMapRpt.getFK_Flow()
							+ "') AND No IN (SELECT DISTINCT FlowStarter FROM " + currMapRpt.getPTable()
							+ " WHERE FlowStarter!='')";
					break;
				...
				case AnyoneAndAnydept: // 任何部门.
					sql = "SELECT No,Name FROM WF_Emp WHERE No IN (SELECT DISTINCT FlowStarter FROM "
							+ currMapRpt.getPTable() + " WHERE FlowStarter!='')";
					break;
				default:
					break;
				}
				...
				if (!SystemConfig.getAppSettings().get("Admin").toString().equals(WebUser.getNo())) {
					dt = DBAccess.RunSQLReturnTable(
							"SELECT No,Name FROM Port_Dept WHERE No IN (SELECT FK_Dept FROM  WF_DeptFlowSearch WHERE FK_Emp='"
									+ WebUser.getNo() + "' AND FK_Flow='" + currMapRpt.getFK_Flow() + "')");
					if (dt.Rows.size() == 0) {
	...
	private void InitByMapV2(Map map, int page, String ensName) {
		...
		for (AttrSearch attr : bigAttrs.subList(0, bigAttrs.size())) {
			...
			// 增加事件.
			DDL ddl = tempContollers.get("DDL_" + attr.Key) == null ? null
					: (DDL) tempContollers.get("DDL_" + attr.Key);
			ddl.addAttr("onchange", "Redirect" + attr.Key + "()");
			DDL ddlSmil = tempContollers.get("DDL_" + attr.RelationalDtlKey) == null ? null
					: (DDL) tempContollers.get("DDL_" + attr.RelationalDtlKey);
			String script = "";
			...
			if (en.getEnMap().getAttrs().Contains(attr.Key)) {
				...
				script += "\t\n		if(data" + attr.Key + "[i][1].substr(0, value_Big_length" + attr.Key
						+ ") == value_Big" + attr.Key + "){";
				script += "\t\n			ddlSmall" + attr.Key + ".options[index" + attr.Key + "++] = new Option(data"
						+ attr.Key + "[i][0],data" + attr.Key + "[i][1]);";
				script += "\t\n		}";
				script += "\t\n	}";
			...
			logger.debug("need to add script for html");
			logger.debug(script);
	...
	private void InitByMapV2(boolean isShowKey, DTSearchWay sw, AttrsOfSearch attrsOfSearch, AttrSearchs attrsOfFK,
			Attrs attrD1, int page, UserRegedit ur) {
		int keysNum = 0;
		if (isShowKey) {
			AddLab("Lab_Key", "关键字:&nbsp;");
			...
			if (attr.getMyFieldType() == FieldType.Enum) {
				...
				if ("BP.Port.Depts".equals(attr.getUIBindKey())) {
					...
					if (depts.size() > SystemConfig.getMaxDDLNum())
						AddLab("lD", "<a href=\"javascript:onDDLSelectedMore('DDL_" + key + "', '" + EnsName
		...
		// logger.debug("html label count:::::::::::" + keysNum);
		if (_AddSearchBtn)
	...
	public LinkButton GetLinkBtnByID(NamesOfBtn search) {
		return uiFactory.getTmpMap().get(search.getCode()) == null ? null
				: (LinkButton) uiFactory.getTmpMap().get(search.getCode());
	}
	...
	/*
	 * public String toString(){ return uiFactory.ListToString(); }
	 */
