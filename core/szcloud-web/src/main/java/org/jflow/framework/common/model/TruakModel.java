...
public class TruakModel extends BaseModel {
	 ...
	 * 日志对象
	 */
	private static Log logger = LogFactory.getLog(TruakModel.class);
	...
	// //fws.Retrieve(ShiftWorkAttr.FK_Node, this.FK_Node, ShiftWorkAttr.WorkID,
	// this.WorkID);
	//
	...
	public final void BindTrack_ViewSpecialWork() {
		...
		// ShiftWorks fws = new ShiftWorks();
		// fws.Retrieve(ShiftWorkAttr.FK_Node, this.FK_Node,
		// ShiftWorkAttr.WorkID, this.WorkID);
	...
	public final void BindTrack_ViewWork() {
		...
		if (wk.RetrieveFromDBSources() == 0) {
			AddEasyUiPanelInfo("打开(" + nd.getName() + ")错误",
					"<h4>当前的节点数据已经被删除！</h4>" + "<p style='font-weight:bold'>造成此问题出现的原因如下：<br /><br />"
							+ "1、当前节点数据被非法删除；<br />" + "2、节点数据是退回人与被退回人中间的节点，这部分节点数据查看不支持。</p>",
					"icon-no");
			return;
		}
		...
		if (frms.size() == 0) {
			if (nd.getHisFormType() == NodeFormType.FreeForm) {
				...
				this.UCEn1.BindCCForm(wk, "ND" + nd.getNodeID(), true, 0); // ,
																			// false,
																			// false,
																			// null);
			...
			if (nd.getHisFormType() == NodeFormType.FixForm) {
				...
				this.UCEn1.BindColumn4(wk, "ND" + nd.getNodeID()); // , false,
																	// false,
																	// null);
				if (wk.getWorkEndInfo().length() > 2) {
					this.UCEn1.appendPub(wk.getWorkEndInfo());
			...
			if (bills.size() >= 1) {
				...
				String urlr = appPath + "WF/WorkOpt/PrintDoc.jsp?FK_Node=" + nd.getNodeID() + "&FID=" + tk.getFID()
						+ "&WorkID=" + tk.getWorkID() + "&FK_Flow=" + tk.FK_Flow;
				this.UCEn1.appendPub(
						"<p><a  href=\"javascript:WinOpen('" + urlr + "','dsdd');\"  />" + title + "</a></p>");
			...
			if (frms.size() == 1) {
				...
				src = fn.getFrmUrl() + ".jsp?FK_MapData=" + frm.getNo() + "&FID=" + fid + "&IsEdit=0&IsPrint=0&FK_Node="
						+ nd.getNodeID() + "&WorkID=" + tk.getWorkID();
				this.UCEn1.appendPub("\t\n <DIV id='" + frm.getNo() + "' style='width:" + frm.getFrmW() + "px; height:"
						+ frm.getFrmH() + "px;text-align: left;' >");
				this.UCEn1.appendPub("\t\n <iframe ID='F" + frm.getNo() + "' src='" + src
						+ "' frameborder=0  style='position:absolute;width:" + frm.getFrmW() + "px; height:"
				...
				for (Frm frm : Frms.convertFrms(frms)) {
					FrmNode fn = frm.HisFrmNode;
					this.UCEn1.appendPub("\t\n <DIV id='" + frm.getNo() + "' style='width:" + frm.getFrmW()
							+ "px; height:" + frm.getFrmH() + "px;text-align: left;' >");
	...
	public final void init() {
		...
		Pub1.append(this.AddTDGroupTitle("表单")); // moved by
													// liuxc,2014-12-18,应zhangqingpeng要求将此列提前
		...
		for (DataRow dr : dt.Rows) {
			...
			java.text.DateFormat df = new java.text.SimpleDateFormat("yy年MM月dd日HH:mm");
			Pub1.append(this.AddTD(df.format(dtt)));

			if (at == ActionType.Forward || at == ActionType.ForwardAskfor || at == ActionType.WorkCheck
																			...
																			// liuxc,2014-12-3,正常结束结点也显示表单
			{
				Pub1.append(this
						.AddTD("<a class='easyui-linkbutton' data-options=\"iconCls:'icon-sheet'\" href=\"javascript:WinOpen('"
								+ BP.WF.Glo.getCCFlowAppPath() + "WF/WFRpt.jsp?WorkID="
								+ dr.getValue(TrackAttr.WorkID).toString() + "&FK_Flow=" + this.getFK_Flow()
								+ "&FK_Node=" + dr.getValue(TrackAttr.NDFrom).toString() + "&DoType=View&MyPK="
								+ dr.getValue(TrackAttr.MyPK).toString() + "','"
								+ dr.getValue(TrackAttr.MyPK).toString() + "');\">表单</a>"));
			} else {
				Pub1.append(this.AddTD(""));
			}

			Pub1.append(this.AddTD(dr.getValue(TrackAttr.NDFromT).toString()));
			Pub1.append(this.AddTD(BP.WF.Glo.GenerUserImgSmallerHtml(dr.getValue(TrackAttr.EmpFrom).toString(),
					dr.getValue(TrackAttr.EmpFromT).toString())));

			if (at == ActionType.FlowOver || at == ActionType.CC || at == ActionType.UnSend) {
				Pub1.append(this.AddTD());
				Pub1.append(this.AddTD());
			} else {
				Pub1.append(this.AddTD(dr.getValue(TrackAttr.NDToT).toString()));
				Pub1.append(this.AddTD(dr.getValue(TrackAttr.EmpToT).toString()));
			}
			...
			Pub1.append(this.AddTD("<img src='../../Img/Action/" + at.toString() + ".png' class='ActionType' border=0/>"
					+ Track.GetActionTypeT(at)));

			// 删除信息.
			String tag = dr.getValue(TrackAttr.Tag) == null ? null : dr.getValue(TrackAttr.Tag).toString();
			if (tag != null) {
				tag = tag.replace("~", "'");
			}

			String msg = dr.getValue(TrackAttr.Msg) == null ? "" : dr.getValue(TrackAttr.Msg).toString();
			switch (at) {
			case CallChildenFlow: // 被调用父流程吊起。
				if (!StringHelper.isNullOrEmpty(tag)) {
					AtPara ap = new AtPara(tag);
					Pub1.append(this.AddTD("class=TD",
							"<a target=b" + ap.GetValStrByKey("PWorkID") + " href='Track.jsp?WorkID="
									+ ap.GetValStrByKey("PWorkID") + "&FK_Flow=" + ap.GetValStrByKey("PFlowNo") + "' >"
									+ msg + "</a>"));
				} else {
			...
			case StartChildenFlow: // 吊起子流程。
				if (!StringHelper.isNullOrEmpty(tag)) {
					AtPara ap = new AtPara(tag);
					Pub1.append(this.AddTD("class=TD",
							"<a target=b" + ap.GetValStrByKey("CWorkID") + " href='Track.jsp?WorkID="
									+ ap.GetValStrByKey("CWorkID") + "&FK_Flow=" + ap.GetValStrByKey("CFlowNo") + "' >"
									+ msg + "</a>"));
				} else {
					Pub1.append(this.AddTD("class=TD", msg));
				}
	...
	// Pub1.append(this.Add("流程发起人：" + gwf.StarterName + "，发起日期：" + gwf.RDT + "
	// ；流程状态：" + gwf.WFStateText);
	//
	// ReturnWorks rws = new ReturnWorks();
	...
	// Pub1.append(this.AddTD("<a href='" + this.Request.ApplicationPath +
	// "WF/WFRpt.jsp?WorkID=" + gwf.WorkID + "&FK_Flow=" + gwf.FK_Flow + "&FID="
	// + gwf.FID + "' target=_b" + gwf.WorkID + ">工作报告</a>");
	// Pub1.append(this.AddTREnd();
	// }
	...
	public final void ADDWork(Work en, ReturnWorks rws, int nodeId) {
		...
		if (enDtls.size() > 0) {
			for (EnDtl enDtl : enDtls) {
				String url = "WFRptDtl.jsp?RefPK=" + en.getPKVal().toString() + "&EnName="
