	...
	public final void BindTrack_ViewWork()
	{
		...
		if (frms.size() == 0)
		{
			if (nd.getHisFormType() == NodeFormType.FreeForm)
			{
				...
				this.UCEn1.BindCCForm(wk, "ND" + nd.getNodeID(), true, 0); //, false, false, null);
...
//C# TO JAVA CONVERTER TODO TASK: There is no preprocessor in Java:
	...
	public final void init()
	{
		...
		for (DataRow dr : dt.Rows)
		{
			...
			if (at == ActionType.Forward || at == ActionType.ForwardAskfor || at == ActionType.WorkCheck || at == ActionType.Order || at == ActionType.FlowOver) //added by liuxc,2014-12-3,正常结束结点也显示表单
			{
				Pub1.append(this.AddTD("<a class='easyui-linkbutton' data-options=\"iconCls:'icon-sheet'\" href=\"javascript:WinOpen('" + BP.WF.Glo.getCCFlowAppPath() + "WF/WFRpt.jsp?WorkID=" + dr.getValue(TrackAttr.WorkID).toString() + "&FK_Flow=" + this.getFK_Flow() + "&FK_Node=" + dr.getValue(TrackAttr.NDFrom).toString() + "&DoType=View&MyPK=" + dr.getValue(TrackAttr.MyPK).toString() + "','" + dr.getValue(TrackAttr.MyPK).toString() + "');\">表单</a>"));
			}
			else
			{
			...
			Pub1.append(this.AddTD(BP.WF.Glo.GenerUserImgSmallerHtml(dr.getValue(TrackAttr.EmpFrom).toString(), dr.getValue(TrackAttr.EmpFromT).toString())));
			...
			Pub1.append(this.AddTD("<img src='../../Img/Action/" + at.toString() + ".png' class='ActionType' border=0/>" + Track.GetActionTypeT(at)));
			...
			switch (at)
			{
				case CallChildenFlow: //被调用父流程吊起。
					if (!StringHelper.isNullOrEmpty(tag))
					{
						...
						Pub1.append(this.AddTD("class=TD", "<a target=b" + ap.GetValStrByKey("PWorkID") + " href='Track.jsp?WorkID=" + ap.GetValStrByKey("PWorkID") + "&FK_Flow=" + ap.GetValStrByKey("PFlowNo") + "' >" + msg + "</a>"));
					}
					else
					{
				...
				case StartChildenFlow: //吊起子流程。
					...
					{
						AtPara ap = new AtPara(tag);
						Pub1.append(this.AddTD("class=TD", "<a target=b" + ap.GetValStrByKey("CWorkID") + " href='Track.jsp?WorkID=" + ap.GetValStrByKey("CWorkID") + "&FK_Flow=" + ap.GetValStrByKey("CFlowNo") + "' >" + msg + "</a>"));
					}
					else
					{
						Pub1.append(this.AddTD("class=TD", msg));
