package org.jflow.framework.controller.app;
...
import org.jflow.framework.common.app.CommModel;
...
public class SearchCommController {
	 ...
	 * 日志对象
	 */
	private static Log logger = LogFactory.getLog(SearchCommController.class);
	...
	@ResponseBody
	public AjaxJson search(TempObject object, HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			try {
				...
				// this.getEnsName() + "&RefVal=" + this.getRefVal() +
				// "&RefKey=" + this.getRefKey() + "&MainEnsName=" +
				// this.getMainEnsName());
				CommModel.BindPageIdxEasyUi(this.UCSys2, qo.GetCount(), this.getPageIdx(map), this.PageSize,
			...
			if (this.HisEn.getEnMap().IsShowSearchKey) {
				...
				if (keyVal.length() >= 1) {
					...
					for (Entity myen : Entities.convertEntities(this.HisEns)) {
						for (Attr attr : attrs) {
							if (attr.getIsFKorEnum())
								continue;

							/*
							 * if (attr.getIsPK()) continue;
							 */
