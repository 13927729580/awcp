...
(function ($) {
    ...
    var fileAjax = function (method, data, callback){
        $.ajax({
            url: basePath + 'ouson/file/' + method + '.do',
            data: data,
            async: false,
    ...
    var initUploader = function (option, uploader) {
        var itemOption = {
            ...
            // 文件接收服务端。
            server: basePath + 'ouson/file/upload.do',
            // 选择文件的按钮。可选。
        ...
        uploader.on('error', function (handler) {
            if (handler == 'F_DUPLICATE') {
                ...
                alert('Single file size can not exceed 2MB.');
            } else if (handler == 'Q_EXCEED_NUM_LIMIT') {
                alert("The number of files exceeded limits , can only upload " + (option.fileNum) + " files.");
            } else if (handler == 'Q_EXCEED_SIZE_LIMIT') {
                alert('File size limit exceeded.');
            } else if (handler == 'Q_TYPE_DENIED') {
            	if(itemOption.accept.extensions!=''){
                	alert('Please upload '+itemOption.accept.extensions+' type of file.');
                }else{
            ...
            fileAjax("get", {'id': response.msg}, function (result) {
                ...
                if(option.auto){
                    ...
                    var tHtml = '<tr id="'+item.id+'" class="item"><td class="hidden formData text-center"><input id="boxs" type="hidden" value="'+item.id+'">' +
                        '<td  class="text-center"><a class="download" href="' + basePath + 'ouson/file/download.do?fileId=' + item.id + '" data-id="' + item.id + '">' + fileNames + '</a></td>' +
                    ...
                    var dHtml = '<div id="' + item.id + '" class="item clearfix">'
                        + '<h4 class="info" data-size="' + item.length + '"><a data-backdrop="static" data-width="1000px" data-height="500px" data-iframe="' + basePath + 'ouson/file/preview.do?fileId=' + item.id + '" data-id="' + item.id + '" data-toggle="modal" class="download">' + item.filename + "</a>&nbsp;&nbsp;--" + toMb(item.length) + 'Mb<span class="cancel"><i class="icon-remove"></i></span></h4>'
                        + '</div>';
                    ...
                    if (result.result == 1) {
                        ...
                        if(option.auto){
                            ...
                            var tHtml = '<tr id="'+file.id+'" class="item"><td class="hidden formData text-center"><input id="boxs" type="hidden" value="'+file.id+'"> </td>' +
                                '<td  class="text-center"><a class="download"  data-backdrop="static" data-width="1000px" data-height="500px"  data-toggle="modal" data-iframe="' + basePath + 'ouson/file/preview.do?fileId=' + file.id + '" data-id="' + file.id + '">' +fileNames + '</a></td>' +
                                '<td  class="text-center">'+_file[_file.length-1]+'</td>';
                            ...
                            var dHtml = '<div id="' + file.id + '" class="item clearfix">';
                                dHtml += '<h4 class="info" data-size="' + file.length + '"><a class="download"  data-backdrop="static" data-toggle="modal" data-width="1000px" data-height="500px" data-iframe="' + basePath + 'ouson/file/preview.do?fileId=' + file.id + '" data-id="' + file.id + '">' + file.filename + "</a>&nbsp;&nbsp;--" + toMb(file.length) + 'Mb';
                                dHtml += option.isReadonly?'':'<span class="cancel"><i class="icon-remove"></i></span>';
                                dHtml += '<a href="' + basePath + 'ouson/file/download.do?fileId=' + file.id + '" class="submit btn btn-mini">下载</a>';
                                dHtml += '</h4></div>';
                            option.$oldlist.append(dHtml);
                            initTable();
                            setInterval(formatUploadTable,100);
