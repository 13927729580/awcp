...
function searchRole(){
	...
	$.post("./component/getSystemId.do",function(data){
		systemId=data.systemId;
	});
	if(systemId!=null){
	var treeNodes=[];
	var url = "./unit/listRolesInSysByAjax.do?boxs="+systemId;
	$.ajaxSetup({ 
		async : false
	}); 
...
function searchGroup(){
	...
	var treeNodes=[];
	var url = "./UserGroup/queryGroup.do?";
	$.ajaxSetup({ 
		async : false
	}); 
	 $.get(url,function(data){
...
function searchNormalPerson(){
    $(".select-panel .panel-body").html('<div class="tip">请从左侧选择菜单（用户）</div>');
    $("#myDepartTab li").removeAttr("class");
    $("#myDepartTab li:first-child").addClass("active");
    $("#departTab").show();
	$("#groupTab").hide();
	$("#tab1").height(316);
	$("#tab1").css("border-left-color","#ccc");
	var url = "./common/user/queryByUser.do?";
	$.ajaxSetup({ 
		async : false
	}); 
	 $.get(url,function(data){
		 ...
		 if(data!=null && data!=""){
		    ...
		    //常用联系人为空 加载所有部门
		    $("#myDepartTab li").removeClass("active");
		    $("#myDepartTab li:last-child").addClass("active");
		    searchDept();
		 }
	 });
...
function searchNormalDepart(){
	...
	var treeNodes=[];
	var url = "./common/user/queryByGroup.do?";
	$.ajaxSetup({ 
		async : false
	}); 
	 $.get(url,function(data){
		 $.each(data,function(i,item){
			 var nodes={
					id:item["groupId"],pId:item["parentGroupId"],name:item["groupChName"]
				}
			treeNodes.push(nodes);
...
//加载所有部门树
function searchAllDepart(){
	searchDept();
}
//加载自定义分组树
function searchDefineGroup(){


}
//加载系统分组树
function searchSystemGroup(){
...
function searchDept(){
	...
	var url = "./taskManagement/persons.do?type=simple";
...
//常用联系人的显示片段
function dataToListByUser(data){
		if(data.length==0){
            $("#tree1").html('<div class="tip">选择常用联系人</div>');
        }else{
            var list='<ul class="name-list clearfix">';
            $.each(data,function(index,item){
               var checkClass = contains(membersID,item.userId)?"icon-check":"icon-check-empty";
                if(index<2){
                    list += (index==0)?'<li style="border-width:1px;width:50%;padding:8px 10px;margin-top:-1px;" title="'+item.name+'"><i data-id="'+item.userId+'" class="'+checkClass+'"></i>'+item.name+'</li>':'<li style="border-top-width:1px;width:50%;padding:8px 10px;margin-top:-1px;"><i data-id="'+item.userId+'" class="'+checkClass+'"></i>'+item.name+'</li>';
                }
                else if((index+1)%2 == 1){
                     list += '<li style="border-left-width:1px;width:50%;padding:8px 10px;margin-top:-1px;" title="'+item.name+'"><i data-id="'+item.userId+'" class="'+checkClass+'"></i>'+item.name+'</li>';
                }
                else{
                    list += '<li style="width:50%;padding:8px 10px;margin-top:-1px;" title="'+item.name+'"><i data-id="'+item.userId+'" class="'+checkClass+'"></i>'+item.name+'</li>';
                }
            });
            list += '</ul>';
            $("#tree1").html(list);
