    ...
    (function ($) {
        $.fn.jalendar = function (options) {
            ...
            $this.append(
                ...
                div('div', 'jalendar-wood').append(
                    ...
                    div('div', 'jalendar-pages').append(
                        ...
                        div('div', 'total-bar').html( lTotalEvents[settings.lang] + '<b style="color: '+'red'+'"></b>'),
                        div('div', 'days')
                    ),
                    div('div', 'add-event').append(
                        div('div', 'add-new').append(
                            ...
                            div('div', 'add-time').append(
                                div('div', 'disabled'),
                                div('div', 'select').addClass('hour').css('background-color', 'red').append(
                                    div('span').text('00'),
                                    div('div', 'dropdown').append(clockHour)
                                ),
                                div('div', 'left').append(':'),
                                div('div', 'select').addClass('min').css('background-color', 'red').append(
                                    div('span').text('00'),
            ...
            function calcMonth() {
                ...
                if ( month == d.getMonth() ) {
                    $this.find('.day.this-month').removeClass('today').eq(date-1).addClass('today').css('color', 'red');
                } else {
            ...
            $this.on('click', '.this-month', function () {
                ...
                $this.children('.jalendar-wood, .wood-bottom').animate({width : '+=0' }, 200, function() {
            ...
            $this.on('click', allDay[0], function(){
                $(this).removeAttr('data-type').attr('data-type', 'enabled').children('.check').children().css('background-color', 'red');
                dropdown[2].children('.dropdown').hide(0);
            ...
            $this.find('.submit').on('click', function(){
                ...
                //ajax 调用后台保存，保存成功后，返回一个uuid,赋值给dataId做唯一标识
