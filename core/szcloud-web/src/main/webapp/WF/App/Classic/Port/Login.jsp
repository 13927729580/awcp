...
<%@ page language="java" contentType="text/html; charset=UTF-8"
    ...
    String user = request.getParameter("TB_No");
	String pass = request.getParameter("TB_Pass"); 
    if(!StringHelper.isNullOrEmpty(isDoLogin) && !StringHelper.isNullOrEmpty(user) && !StringHelper.isNullOrEmpty(pass)){
		...
		try
		{
			...
			{
				WebUser.SignInOfGenerLang(em, WebUser.getSysLang());
		
				if (request.getRequestURL().toString().toLowerCase().contains("wap"))
				{
					WebUser.setIsWap(true);
				}
				else
				{
				...
				WebUser.setToken(request.getSession().getId());
		
				response.sendRedirect("Default.jsp");
				return;
			}
			PubClass.Alert("用户名密码错误，注意密码区分大小写，请检查是否按下了CapsLock.", response);
		}
		catch (RuntimeException ex)
		{
			PubClass.Alert("用户名密码错误!@检查是否按下了CapsLock.", response);
		}
    }
    %>
...
<body>
 <div style="height:56px; width:800px; margin:0 auto;">
        ...
        <a><img src="<%=Glo.getCCFlowAppPath() %>DataUser/ICON/LogBig.png" alt="ccflow" style=" height:60px;" /></a></div>
    ...
    <form id="Form1" method="post" action="Login.jsp?doLogin=true">
    <section class="main" id="mainBg">
     <div id="divMain" style="background-repeat:repeat-x; width:100%; background-position:left; display:block;margin:0;padding:0;border:none" >
		<div class="main-inner" style="background-position: center top; background-repeat:repeat-x; top: 0px; left: 0px;" >
			<div id="theme">
            <img src="<%=Glo.getCCFlowAppPath() %>DataUser/ICON/Login.jpg" style="height:440px; width:900px;" />
            </div>
               ...
               <tr>
                <td >
                 &nbsp;&nbsp;<font size="2">&nbsp;&nbsp;<b>用户名</b></font>
                  ...
                  <td >
                  <input id="TB_No" type="text" name="TB_No" Font-Bold="true" Width="194px" BorderStyle="None" Font-Size="14px"/>
                </td>
               </tr>
               <tr>
                <td >
                 ...
                 <input id="TB_Pass" name="TB_Pass" type="password" Font-Bold="true" Width="194px"  BorderStyle="None" Font-Size="14px"/>
                </td>
               </tr>
               <tr>
                <td colspan='2' align="center">
                	<input type="checkbox" id="IsRemember" name="IsRemember" checked="true" Height="25px">是否记住密码
                </td>
               </tr>
               <tr  style="text-align:center">
                <td colspan="2" class="style8">
                	<input type="submit"  value=""  Style="border-style: none; border-color: inherit; border-width: medium; background: url('Img/btlogin.png'); width: 108px; 
    ...
    <br />
		<div class="footer-inner" id="footerInner">			
			<nav class="footerNav">
                 <%=BP.Sys.SystemConfig.getSysName() %>
			</nav>
		</div>
	</footer>
    </form>
