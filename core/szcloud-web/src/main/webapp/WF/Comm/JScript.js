...
/*
 * function RefMethod1(path, index, warning, target, ensName, keys) { if
 * (warning != null && warning != '' && warning != 'null' && warning !==
 * undefined) { if (confirm(warning)){ var url = "../Comm/RefMethod.jsp?Index=" +
 * index + "&EnsName=" + ensName + keys; if (target == null || target == '') var
 * b = WinOpen(url, 'remmed'); else var a = WinOpen(url, target); return true; }
 * }else{ var url = "../Comm/RefMethod.jsp?Index=" + index + "&EnsName=" +
 * ensName + keys; if (target == null || target == '') var b = WinOpen(url,
 * 'remmed'); else var a = WinOpen(url, target); return true; } }
 */
function TROut(ctrl) {
...
function OpenHelperTBNo(appPath, EnsName, ctl) {
	var url = appPath + '/Comm/DataHelp.htm?' + appPath
			+ '/Comm/HelperOfTBNo.aspx?EnsName=' + EnsName;
	var str = window
			.showModalDialog(
					url,
					'',
					'dialogHeight: 550px; dialogWidth:950px; dialogTop: 100px; dialogLeft: 100px; center: no; help: no');
	if (str == undefined)
		return;
	if (str == null)
		return;
...
function WinShowModalDialog(url, winName) {
	var v = window
			.showModalDialog(
					url,
					winName,
					'scrollbars=yes;resizable=yes;center=yes;minimize:yes;maximize:yes;dialogHeight: 650px; dialogWidth: 850px; dialogTop: 100px; dialogLeft: 150px;');
	return;
}

function WinShowModalDialog(url, winName, w, h) {
	var explorer = window.navigator.userAgent;
	if (explorer.indexOf("Chrome") >= 0) {// 锟饺革拷锟�
		window
				.open(
						url,
						"sd",
						"scrollbars=yes,resizable=yes,center=yes,minimize=yes,maximize=yes,height= 600px,width= 550px, top=50px, left= 650px");
	} else {// IE,锟斤拷锟�
		var v = window
				.showModalDialog(
						url,
						'',
						'scrollbars=yes;resizable=yes;center=yes;minimize:yes;maximize:yes;dialogHeight: 650px; dialogWidth: 550px; dialogTop: 50px; dialogLeft: 650px;');
	}
	return;
}
function WinShowModalDialog_Accepter(url) {// 14.12.11 锟斤拷 选锟斤拷锟斤拷
	// 注锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷
	var explorer = window.navigator.userAgent;
	if (explorer.indexOf("Chrome") >= 0) {// 锟饺革拷锟�
		window
				.open(
						url,
						"sd",
						"scrollbars=yes,resizable=yes,center=yes,minimize=yes,resizable=no,maximize=yes,height= 600px,width= 550px, top=50px, left= 650px");
	} else {// IE,锟斤拷锟�
		var v = window
				.showModalDialog(
						url,
						'',
						'scrollbars=yes;resizable=yes;center=yes;resizable:no;minimize:yes;maximize:yes;dialogHeight: 650px; dialogWidth: 550px; dialogTop: 50px; dialogLeft: 650px;');
	}
	return;
}
function ReturnVal(ctrl, url, winName) {
	// update by dgq 2013-4-12 锟叫讹拷锟斤拷没锟叫ｏ拷
	if (ctrl && ctrl.value != "") {
		if (url.indexOf('?') > 0)
	...
	// // var v = window.showModalDialog(url, winName,
	// 'scrollbars=yes;resizable=yes;center=yes;minimize:yes;maximize:yes;dialogHeight:
	// 650px; dialogWidth: 850px; dialogTop: 100px; dialogLeft: 150px;');
	// var v = window.showModalDialog(url, winName,
	// 'scrollbars=yes;resizable=yes;center=yes;minimize:yes;maximize:yes;dialogHeight:
	// 650px; dialogWidth: 850px; dialogTop: 100px; dialogLeft: 150px;');
	// if (v == null || v == '' || v == 'NaN') {
	...
	// }
	// else {
	// window.showModalDialog(url + '?temp=' + Math.random(), winName,
	// 'scrollbars=yes;resizable=yes;center=yes;minimize:yes;maximize:yes;dialogHeight:
	// 650px; dialogWidth: 850px; dialogTop: 100px; dialogLeft: 150px;');
	// }

	// OpenJbox();
	if (window.ActiveXObject) {// 锟斤拷锟斤拷锟絀E锟斤拷锟斤拷锟街达拷锟斤拷锟斤拷蟹锟斤拷锟�
		var v = window
				.showModalDialog(
						url,
						winName,
		...
		ctrl.value = v;
	} else {// 锟斤拷锟斤拷锟絚hrome锟斤拷执锟斤拷锟斤拷锟叫凤拷锟斤拷a
		var v = window
				.showModalDialog(
						url,
						'',
 ...
 * ********************************************************
 */
/* 锟斤拷4锟斤拷证 锟斤拷锟斤拷锟斤拷遣锟斤拷锟揭伙拷锟斤拷锟斤拷郑锟�onkeypress */
...
function ShowDateTime(appPath, ctrl) {
	...
	val = window.showModalDialog(url, '',
			'dialogHeight: 335px; dialogWidth: 340px; center: yes; help: no');
	if (val == undefined)
		return;
	ctrl.value = val;
}
...
function DefaultVal1(appPath, ctrl, className, attrKey, empId) {
	if (event.button != 2)
		return;
	url = appPath + '/Comm/DataHelp.htm?' + appPath
			+ '/Comm/HelperOfTB.aspx?EnsName=' + className + '&AttrKey='
			+ attrKey + '&empId=' + empId;
	str = ctrl.value;
	str = window
			.showModalDialog(
					url + '&Key=' + str,
					'',
					'dialogHeight: 500px; dialogWidth:850px; dialogTop: 100px; dialogLeft: 100px; center: no; help: no');
	if (str == undefined)
		return;
	ctrl.value = str;
}
...
// 锟矫碉拷cooke .锟斤拷锟斤拷锟絅ull , 锟斤拷锟截碉拷val
...
function HalperOfDDL(appPath, EnsName, refkeyvalue, reftext, ddlID) {
	...
	// modified by ZhouYue 2013-05-20 changed "/DataHelp.htm..." to
	// "../DataHelp.htm"
	if (appPath == '/')
		url = "../HelperOfDDL.jsp?EnsName=" + EnsName + "&RefKey="
				+ refkeyvalue + "&RefText=" + reftext;
	else
		url = appPath + "/Comm/HelperOfDDL.jsp?EnsName=" + EnsName + "&RefKey="
				+ refkeyvalue + "&RefText=" + reftext;
	var str = window
			.showModalDialog(
					url,
					'',
					'dialogHeight: 500px; dialogWidth:800px; dialogTop: 100px; dialogLeft: 100px; center: no; help: no');
	SetDDLVal(ddlID, str);
}
...
function onDDLSelectedMore(ddlID, MainEns, EnsName, refkeyvalue, reftext) {
	var url = '';
	url = "../Comm/DataHelp.htm?HelperOfDDL.jsp?EnsName=" + EnsName
			+ "&RefKey=" + refkeyvalue + "&RefText=" + reftext + "&MainEns="
			+ MainEns + "&DDLID=" + ddlID;
	var str = window
			.showModalDialog(
					url,
...
/* 锟斤拷锟斤拷选锟斤拷 cb1.Attributes["onclick"] = "SetSelected(this,'" + ctlIDs + "')"; */
...
/* 锟斤拷锟斤拷锟斤拷欠锟斤拷锟斤拷侄锟斤拷锟斤拷锟� */
function IsDigit(s) {
	if (s.value == '' || s.value == ' ')
		return true;
	var patrn = new RegExp("^[a-zA-Z][a-zA-Z0-9_]*$");
	if (!patrn.exec(s.value)) {
		alert("锟斤拷锟斤拷锟斤拷锟斤拷母锟斤拷锟斤拷锟街ｏ拷锟斤拷一锟斤拷锟街凤拷锟斤拷锟斤拷锟斤拷锟侥革拷锟�");
		s.value = "";
		return false;
	}
	return true;
...
function TBHelp(ctrl, appPath, enName, attrKey) {
	...
	var url = appPath + "WF/Comm/HelperOfTBEUI.jsp?EnsName=" + enName
			+ "&AttrKey=" + attrKey;
	// url = url + "/WF/Comm/HelperOfTBEUI.aspx?EnsName=" + enName + "&AttrKey="
	// + attrKey;
	var str = window
			.showModalDialog(
					url,
					'sd',
					'dialogHeight: 500px; dialogWidth:400px; dialogTop: 120px; dialogLeft: 200px; center: no; help: no');
	if (str == undefined)
		return;
	// $("*[id$=" + ctrl + "]").val(str);
	 ...
	 * if (window.ActiveXObject) {
	 * 
	 * var str = window.showModalDialog(url, 'sd', 'dialogHeight: 500px;
	 * dialogWidth:400px; dialogTop: 120px; dialogLeft: 200px; center: no; help:
	 * no'); if (str == undefined) return; $("*[id$=" + ctrl + "]").val(str); }
	 * else { $.jBox("iframe:" + url, { title: "4567", width: 800, height: 350,
	 * buttons: { 'OK': 'ok' }, submit: function (v, h, f) { var row =
	 * h[0].firstChild.contentWindow.getSelected(); } }); }
	 */
}
...
function SigantureAct(ele, UserNo, FK_MapData, KeyOfEn, WorkID) {
	if (ele) {
		...
		$.ajax({
			type : "get",
			url : localHref + "/WF/Comm/HelperOfSiganture.do",
			data : json_data,
...
// function closeWins() {if (window.dialogArguments &&
// window.dialogArguments.window) window.dialogArguments.window.location =
// window.dialogArguments.window.location;}
