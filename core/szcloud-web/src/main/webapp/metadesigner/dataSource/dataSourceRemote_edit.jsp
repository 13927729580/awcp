			...
			<form class="form-horizontal" id="groupForm"
				...
				<div class="form-group">
					<input name="id" class="form-control" id="id"
							type="hidden" placeholder="" value="${vo.id}">
					...
					<div class="col-md-4">
						<select data-placeholder="请选择属性类型..." id="sourceType" class="chosen-select form-control" tabindex="2" name="sourceType">
									<option value="MYSQL" <c:if test="${vo.sourceType=='MYSQL'}">selected="selected"</c:if>>MYSQL</option>
				...
				<div class="form-group">
					...
					<div class="col-md-4">
						<input name="sourceDriver" class="form-control" id="sourceDriver"
							type="text" placeholder="" <c:if test="${empty vo.sourceDriver}">value="com.mysql.jdbc.Driver"</c:if>  <c:if test="${not empty vo.sourceDriver}">value="${vo.sourceDriver}"</c:if> >
					</div>
				</div>
				<div class="form-group">
				...
				<div class="form-group"><!-- 表单提交按钮区域 -->
		            <div class="col-md-offset-2 col-md-10">
		            	<button type="button" class="btn btn-success" id="test"><i class="icon-save"></i>测试</button>
		              	<button type="button" class="btn btn-success" id="saveBtn" disabled='disabled'><i class="icon-save"></i>保存</button>
						<button type="button" class="btn" id="undoBtn"><i class="icon-undo"></i>取消</a>
		            </div>
		        </div>
			</form>
			</div>
				...
				var dialog = top.dialog.get(window);
       			...
       			$("#saveBtn").click(function(){
       				$.ajax({
						...
						data:{"id":$("#id").val(),"name":$("#name").val(),"sourceType":$("#sourceType").val(),"sourceDriver":$("#sourceDriver").val(),"sourceUrl":$("#sourceUrl").val(),"userName":$("#userName").val(),"userPwd":$("#userPwd").val()},
						datatype:"json",
