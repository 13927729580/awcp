...
$(function() {
	...
	$(document).delegate('#xf-form-button button', 'click', function(e) {
		switch($(this).html()) {
			...
			case '发起流程':
				...
				break;
			case '驳回':
				$('#xf-form').attr('action', '${scopePrefix}/workflow/bpm/workspace-rollback.do');
				$('#xf-form').submit();
				break;
			case '转办':
				$('#modal form').attr('action', '${scopePrefix}/workflow/bpm/workspace-doDelegate.do');
				$('#modal').modal();
				break;
			case '协办':
				$('#modal form').attr('action', '${scopePrefix}/workflow/bpm/workspace-doDelegateHelp.do');
				$('#modal').modal();
				break;
			case '自由跳转':
				$('#modal form').attr('action', '${scopePrefix}/workflow/bpm/console-jump.do');
				$('#modal').modal();
				break;
		}
	});
	...
	<link type="text/css" rel="stylesheet" href="${scopePrefix}/workflow/widgets/userpicker/userpicker.css">
    <script type="text/javascript" src="${scopePrefix}/workflow/widgets/userpicker/userpicker.js"></script>
	<script type="text/javascript">
		/* $(function() {
			createUserPicker({
				modalId: 'userPicker',
		...
		createUserPicker({
			multiple: true,
			url: '${scopePrefix}/rs/workflow/user/search'
		});
		})
    </script>
  </head>
  ...
  <body>
    <section style="width: 98%;margin: 0 auto;">
	  ...
	  <form id="xf-form" method="post" action="${scopePrefix}/workflow/form/form-completeTask.do" class="xf-form">
