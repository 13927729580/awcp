...
<body>
	<div class="errorPage">
		...
		<div class="buttons">
			<a class="btn btn-primary" href="<%=basePath%>main.jsp">返回主页面</a>
			<a class="btn" href="javascript:history.go(-1)">返回上一页</a>
		</div>
	</div>
	<script>
		$(function(){
			var httpStatus = '<%=request.getAttribute("javax.servlet.error.status_code")%>';
			...
			$.each(statusInfo,function(n,item){
				$statusMessage.html("错误"+httpStatus+":"+item[httpStatus]);
			});
			/* switch(httpStatus){
				case 401:
					$statusCode.html();
