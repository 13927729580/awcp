...
public class PageActBeanWorker {
	...
	/**
	 * @Title: convertConfToAct @Description: JSON对象转化为PageAct对象 @param object
	 * json对象 @return PageAct PageAct对象 @throws
	 */
	public static PageAct convertConfToAct(JSONObject object) {
		...
		if (StringUtils.isBlank(act.getClientScript())) { // 当没有客户端脚本时,设置默认的客户端脚本
			...
			case 2002: // 返回
				act.setClientScript(
						"actBackRun(\"" + act.getPageId() + "\",\"" + act.getExtbute().get("target") + "\");");
				break;
			case 2003: // 删除
			...
			case 2009: // 新增
				act.setClientScript(
						"actNewRun(\"" + act.getPageId() + "\",\"" + act.getExtbute().get("target") + "\");");
				break;
			case 2010: // 编辑
				act.setClientScript(
						"actUpdateRun(\"" + act.getPageId() + "\",\"" + act.getExtbute().get("target") + "\");");
				break;
			case 2014:
				act.setClientScript("downloadFile(\"" + act.getPageId() + "\",true);");
				break;
			case 2015:
				act.setClientScript("actRunWithNoValidators(\"" + act.getPageId() + "\");");
				break;
			case 2016:
				act.setClientScript("downloadFile(\"" + act.getPageId() + "\",false);");
				break;
			default:
				act.setClientScript("actRunWithNoValidators(\"" + act.getPageId() + "\");");
			}
	...
	/**
	 * @Title: convertConfToPageActs @Description: 配置字符串转化为PageAct map @param
	 * confContext 配置字符串 @return Map<String,PageAct> key:pageId--PageAct标识
	 * value:PageAct对象 @throws
	 */
