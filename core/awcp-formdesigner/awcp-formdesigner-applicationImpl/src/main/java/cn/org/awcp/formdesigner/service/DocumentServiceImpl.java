...
public class DocumentServiceImpl implements DocumentService {
    ...
    public String saveModelData(DynamicPageVO pageVO, DocumentVO vo, String datadefineName) {
        ...
        if (dd != null) {
            ...
            String tableName=metaModelServiceImpl.queryByModelCode(md.getModelCode()).getTableName();
            vo.setTableName(tableName);
            Map<String, Object> insert=metaModelOperateServiceImpl.markMetaInsert(data, tableName);
            if(insert==null){
                return null;
            }
            Map<String, Object>  params = (Map<String, Object>)insert.get(MetaModelOperateService.PARAMS);
            String sql=(String) insert.get(MetaModelOperateService.SQL);
            boolean isNumberPrimaryKey= (boolean)insert.get(MetaModelOperateService.IS_NUMBER_PRIMARY_KEY);
            if(isNumberPrimaryKey){
    ...
    public Map saveModelDataFlow(DynamicPageVO pageVO, DocumentVO vo, String datadefineName, boolean masterDateSource) {
        ...
        if (dd != null) {
            ...
            String tableName=metaModelServiceImpl.queryByModelCode(md.getModelCode()).getTableName();
            vo.setTableName(tableName);
            Map<String, Object> map = metaModelOperateServiceImpl.get(id, tableName);
            resultMap = new HashMap();
            if (map != null && map.size() > 0) {
                if (metaModelOperateServiceImpl.update(data, tableName)) {
