...
public class StoreServiceImpl implements StoreService {
	...
	public boolean delete(String[] ids) {
		try {
			for (String id : ids) {
				Store store = BeanUtils.getNewInstance(Store.get(Store.class, id), Store.class);
				store.remove();
			}
			return true;
		} catch (Exception e) {
	...
	public StoreVO findById(String id) throws MRTException {
		 ...
		 * id); Store store = queryChannel.querySingleResult(Store.class, "get",
		 * params);
		 */
		StoreVO vo = BeanUtils.getNewInstance(Store.get(Store.class, id), StoreVO.class);
		return vo;
	}
	...
	public List<StoreVO> findByIds(String[] ids) {
		...
		for (String id : ids) {
			Store store = Store.get(Store.class, id);
	...
	@Override
	public PageList<JSONObject> findComponentByDyanamicPageId(String dyanamicPageId) {
		BaseExample baseExample = new BaseExample();
	...
	@Override
	public PageList<JSONObject> findParentComponentByDyanamicPageId(String dyanamicPageId) {
		BaseExample baseExample = new BaseExample();
		...
		for (StoreVO store : list) {
			JSONObject component = JSON.parseObject(store.getContent());
			String pageId = component.getString("dynamicPageId");
			if (!pageId.equals(dyanamicPageId)) {
				ret.addAll(findParentComponentByDyanamicPageId(component.getString("dynamicPageId")));
			}
			ret.add(component);
		}
	...
	public List<StoreVO> queryComponentByLayoutId(String layoutID, Long systemId) {
		...
		List<Store> result = Store.selectByExample(Store.class, baseExample);
		List<StoreVO> storeList = new ArrayList<StoreVO>();
		for (Store store : result) {
			storeList.add(BeanUtils.getNewInstance(store, StoreVO.class));
		}
